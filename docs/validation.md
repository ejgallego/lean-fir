...
edges over at least 111 cases. Required oracle edges must share one matrix
...
current baseline, the 111 source cases are shared by the source-LCNF and V8
...
The compiler-generated corpus currently has 111 cases. Direct identity cases
...
`Nat.add`, `Int.ofNat`, `Int.neg`, `Int.add`, `Int.decLt`, `ByteArray.size`,
...
re-encode both signed immediate and heap representations; `Int.add` computes
with exact Lean `Int`, while `Int.decLt` returns the scalar `UInt8` discriminant
consumed by lowered pattern matching.  Byte-array size reads the
...
The default native-to-V8 matrix covers all 111 corpus cases, including a natural
