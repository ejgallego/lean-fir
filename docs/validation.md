...
directions, classify both constructors, and convert a multi-limb `Nat` to both
...
Six paired integer division/remainder cases cover all four divisor/dividend
sign quadrants, a negative multi-limb dividend, and a zero divisor. Each pins
...
Three paired natural division/remainder cases cover tagged arithmetic,
multi-limb quotient normalization, and the zero-divisor convention. Each pins
`Nat.div` followed by `Nat.mod` exactly once; division by zero returns zero
while remainder by zero returns the original dividend.
...
external dispatch per case.
Six String-measurement cases compare Unicode scalar length and UTF-8 byte size
for empty, ASCII, and embedded-NUL/BMP/non-BMP inputs. The non-ASCII boundary
pins the intentional distinction between three characters and seven encoded
bytes, with one exact external dispatch and no result allocation.
Eight String-navigation cases pin raw UTF-8 positions through exact
`String.Internal.posOf`, `String.Internal.offsetOfPos`, and
`String.Internal.next` dispatch. They distinguish found and missing non-BMP
searches, scalar boundaries from continuation-byte positions, clamped
past-end offsets, and one-byte advancement at a continuation or exact end.
Six String-extraction cases allocate full and non-BMP substrings, then pin
invalid-begin, invalid-end, past-end, and reversed-position behavior. The two
invalid-boundary cases are permanent native/LCNF/V8 regressions for
`FIR-BUG-wasm-none-string-extract-invalid-boundary`.
Nine String-construction cases pin `String.Internal.append` and
`String.Internal.pushn` against the native oracle. They cover empty, NUL, BMP,
and non-BMP contents; zero and repeated push counts; retained-source
copy-on-write; and one String used simultaneously as append's consumed left and
borrowed right operand. Exact final-impure and external traces distinguish
...
heap-natural input/result. Exact `Nat.sub` cases separately cover a retained
multi-limb result, equality, and saturating underflow. Runner-supplied
`ByteArray` identity, size,
...
multiplication, division, remainder, and saturating subtraction share one exact
binary-`Nat` adapter that decodes tagged or heap operands and re-encodes through
the same tagged/heap boundary as the interpreter. Division and remainder
explicitly model their distinct zero-divisor results. The three natural
decisions share an exact comparison adapter, preserve heap and world, and
return the scalar `UInt8` discriminant consumed by lowered Boolean matching.
...
bytes. The String navigation adapters preserve Lean's raw UTF-8 byte-position
semantics: search returns a byte index, byte-to-character offset rounds a
...
re-encode both signed immediate and heap representations; `Int.add`, `Int.ediv`,
...
return Lean 4.33's unboxed Boolean as `uint8`, covering both one and zero; the
...
well. The two Boolean-capture regressions execute on all three
native/LCNF/V8 edges. Their exact traces require tagged Boolean boxing,
partial application, closure invocation, unboxing, and ownership operations;
the structural erased-facade and precise UInt8-boxing repair recorded by
`FIR-BUG-wasm-none-generic-scalar-closure-admission` therefore remains an
executable admission requirement rather than being replaced by a weaker
monomorphic fixture.
...
After the structural scalar-closure admission landed, all ten unsigned cases
compile through the public provider and agree across native Lean, LCNF, and
V8. Matching source and V8 coverage domains require the complete five-width,
zero/maximum matrix.
...
All twenty signed closures now compile through the same public provider and
agree across native Lean, LCNF, and V8. Matching source and V8 coverage
domains require all five widths, all four boundary values, both Wasm lanes,
and every sign-bit pair.
...
The closure-use multiplicity matrix covers zero, one, two, and three uses of
the same mixed-kind closure. Every source program captures a large `Nat`,
`String`, `ByteArray`, boxed `USize` and `UInt32`, and exact-bit
`Float32`/`Float` values. The zero-use case passes the closure across a noinline
consumer, so final LCNF retains four boxes and a real `pap`; its exact 14-step
trace executes one release and none of the statically present closure-body
unbox/constructor/write forms. The one-use case pins 36 transitions. The
two-use case pins 62, including both `fvar` invocations, eight unboxes, ten
decrements, two mixed constructors, their `uset`/`sset` writes, projections,
and the final result pair. The three-use case pins 87 transitions, three
`fvar` invocations, twelve unboxes, fifteen decrements, three mixed results,
and the two constructors of its nested result triple. The latter two cases
distinguish shared intermediate applications from the unique final
application; all four carry one multiplicity tag so the matrix cannot shrink
silently. Matching source and V8 coverage domains require the zero-use,
three-use, shared-intermediate, unique-final, and complete four-case paths.
All four compile through the public provider and agree across native Lean,
LCNF, and V8. Their exact LCNF traces remain the ownership witness even though
the real-engine tier records only the observable result. The captured
...
The repeated-child release pair adds alias multiplicity to that boundary. It
stores the same retained leaf in two fields of one erased-neighbor owner. The
unique-owner case executes two consecutive field projections and decrements,
then reuses the now-exclusive surviving leaf; its exact 62-step trace contains
four `inc`, three `dec`, three `ctor`, five `oproj`, and three `oset` forms.
The paired case retains the whole owner, so release stops before either field:
both original fields remain observable and the outside leaf update allocates.
Its exact 64-step trace contains eight `inc`, three `dec`, six `ctor`, three
`oproj`, and exactly zero `oset` forms. Native Lean, final LCNF, and real V8
...
negative values represented by heap integer objects.
Multi-limb signed identities, negation in both directions, constructor
classification, and exact `Nat → Int` conversion extend those checks through
`2^128 + 17`. A heap-backed
