...
Status: **PA0 complete; PA1 theorem extraction and PA2 branch assembly are
active; case admission is compiler-derived.**
...
|---:|---|---|---|---|---|---|---|---|
| 1 | return | `.ret` | `.ret`; cost `0`; exact `SemanticValueAtAbi functionResult` | Existing: `ConcreteStructuredAlignedValidationState.admit_return`, `ConcreteStructuredAlignedValidationState.returnSemantic_ofUseSite`, `ConcreteStructuredReturnUseSiteProvenanceAt`, and `ConcreteStructuredValidatedCodeOutcome.admit_return_of_compiler` derive the complete current admission. Missing: construct the use-site predicate from the precise producer origin on non-directional object-family edges. | `SemanticEnvAtLocalKinds.ofStateRelated` derives ordinary local typing from the live relation. Validation still proves only carrier compatibility at a non-directional use, so the precise producer-origin fact remains genuinely semantic; no compiler-local equation or admission object remains client-supplied. | C | W6-Results | `sumTo`; precise `.object`/`.tagged` negative fixture |
...
| 4 | direct call | `.directCall` | `.directCall`; cost `0`; `DirectInternalCallSite` | Existing: `supportedNamedCall_internal_facts`, `effectiveDeclarationResultKind?_declared_refines`, `ConcreteStructuredAlignedValidationState.directInternalCallBoundary`, `ConcreteStructuredValidationLocalsAgree.compileArgs_of_supported`, `SemanticArgumentsAtAbi.ofKindsRefine`, `ConstructorArgumentsRelated.ofSemanticValuesAtAbi`, `DirectInternalCallCompilerAdmission.toSite_of_step`, and `ConcreteStructuredValidatedCodeOutcome.admit_directCall_of_compiler` reconstruct the complete zero-cost current admission. Missing: construct `ConcreteStructuredDirectCallArgumentsAt` from semantic heap-reference provenance for compiler-compatible arguments whose row is `tobject` but whose parameter requires `object`. | Production now requires the effective callee result to directionally refine the source `let` result for non-cached calls. Across W7's 11,487 result edges no product edge was rejected; the full validation corpus additionally preserves a legitimate compatible reverse object-family result for the separate nullary-cache lane. All 158 non-refining argument edges are exactly `tobject -> object`; they are now the only ordinary direct-call gap, and an origin census determines the minimal compiler-owned analysis needed to close them. No compiler-local agreement, destination equation, or dynamic call-site object remains client-supplied. | C(arguments only) | W6-Results | `sumTo`; `direct-call`; reverse result negative fixture; `tobject -> object` argument fixtures |
...
- Fourth PA1 result slice closes the ordinary direct-call result half.
  ...
  destination-local selection from residual validation.
- Fifth PA1 result slice removes directional argument refinement from the
  static compiler admission. `SemanticArgumentsAtAbi` types only the current
  source argument row at the callee ABI; successful `evalArgs` transports it
  to `SemanticValuesAtAbi`, and
  `ConstructorArgumentsRelated.ofSemanticValuesAtAbi` reclassifies the same
  physical row without changing values or addresses. The live state relation
  and `SemanticArgumentsAtAbi.ofKindsRefine` discharge every directional row.
  Consequently the remaining direct-call obligation is exactly the semantic
  heap-reference origin of the 158 observed `tobject -> object` uses, not a
  blanket argument invariant.
- First PA2 direct-call slice internalizes residual local-row agreement in
  `DirectInternalCallCompilerAdmission` and publishes
  `ConcreteStructuredValidatedCodeOutcome.admit_directCall_of_compiler`.
  The theorem derives exact zero-cost admission directly from the guarded
  validated outcome and current successful source step. Its only open input,
  `ConcreteStructuredDirectCallArgumentsAt`, is the current argument row at
  the compiler-selected callee ABI and is precisely the producer-origin fact
  being audited for the 158 non-refining uses.
- First PA2 return slice publishes
  `ConcreteStructuredValidatedCodeOutcome.admit_return_of_compiler`. Residual
  validation and the live physical relation construct the complete zero-cost
