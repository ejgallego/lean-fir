...
| 6 | lazy-cache hit | `.lazyHit` | `.lazyHit`; cost `0`; call/generated environment/current global lookup | `ConcreteStructuredLazyReadyAdmission.of_runtimeLookup` selects the hit directly from the current semantic global table; `ConcreteStructuredAlignedValidationState.lazyCacheCallSupported` derives the exact `leanCompatible` result condition and obtains the destination lane from `ConcreteResidualLocalAlignment`. `ConcreteSupportedFunction.residualLocalAlignment` plus the generic `letHead`/`letContinuation` rules derive that row from production lowering. | Branch choice, directional compatibility, and destination provenance are closed without a cache-specific invariant. The remaining interface task is to retain the generic residual row in the validated global relation. | B(relation wiring) | W6-Results | cached Nat/String/Array hit cases |
| 7 | lazy-cache miss | `.lazyMiss` | `.lazyMiss`; cost `0`; initializer execution and empty lookup | `ConcreteStructuredLazyReadyAdmission.of_runtimeLookup` derives the empty lookup. `ConcreteStructuredSourceAdmissionSafeAt.lazy_of_compiler` consumes only the compiler site plus an empty-cache capability. | Internal non-object misses are implemented. External initializers and `.object`/`.tobject` result publication are accepted by lowering but not yet covered by this simulator branch; they must be implemented rather than hidden in a universal source invariant. | C(backend coverage) | W6-Results | miss and first-publication cases; external/object miss fixtures |
