...
theorem decrementReferenceOnceFuel_ok_mono
    ...
    {check : Bool} {descriptors : ClosureDescriptorTable} (fuelLe : fuel ≤ more)
    (operation :
      decrementReferenceOnceFuel fuel state object check descriptors = .ok result) :
    decrementReferenceOnceFuel more state object check descriptors = .ok result := by
  induction fuel generalizing more state result object check descriptors with
  | zero =>
      cases more with
...
theorem ConstructorObjectRel.readOwnedReferences
  ...
  obtain ⟨actualHeader, actualRead, headerKind, _, _, objectCount, _, _⟩ :=
    related.header
...
theorem LiveHeapRel.decrementReferenceOnceFuel_tagged
    ...
    (fuel : Nat) (check : Bool)
    (descriptors : ClosureDescriptorTable := #[]) :
    decrementReferenceOnceFuel fuel state word check descriptors =
      if check then .ok state else .error (.source .expectedHeapReference) := by
  cases tagged with
  | immediate actualPayload fits =>
      cases fuel <;>
...
theorem LiveHeapRel.decrementReferenceOnce_tagged
    ...
    decrementReferenceOnce state word check descriptors =
      if check then .ok state else .error (.source .expectedHeapReference) := by
  unfold decrementReferenceOnce
  exact related.decrementReferenceOnceFuel_tagged tagged _ check descriptors
...
theorem OwnershipValueRel.releaseStep
    ...
    ((∀ location, value ≠ .object (.heap location)) ∧
      decrementReferenceOnceFuel fuel state word true descriptors = .ok state) := by
...
theorem OwnershipValuesRel.foldlM_refines
    ...
    (recurse : ∀ {before : MemoryState} {semantic nextSemantic : RuntimeState}
      ...
      ∃ after,
        decrementReferenceOnceFuel fuel before address true descriptors = .ok after ∧
    ...
    ∃ finalState,
      words.foldlM (init := state) (fun next child =>
        decrementReferenceOnceFuel fuel next child true descriptors) = .ok finalState ∧
  ...
  | @cons word value words values head tail ih =>
      have noOpCase
          (concreteHead :
            decrementReferenceOnceFuel fuel state word true descriptors = .ok state)
          (semanticHead :
            (match value with
            | .object (.heap child) =>
...
theorem decrementReferenceOnceFuel_leaf_one_eq_public
    ...
    (owned : readOwnedReferences state address header descriptors = .ok [])
    (fuel : Nat) (check : Bool) :
    decrementReferenceOnceFuel (fuel + 1) state address check descriptors =
      decrementReferenceOnce state address check descriptors := by
  have heap :=
...
theorem Header.readStringBytes_of_write_eq_ok
    (before after : LinearMemory) (address : Word32) (updatedHeader : Header)
    (index count : Nat)
    (headerInBounds : address.value + headerBytes ≤ before.size)
    (written : updatedHeader.write before address = .ok after) :
    readStringBytes after address.value index count =
      readStringBytes before address.value index count := by
  induction count generalizing index with
  | zero => rfl
  | succ count ih =>
      unfold readStringBytes
      rw [Header.readByte_of_write_eq_ok_other before after address updatedHeader
        (address.value + headerBytes + index) headerInBounds written
        (.inr (by omega))]
      rw [ih (index + 1)]
...
/-- A common ownership-header rewrite preserves the entire canonical Natural
validator boundary.  Callers supply only the new ownership invariant; raw
header exactness, canonical limbs, payload reads, and extent are framed here
once for increment, decrement, and persistence transitions. -/
theorem NaturalValidatorAdmission.writeOwnershipMetadata
    {state : MemoryState} {address : Word32} {value : Nat} {header : Header}
    (related : NaturalValidatorAdmission state address value header)
    (valid : state.FrontierInvariant) (nextCount : UInt32)
    (nextPersistent : Bool)
    (nextOwnership : if nextPersistent then nextCount = 0 else nextCount ≠ 0) :
    ∃ result updatedHeader memory,
      writeLiveHeader state address updatedHeader = .ok result ∧
      updatedHeader = {
        header with refCount := nextCount, persistent := nextPersistent } ∧
      ...
      updatedHeader.write state.memory address = .ok memory ∧
      result.FrontierInvariant ∧
      NaturalValidatorAdmission result address value updatedHeader := by
  obtain ⟨result, updatedHeader, memory, operation, updatedEq, resultEq,
      headerWrite, finalValid, headerAfter⟩ :=
    writeOwnershipMetadata_header valid related.headerRead related.headerOwned
      nextCount nextPersistent
  subst updatedHeader
  subst result
  have headerInBounds : address.value + headerBytes ≤ state.memory.size :=
  ...
  refine {
    headerRead := headerAfter
    rawHeader := Header.ExactWords.ofWrite_eq_ok headerInBounds headerWrite
  ...
  exact related.limbAt offset limb limbAt
...
theorem ConstructorObjectRel.writeOwnershipMetadata
  ...
  obtain ⟨objectHeader, objectHeaderRead, headerKind, allocationBytes, tag,
      objectCount, usizeCount, scalarCount⟩ := related.header
...
theorem ConstructorObjectRel.incrementReference
  ...
  obtain ⟨objectHeader, objectHeaderRead, headerKind, _, _, _, _, _⟩ :=
    related.header
  rw [headerRead] at objectHeaderRead
  cases objectHeaderRead
  have heap :=
...
theorem ClosureCellRel.decrementReferenceOnce_above_one
    ...
    {address : Word32} {cell : HeapCell}
    (related : ClosureCellRel state witness address cell)
    (valid : state.FrontierInvariant) (ordinary : cell.persistent = false)
...
theorem LiveCellRel.decrementReferenceOnce_boxed_above_one
  ...
  | @boxed kind scalar header _ descriptor objectEq objectRelated refCount persistent live =>
      ...
      obtain ⟨result, updatedHeader, operation, updatedEq, finalValid,
          headerReadAfter, objectAfter⟩ :=
        objectRelated.decrementReferenceOnce_above_one valid cell.rc refCount oneLt
          headerOrdinary check
      subst updatedHeader
      have nextFits : cell.rc - 1 < UInt32.size := by
        have oldFits := UInt32.toNat_lt_size header.refCount
        rw [refCount] at oldFits
  ...
  | @boxed kind scalar header _ descriptor objectEq objectRelated refCount persistent live =>
      ...
      obtain ⟨result, updatedHeader, operation, updatedEq, finalValid,
        objectAfter⟩ := objectRelated.incrementReference valid cell.rc amount
          refCount headerOrdinary fits check
      subst updatedHeader
      refine ⟨result, operation, finalValid, ?_⟩
      apply LiveCellRel.boxed descriptor (by simpa using objectEq) objectAfter
      · simp only
...
theorem LiveCellRel.incrementReference_constructor
  ...
  | constructor descriptor objectEq objectRelated headerRead headerKind refCount
        persistent live =>
      have headerOrdinary : _ := persistent.trans ordinary
      obtain ⟨result, updatedHeader, operation, updatedEq, finalValid,
          headerReadAfter, objectAfter⟩ :=
        objectRelated.incrementReference headerRead valid cell.rc amount refCount
...
theorem LiveCellRel.incrementReference_natural
  ...
  | @natural value header _ descriptor objectEq objectRelated refCount
      ...
      have headerCountNe : header.refCount ≠ 0 := by
        simpa [headerOrdinary] using objectRelated.ownership
      have oldCountPositive : 0 < cell.rc := by
        have headerToNatNe : header.refCount.toNat ≠ 0 := by
          intro zero
          apply headerCountNe
          apply UInt32.toNat_inj.mp
          simpa using zero
        rw [refCount] at headerToNatNe
        omega
      have nextCountNe : UInt32.ofNat (cell.rc + amount) ≠ 0 := by
        intro zero
        have zeroNat := congrArg UInt32.toNat zero
        rw [UInt32.toNat_ofNat_of_lt' fits] at zeroNat
        simp at zeroNat
        omega
      have nextOwnership :
          if header.persistent then UInt32.ofNat (cell.rc + amount) = 0
          else UInt32.ofNat (cell.rc + amount) ≠ 0 := by
        simpa [headerOrdinary] using nextCountNe
      obtain ⟨result, updatedHeader, memory, write, updatedEq, resultEq,
          headerWrite, finalValid, objectAfter⟩ :=
        objectRelated.writeOwnershipMetadata valid
          (UInt32.ofNat (cell.rc + amount)) header.persistent nextOwnership
      have heap :=
        (MemoryState.PrefixExtension.readLiveHeader_facts state address header
          objectRelated.headerRead).1
      have operation :
          incrementReference state address amount check = .ok result := by
        unfold incrementReference
        rw [heap]
        simp only
        rw [objectRelated.headerRead]
        simp only [Bind.bind, Except.bind, liftMemory]
        rw [if_neg (by simp [notPromoted])]
        rw [if_neg (by simp [headerOrdinary])]
        rw [refCount]
      ...
      subst updatedHeader
      refine ⟨result, operation, finalValid, ?_⟩
      apply LiveCellRel.natural descriptor (by simpa using objectEq) objectAfter
      · simp only
        exact UInt32.toNat_ofNat_of_lt' fits
      · simpa using persistent
      · simpa using live
...
theorem LiveCellRel.decrementReferenceOnce_natural_above_one
  ...
  | @natural value header _ descriptor objectEq objectRelated refCount
      ...
      have nextFits : cell.rc - 1 < UInt32.size := by
        ...
        rw [refCount] at oldFits
        omega
      have nextCountNe : UInt32.ofNat (cell.rc - 1) ≠ 0 := by
        intro zero
        have zeroNat := congrArg UInt32.toNat zero
        rw [UInt32.toNat_ofNat_of_lt' nextFits] at zeroNat
        simp at zeroNat
        omega
      have nextOwnership :
          if header.persistent then UInt32.ofNat (cell.rc - 1) = 0
          else UInt32.ofNat (cell.rc - 1) ≠ 0 := by
        simpa [headerOrdinary] using nextCountNe
      obtain ⟨result, updatedHeader, memory, write, updatedEq, resultEq,
          headerWrite, finalValid, objectAfter⟩ :=
        objectRelated.writeOwnershipMetadata valid
          (UInt32.ofNat (cell.rc - 1)) header.persistent nextOwnership
      have heap :=
        (MemoryState.PrefixExtension.readLiveHeader_facts state address header
          objectRelated.headerRead).1
      have operation :
          decrementReferenceOnce state address check = .ok result := by
        simp only [decrementReferenceOnce, decrementReferenceOnceFuel]
        rw [heap]
        simp only
        rw [objectRelated.headerRead]
...
/-- String bytes and layout metadata frame across a common-header increment,
yielding the same decoded semantic string at the incremented live-cell count. -/
theorem LiveCellRel.incrementReference_string
    {state : MemoryState} {witness : RefinementWitness}
    {address : Word32} {cell : HeapCell}
    (related : LiveCellRel state witness address cell)
    (stringCell : ∃ value : String, cell.object = .string value)
    (valid : state.FrontierInvariant) (ordinary : cell.persistent = false)
    (amount : Nat) (fits : cell.rc + amount < UInt32.size) (check : Bool) :
    ∃ result,
      incrementReference state address amount check = .ok result ∧
  ...
  | @string value header _ descriptor objectEq objectRelated refCount persistent live =>
      ...
      obtain ⟨result, updatedHeader, memory, operation, updatedEq, resultEq,
          headerWrite, finalValid, headerReadAfter⟩ :=
        incrementReference_header valid objectRelated.headerRead headerOwned
      ...
      have objectAfter : StringObjectRel ({ state with memory } : MemoryState)
          address value { header with refCount := UInt32.ofNat (cell.rc + amount) } := {
        headerRead := headerReadAfter
      ...
      apply LiveCellRel.string descriptor (by simpa using objectEq) objectAfter
      · simp only
        exact UInt32.toNat_ofNat_of_lt' fits
      · simpa using persistent
      · simpa using live
...
/-- String cells above one retain their exact UTF-8 decoder while the common
header and semantic cell counts decrease together. -/
theorem LiveCellRel.decrementReferenceOnce_string_above_one
    {state : MemoryState} {witness : RefinementWitness}
    {address : Word32} {cell : HeapCell}
    (related : LiveCellRel state witness address cell)
    (stringCell : ∃ value : String, cell.object = .string value)
  ...
  | @string value header _ descriptor objectEq objectRelated refCount persistent live =>
      ...
      obtain ⟨result, updatedHeader, memory, write, updatedEq, resultEq,
          headerWrite, finalValid, headerReadAfter⟩ :=
        writeReferenceCount_header valid objectRelated.headerRead headerOwned
      ...
      have decoderEq :
        ...
        rw [Header.readStringBytes_of_write_eq_ok state.memory memory address
          ...
          0 _ headerInBounds headerWrite]
      have objectAfter : StringObjectRel ({ state with memory } : MemoryState)
          address value { header with refCount := UInt32.ofNat (cell.rc - 1) } := {
        headerRead := headerReadAfter
        headerKind := by simpa using objectRelated.headerKind
        marker := by simpa using objectRelated.marker
        byteCount := by simpa using objectRelated.byteCount
...
theorem LiveCellRel.decrementReferenceOnce_leaf_one
  ...
  | @natural value header _ descriptor objectEq objectRelated refCount
      ...
      obtain ⟨result, memory, operation, resultEq, headerWrite, finalValid,
          deadRelated⟩ :=
        Fir.Wasm.Concrete.decrementReferenceOnce_leaf_one valid
          objectRelated.headerRead objectRelated.headerOwned
...
theorem LiveCellRel.incrementReference
  ...
  | @constructor info fieldKinds semantic header _ descriptor objectEq objectRelated
      ...
      let localRelated : LiveCellRel state witness address cell :=
        .constructor descriptor objectEq objectRelated headerRead headerKind refCount
          persistent live
      exact localRelated.incrementReference_constructor ⟨semantic, objectEq⟩ valid ordinary
        amount fits check
  | @boxed kind scalar header _ descriptor objectEq objectRelated refCount persistent live =>
      let localRelated : LiveCellRel state witness address cell :=
        .boxed descriptor objectEq objectRelated refCount persistent live
      exact localRelated.incrementReference_boxed ⟨kind, scalar, objectEq⟩ valid ordinary
        amount fits check
  | @natural value header _ descriptor objectEq objectRelated refCount
        persistent live =>
      let localRelated : LiveCellRel state witness address cell :=
  ...
  | closure closureRelated =>
      ...
      exact ⟨result, operation, finalValid, .closure closureAfter⟩
...
theorem LiveCellRel.decrementReferenceOnce_above_one
  ...
  | @constructor info fieldKinds semantic header _ descriptor objectEq objectRelated
      ...
      exact localRelated.decrementReferenceOnce_constructor_above_one
        ⟨semantic, objectEq⟩ valid ordinary oneLt check
  | @boxed kind scalar header _ descriptor objectEq objectRelated refCount persistent live =>
      let localRelated : LiveCellRel state witness address cell :=
        .boxed descriptor objectEq objectRelated refCount persistent live
      exact localRelated.decrementReferenceOnce_boxed_above_one
        ⟨kind, scalar, objectEq⟩ valid ordinary oneLt check
  | @natural value header _ descriptor objectEq objectRelated refCount
        persistent live =>
      let localRelated : LiveCellRel state witness address cell :=
        .natural descriptor objectEq objectRelated refCount persistent live
...
theorem LiveCellRel.live_eq_true
  ...
  | natural _ _ _ _ _ live => exact live
...
theorem LiveCellRel.decValueOnce_above_one_eq
  ...
  exact related.decLocationFuel_above_one_eq ordinary runtime location found oneLt
    runtime.heap.length
...
theorem LiveCellRel.decValueOnce_leaf_one_eq
    ...
    Fir.LeanIR.Impure.decValueOnce runtime (.object (.heap location)) check =
      setCell runtime location { cell with rc := 0, live := false } := by
  unfold Fir.LeanIR.Impure.decValueOnce Fir.LeanIR.Impure.decLocation
  exact related.decLocationFuel_leaf_one_eq leafCell ordinary runtime location found one
    runtime.heap.length
...
theorem LiveCellRel.decrementReferenceOnce_leaf_refines_one
  ...
  obtain ⟨result, _, _, operation, _, _, _, finalValid, deadRelated⟩ :=
    related.decrementReferenceOnce_leaf_one leafCell valid ordinary one check
  exact ⟨result, operation,
    related.decValueOnce_leaf_one_eq leafCell ordinary runtime location found one check,
...
theorem LiveCellRel.decrementReferenceOnce_boxed_refines_above_one
  ...
  obtain ⟨result, operation, finalValid, relatedAfter⟩ :=
    related.decrementReferenceOnce_boxed_above_one boxedCell valid ordinary oneLt check
  exact ⟨result, operation,
    related.decValueOnce_above_one_eq ordinary runtime location found oneLt check,
...
theorem LiveCellRel.decrementReferenceOnce_refines_above_one
    ...
    ∃ result,
      ...
      LiveCellRel result witness address { cell with rc := cell.rc - 1 } := by
  obtain ⟨result, operation, finalValid, relatedAfter⟩ :=
    related.decrementReferenceOnce_above_one valid ordinary oneLt check
  exact ⟨result, operation,
    related.decValueOnce_above_one_eq ordinary runtime location found oneLt check,
    finalValid, relatedAfter⟩
...
theorem LiveCellRel.incValue_eq
  ...
  simp only [getLiveCell, found, related.live_eq_true, ordinary,
...
theorem LiveCellRel.incValue_boxed_eq
    ...
    {address : Word32} {cell : HeapCell}
    (related : LiveCellRel state witness address cell)
    (_boxedCell : ∃ (kind : BoxedScalarKind) (scalar : BoxedScalar),
      cell.object = .boxed kind.semanticType scalar.semanticValue)
    (ordinary : cell.persistent = false)
    (runtime : RuntimeState) (location : Location)
    (found : findCell? runtime.heap location = some cell)
    ...
    Fir.LeanIR.Impure.incValue runtime (.object (.heap location)) amount check =
      setCell runtime location { cell with rc := cell.rc + amount } := by
  have cellLive : cell.live = true := related.live_eq_true
  unfold Fir.LeanIR.Impure.incValue Fir.LeanIR.Impure.incLocation
  simp only [getLiveCell, found, cellLive, ordinary, ↓reduceIte,
