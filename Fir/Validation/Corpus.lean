...
structure RepeatedReleaseReplacement where
  firstPayload : Nat
  secondPayload : Nat
  marker : Nat
...
@[noinline]
def mkRepeatedReleaseOwner (leaf : ReleaseLeaf) : RepeatedReleaseOwner :=
  { proof := True.intro, first := leaf, second := leaf, marker := 31 }
...
@[noinline]
def replaceRepeatedReleaseOwner
    (owner : RepeatedReleaseOwner) (payload : Nat) :
    RepeatedReleaseReplacement :=
  { firstPayload := payload, secondPayload := payload, marker := owner.marker }
...
/--
Store the same leaf in both fields of a unique owner while retaining a third
alias outside. Replacing the owner must release both fields before the outside
leaf becomes reusable.
-/
def repeatedChildReleaseUniqueReuse
    (payload replacementPayload : Nat) :
    RepeatedReleaseReplacement × ReleaseLeaf :=
  let leaf := mkReleaseLeaf payload
...
/--
Retain the repeated-child owner while replacing it. Release must stop at the
shared owner, preserving both original leaf fields and forcing the outside
leaf update to allocate.
-/
def repeatedChildReleaseSharedOwnerStop
    (payload replacementPayload : Nat) :
    RepeatedReleaseReplacement × RepeatedReleaseOwner × ReleaseLeaf :=
  let leaf := mkReleaseLeaf payload
  let owner := mkRepeatedReleaseOwner leaf
  let replacement := replaceRepeatedReleaseOwner owner replacementPayload
...
/-!
Carry two aliases of the same cached String through a self-tail loop while a
third alias remains live outside it. Every iteration swaps the carried aliases,
flips the selector, constructs a fresh owner, and releases the previous owner.
-/
structure TailCachedStringState where
  primary : String
  secondary : String
  choosePrimary : Bool

@[noinline]
def tailTransferCachedString : Nat → TailCachedStringState → String
  | 0, state =>
      if state.choosePrimary then state.primary else state.secondary
  | remaining + 1, state =>
      tailTransferCachedString remaining {
...
@[noinline]
def cachedSharedStringDagAcrossTail
    (steps : Nat) : String × String × CachedSharedStringOwner :=
  let cached := cachedSharedStringOwner
  let aliases := makeTransientCachedStringAliases cached
  let outside := aliases.left
  let selected := tailTransferCachedString steps {
...
/-!
Transfer the same repeated-child state through two mutually recursive
functions. Each call is tail-position, but no call targets the current
declaration, isolating cross-declaration tail ownership from S17's direct
self-tail update and S18's retained non-tail unwind.
-/
mutual
  @[noinline]
  def mutualTailCachedStringA :
      Nat → TailCachedStringState → String
    | 0, state =>
        if state.choosePrimary then state.primary else state.secondary
    | remaining + 1, state =>
        mutualTailCachedStringB remaining {
          primary := state.secondary
          secondary := state.primary
          choosePrimary := !state.choosePrimary
        }
  ...
  @[noinline]
  def mutualTailCachedStringB :
      Nat → TailCachedStringState → String
    | 0, state =>
        if state.choosePrimary then state.primary else state.secondary
    | remaining + 1, state =>
        mutualTailCachedStringA remaining {
          primary := state.secondary
          secondary := state.primary
          choosePrimary := !state.choosePrimary
        }
...
@[noinline]
def cachedSharedStringDagAcrossMutualTail
    (steps : Nat) : String × String × CachedSharedStringOwner :=
  let cached := cachedSharedStringOwner
  let aliases := makeTransientCachedStringAliases cached
  let outside := aliases.left
  let selected := mutualTailCachedStringA steps {
    primary := aliases.right
    secondary := aliases.ignored
    choosePrimary := true
  }
...
private def repeatedReleaseOwnerDatum
    (value : Source.RepeatedReleaseOwner) : ValidationDatum :=
  .ctor "RepeatedReleaseOwner.mk" 0
    #[releaseLeafDatum value.first, releaseLeafDatum value.second, .nat value.marker]

private def repeatedReleaseReplacementDatum
    (value : Source.RepeatedReleaseReplacement) : ValidationDatum :=
  .ctor "RepeatedReleaseReplacement.mk" 0
    #[.nat value.firstPayload, .nat value.secondPayload, .nat value.marker]

private def repeatedReleaseUniqueResultDatum
    (value : Source.RepeatedReleaseReplacement × Source.ReleaseLeaf) :
    ValidationDatum :=
  .ctor "Prod.mk" 0
    #[repeatedReleaseReplacementDatum value.1, releaseLeafDatum value.2]

private def repeatedReleaseSharedResultDatum
    (value : Source.RepeatedReleaseReplacement × Source.RepeatedReleaseOwner ×
      Source.ReleaseLeaf) : ValidationDatum :=
  .ctor "Prod.mk" 0 #[repeatedReleaseReplacementDatum value.1,
    .ctor "Prod.mk" 0
...
private def repeatedReleaseOwnerSchema : ValidationSchema :=
  .ctor "RepeatedReleaseOwner.mk" 0
    #[releaseLeafSchema, releaseLeafSchema, .nat]

private def repeatedReleaseReplacementSchema : ValidationSchema :=
  .ctor "RepeatedReleaseReplacement.mk" 0 #[.nat, .nat, .nat]

private def repeatedReleaseUniqueResultSchema : ValidationSchema :=
  .ctor "Prod.mk" 0 #[repeatedReleaseReplacementSchema, releaseLeafSchema]

private def repeatedReleaseSharedResultSchema : ValidationSchema :=
...
private def repeatedChildReleaseUniqueFormTrace : Array String :=
  #["fap", "fap", "lit", "return", "inc", "ctor", "return", "inc", "fap",
    "lit", "inc", "ctor", "return", "fap", "oproj", "join", "isShared",
    "cases", "oproj", "dec", "oproj", "dec", "jump", "inc", "join", "cases",
    "oset", "oset", "jump", "return", "fap", "oproj", "oproj", "join",
    "isShared", "cases", "jump", "lit", "fap", "extern", "dec", "join",
    "cases", "oset", "jump", "return", "ctor", "return"]

private def repeatedChildReleaseSharedFormTrace : Array String :=
  #["fap", "fap", "lit", "return", "inc", "ctor", "return", "inc", "fap",
    "lit", "inc", "ctor", "return", "inc", "fap", "oproj", "join", "isShared",
    "cases", "inc", "dec", "jump", "inc", "join", "cases", "ctor", "jump",
...
private def cachedSharedStringDagTailZeroFormTrace : Array String :=
  #["fap", "fap", "fap", "lit", "return", "fap", "lit", "return", "inc", "inc",
    "ctor", "return", "inc", "return", "fap", "fap", "inc", "fap", "oproj",
    "oproj", "join", "isShared", "cases", "inc", "inc", "dec", "jump", "inc",
    "join", "cases", "ctor", "jump", "return", "return", "oproj", "oproj",
    "oproj", "lit", "inc", "inc", "ctor", "sset", "fap", "lit", "fap",
    "extern", "cases", "dec", "sproj", "cases", "oproj", "inc", "dec", "return",
    "fap", "lit", "return", "fap", "extern", "inc", "ctor", "inc", "ctor",
    "return"]

private def cachedSharedStringDagTailThreeFormTrace : Array String :=
  #["fap", "fap", "fap", "lit", "return", "fap", "lit", "return", "inc", "inc",
    "ctor", "return", "inc", "return", "fap", "fap", "inc", "fap", "oproj",
    "oproj", "join", "isShared", "cases", "inc", "inc", "dec", "jump", "inc",
    "join", "cases", "ctor", "jump", "return", "return", "oproj", "oproj",
    "oproj", "lit", "inc", "inc", "ctor", "sset", "fap", "lit", "fap",
    "extern", "cases", "oproj", "oproj", "sproj", "join", "isShared", "cases",
    "jump", "lit", "fap", "extern", "dec", "join", "cases", "jump", "join",
    "cases", "oset", "oset", "jump", "sset", "fap", "lit", "fap", "extern",
    "cases", "oproj", "oproj", "sproj", "join", "isShared", "cases", "jump",
    "lit", "fap", "extern", "dec", "join", "cases", "lit", "jump", "join",
    "cases", "oset", "oset", "jump", "sset", "fap", "lit", "fap", "extern",
    "cases", "oproj", "oproj", "sproj", "join", "isShared", "cases", "jump",
    "lit", "fap", "extern", "dec", "join", "cases", "jump", "join", "cases",
...
private def cachedSharedStringDagMutualTailThreeFormTrace : Array String :=
  #["fap", "fap", "fap", "lit", "return", "fap", "lit", "return", "inc", "inc",
    "ctor", "return", "inc", "return", "fap", "fap", "inc", "fap", "oproj", "oproj",
    "join", "isShared", "cases", "inc", "inc", "dec", "jump", "inc", "join", "cases",
    "ctor", "jump", "return", "return", "oproj", "oproj", "oproj", "lit", "inc", "inc",
    "ctor", "sset", "fap", "lit", "fap", "extern", "cases", "oproj", "oproj", "sproj",
    "join", "isShared", "cases", "jump", "lit", "fap", "extern", "join", "cases", "jump",
    "join", "cases", "oset", "oset", "jump", "sset", "fap", "lit", "fap", "extern",
    "cases", "oproj", "oproj", "sproj", "join", "isShared", "cases", "jump", "lit", "fap",
    "extern", "join", "cases", "lit", "jump", "join", "cases", "oset", "oset", "jump",
    "sset", "fap", "lit", "fap", "extern", "cases", "oproj", "oproj", "sproj", "join",
    "isShared", "cases", "jump", "lit", "fap", "extern", "join", "cases", "jump", "join",
    "cases", "oset", "oset", "jump", "sset", "fap", "lit", "fap", "extern", "cases",
    "sproj", "cases", "oproj", "inc", "dec", "return", "dec", "return", "dec", "return",
...
private def preConversionCases : Array Case := #[
  ...
  { id := "cached-shared-string-dag-tail-zero"
    entry := ``Source.cachedSharedStringDagAcrossTail
    dependencies :=
      #[``Source.cachedSharedStringOwner,
        ``Source.makeTransientCachedStringAliases,
        ``Source.tailTransferCachedString]
    args := #[.nat 0]
    argSchemas := #[.nat]
    resultSchema := cachedSharedStringDagAcrossCatchSchema
    native := fun _ => cachedSharedStringDagAcrossCatchDatum
      (Source.cachedSharedStringDagAcrossTail 0)
    tags := #["stress", "ownership", "persistent", "cache", "string",
      ...
      "zero-use", "release-fidelity", "copy-on-write", "external", "constructor",
      "object", "heap", "alias", "shared", "repeated-alias", "alias-preservation",
      "shared-dag", "cache-miss", "cache-hit", "repeated-call", "initialization",
      "mixed-layout", "scalar-field", "path-exclusion", "tail-state"]
    requiredLcnfForms :=
      #["cases", "ctor", "dec", "extern", "fap", "inc", "isShared", "join",
    ...
    requiredExecutedLcnfFormCounts :=
      #[{ form := "cases", minimum := 4, maximum := some 4 },
        ...
        { form := "oset", minimum := 0, maximum := some 0 },
        { form := "return", minimum := 9, maximum := some 9 },
        { form := "sproj", minimum := 1, maximum := some 1 },
        { form := "sset", minimum := 1, maximum := some 1 }]
    requiredExecutedLcnfFormTrace := some cachedSharedStringDagTailZeroFormTrace
    requiredAdministrativeStepKinds := cachedHeapOwnerAdministrativeKinds
    requiredExternals :=
      #[``Nat.decEq, ``Nat.sub, ``String.Internal.append]
    requiredExecutedExternals := #[``Nat.decEq, ``String.Internal.append]
    requiredExecutedExternalCounts :=
      #[{ external := ``Nat.decEq, minimum := 1, maximum := some 1 },
        { external := ``Nat.sub, minimum := 0, maximum := some 0 },
        { external := ``String.Internal.append, minimum := 1, maximum := some 1 }]
    requiredExecutedExternalTrace :=
      some #[``Nat.decEq, ``String.Internal.append]
    provenance := firProvenance
      "Return a cached repeated-child survivor without entering the self-tail loop" },
  { id := "cached-shared-string-dag-tail-three"
    entry := ``Source.cachedSharedStringDagAcrossTail
    dependencies :=
      #[``Source.cachedSharedStringOwner,
        ``Source.makeTransientCachedStringAliases,
    ...
    tags := #["stress", "ownership", "persistent", "cache", "string",
      ...
      "repeated-alias", "alias-preservation", "shared-dag", "cache-miss",
      "cache-hit", "repeated-call", "initialization", "mixed-layout",
      "scalar-field", "tail-state", "loop-carried-owner", "recursive-release",
    ...
    requiredExecutedLcnfFormCounts :=
      #[{ form := "cases", minimum := 16, maximum := some 16 },
        { form := "ctor", minimum := 5, maximum := some 5 },
        { form := "dec", minimum := 6, maximum := some 6 },
        { form := "extern", minimum := 8, maximum := some 8 },
        { form := "fap", minimum := 20, maximum := some 20 },
        { form := "inc", minimum := 12, maximum := some 12 },
        { form := "isShared", minimum := 4, maximum := some 4 },
        { form := "join", minimum := 11, maximum := some 11 },
        { form := "jump", minimum := 11, maximum := some 11 },
        { form := "lit", minimum := 12, maximum := some 12 },
        { form := "oproj", minimum := 12, maximum := some 12 },
        { form := "oset", minimum := 6, maximum := some 6 },
        { form := "return", minimum := 12, maximum := some 12 },
        { form := "sproj", minimum := 4, maximum := some 4 },
        { form := "sset", minimum := 4, maximum := some 4 }]
    requiredExecutedLcnfFormTrace := some cachedSharedStringDagTailThreeFormTrace
    requiredAdministrativeStepKinds := cachedHeapOwnerAdministrativeKinds
    requiredExternals :=
      #[``Nat.decEq, ``Nat.sub, ``String.Internal.append]
    requiredExecutedExternals :=
  ...
  { id := "cached-shared-string-dag-mutual-tail-three"
    entry := ``Source.cachedSharedStringDagAcrossMutualTail
    dependencies :=
      #[``Source.cachedSharedStringOwner,
        ``Source.makeTransientCachedStringAliases,
        ``Source.mutualTailCachedStringA,
        ``Source.mutualTailCachedStringB]
    args := #[.nat 3]
    argSchemas := #[.nat]
    resultSchema := cachedSharedStringDagAcrossCatchSchema
    native := fun _ => cachedSharedStringDagAcrossCatchDatum
      (Source.cachedSharedStringDagAcrossMutualTail 3)
    tags := #["stress", "ownership", "persistent", "cache", "string",
      "cached-recursion-transfer", "cached-mutual-tail",
      "cached-mutual-tail-transfer",
      "repeated-child-alias", "outside-alias", "tail-control",
      "tail-ownership", "mutual-recursion", "cross-declaration-call",
      "loop-carried-owner",
      "release-fidelity", "multiplicity", "copy-on-write", "external",
      "constructor", "object", "heap", "alias", "shared",
      "repeated-alias", "alias-preservation", "shared-dag", "cache-miss",
      "cache-hit", "repeated-call", "initialization", "mixed-layout",
      "scalar-field", "tail-state", "object-update", "recursive-release",
      "release"]
    requiredLcnfForms :=
      #["cases", "ctor", "dec", "extern", "fap", "inc", "isShared", "join",
        "jump", "lit", "oproj", "oset", "return", "sproj", "sset"]
    ...
    requiredExecutedLcnfFormCounts :=
      #[{ form := "cases", minimum := 16, maximum := some 16 },
        ...
        { form := "sset", minimum := 4, maximum := some 4 }]
    requiredExecutedLcnfFormTrace :=
      some cachedSharedStringDagMutualTailThreeFormTrace
    requiredAdministrativeStepKinds := cachedHeapOwnerAdministrativeKinds
    requiredExternals :=
      #[``Nat.decEq, ``Nat.sub, ``String.Internal.append]
    requiredExecutedExternals :=
...
private def postConversionCases : Array Case := #[
  ...
  { id := "repeated-child-release-unique-reuse"
    entry := ``Source.repeatedChildReleaseUniqueReuse
    dependencies :=
      #[``Source.mkReleaseLeaf, ``Source.mkRepeatedReleaseOwner,
        ``Source.replaceRepeatedReleaseOwner, ``Source.ReleaseLeaf.bump]
    args := #[.nat Source.largeNat, .nat 37]
    argSchemas := #[.nat, .nat]
    resultSchema := repeatedReleaseUniqueResultSchema
    native := fun _ => repeatedReleaseUniqueResultDatum
      (Source.repeatedChildReleaseUniqueReuse Source.largeNat 37)
    tags :=
      #["stress", "ownership", "reference-count", "recursive-release",
        "release-fidelity", "repeated-alias", "repeated-child-alias", "multiplicity",
        "outside-alias", "unique", "unique-owner-release", "release", "reuse",
        "constructor", "object", "heap", "string", "large-nat", "erased",
        "allocation"]
    requiredLcnfForms :=
      #["fap", "inc", "ctor", "return", "lit", "oproj", "join", "cases",
        "oset", "jump", "isShared", "dec", "extern"]
    ...
    requiredExecutedLcnfFormCounts :=
      #[{ form := "fap", minimum := 6, maximum := some 6 },
        ...
        { form := "oset", minimum := 3, maximum := some 3 },
        { form := "extern", minimum := 1, maximum := some 1 }]
    requiredExecutedLcnfFormTrace := some repeatedChildReleaseUniqueFormTrace
    requiredAdministrativeStepKinds :=
      #["admin:invoke-name", "admin:yield-cache", "admin:yield-bind",
        "admin:yield-done"]
    requiredExternals := #[``Nat.add]
    ...
    requiredExecutedExternalTrace := some #[``Nat.add]
    provenance := firProvenance
      "Release both fields of a unique repeated-child owner before reusing its outside leaf" },
  { id := "repeated-child-release-shared-owner-stop"
    entry := ``Source.repeatedChildReleaseSharedOwnerStop
    dependencies :=
      #[``Source.mkReleaseLeaf, ``Source.mkRepeatedReleaseOwner,
        ``Source.replaceRepeatedReleaseOwner, ``Source.ReleaseLeaf.bump]
    args := #[.nat Source.largeNat, .nat 37]
    argSchemas := #[.nat, .nat]
    resultSchema := repeatedReleaseSharedResultSchema
    native := fun _ => repeatedReleaseSharedResultDatum
      (Source.repeatedChildReleaseSharedOwnerStop Source.largeNat 37)
    tags :=
      #["stress", "ownership", "reference-count", "recursive-release",
        "release-fidelity", "repeated-alias", "repeated-child-alias", "multiplicity",
        "outside-alias", "shared", "shared-owner-stop", "stop-recursion", "retain",
        "release", "copy-on-write", "constructor", "object", "heap", "string",
        "large-nat", "erased", "allocation", "path-exclusion"]
    requiredLcnfForms :=
      #["fap", "inc", "ctor", "return", "lit", "oproj", "join", "cases",
        "oset", "jump", "isShared", "dec", "extern"]
    requiredExecutedLcnfForms :=
      #["fap", "lit", "return", "inc", "ctor", "oproj", "join", "isShared",
        "cases", "dec", "jump", "extern"]
    requiredExecutedLcnfFormCounts :=
      #[{ form := "fap", minimum := 6, maximum := some 6 },
        { form := "lit", minimum := 3, maximum := some 3 },
        { form := "return", minimum := 6, maximum := some 6 },
        { form := "inc", minimum := 8, maximum := some 8 },
        { form := "ctor", minimum := 6, maximum := some 6 },
        { form := "oproj", minimum := 3, maximum := some 3 },
        { form := "join", minimum := 4, maximum := some 4 },
        { form := "isShared", minimum := 2, maximum := some 2 },
        ...
        { form := "oset", minimum := 0, maximum := some 0 },
        { form := "extern", minimum := 1, maximum := some 1 }]
    requiredExecutedLcnfFormTrace := some repeatedChildReleaseSharedFormTrace
    requiredAdministrativeStepKinds :=
      #["admin:invoke-name", "admin:yield-cache", "admin:yield-bind",
        "admin:yield-done"]
    requiredExternals := #[``Nat.add]
  ...
  { id := "effect-record-aliased-byte-array-arguments"
    entry := ``Source.recordAliasedByteArrays
    args := #[.bytes #[0, 127, 128, 255], .bytes #[0, 127, 128, 255]]
    argSchemas := #[.bytes, .bytes]
    argumentAliases := #[{ source := 0, target := 1 }]
    resultSchema := byteArrayPairSchema
    native := fun _ =>
      let shared : ByteArray := ⟨#[0, 127, 128, 255]⟩
      byteArrayPairDatum (Source.recordAliasedByteArrays shared shared)
    nativeBefore := NativeEffects.reset
    nativeEffects := fun _ => NativeEffects.take
    tags :=
      #["stress", "effect", "external", "bytes", "heap", "mutation", "ownership",
        "shared", "copy-on-write", "alias", "argument-alias"]
