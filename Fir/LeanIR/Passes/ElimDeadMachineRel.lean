...
/-- Foreign-call requests preserve declaration metadata while their runtime
arguments follow the active address renaming. -/
structure ExternalRequestRel (rho : AddressRenaming)
    (source target : ExternalRequest) : Prop where
  name_eq : source.name = target.name
  paramTypes_eq : source.paramTypes = target.paramTypes
  resultType_eq : source.resultType = target.resultType
  args : ArrayRel (ValueRel rho) source.args target.args
...
theorem relatedFaultRel_terminal
    (runtime : ShadowRuntimeRel rho source.runtime target.runtime
      sourceRoots targetRoots)
    (faults : RuntimeFaultRel rho sourceFault targetFault)
    (sourceDone : coreStep source =
      .done (observe source (.fault sourceFault)))
    (targetDone : coreStep target =
      .done (observe target (.fault targetFault)))
    (done : coreStep source = .done sourceObservation) :
    ∃ targetObservation,
      EvaluatesState externals target targetObservation ∧
      ObservationRel sourceObservation targetObservation := by
  have observations : ObservationRel
      (observe source (.fault sourceFault))
      (observe target (.fault targetFault)) := by
    apply runtime.observationRel
        (leftOutcome := .fault sourceFault)
        (rightOutcome := .fault targetFault)
    · exact faults
    · intro value member
  ...
  have observationEq : sourceObservation =
      observe source (.fault sourceFault) := by
    rw [sourceDone] at done
    exact (CoreResult.done.inj done).symm
  refine ⟨observe target (.fault targetFault), ?_, ?_⟩
  · exact ⟨0, target, .refl target, targetDone⟩
  · simpa [observationEq] using observations

/-- Equal faults are the common special case of related terminal faults. -/
...
environment is deliberately not part of the published roots: the waiting
control can inspect only the original arguments, and the prepared stack can
inspect only over-application arguments and the old frames. -/
theorem waitExternalDecl_reachableRelated
...
/-- Declaration-ready named dispatch exposes any common `invokeDecl` result,
including an external request. -/
theorem coreStep_invokeName_of_declReady_result
    (sourceState targetState : MachineState)
    (sourceReady : InvokeNameDeclReady sourceState.runtime name
      sourceArguments)
    (targetReady : InvokeNameDeclReady targetState.runtime name
    ...
    (sourceStep : invokeDecl
      { sourceState with control := .invokeName name sourceArguments }
      name sourceArguments = sourceResult)
    (targetStep : invokeDecl
      { targetState with control := .invokeName name targetArguments }
      name targetArguments = targetResult) :
    coreStep { sourceState with
      control := .invokeName name sourceArguments } = sourceResult ∧
    coreStep { targetState with
      control := .invokeName name targetArguments } = targetResult := by
  constructor
  · cases sourceReady with
    | nonempty nonempty => simpa [coreStep, nonempty] using sourceStep
    | cacheMiss empty miss =>
...
/-- A source lookup of an external declaration determines a target lookup
with the same ABI.  Fully applied related arguments therefore produce
related foreign requests and related waiting machines.  This theorem stops
at the request boundary and makes no assumption about foreign responses. -/
theorem invokeNameDecl_foundExtern_reachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
      ...
      | extern metadata =>
          ...
          | ok targetEnv =>
              ...
              | ok envRelated =>
                  ...
                  have waiting := waitExternalDecl_reachableRelated
...
those requests. -/
theorem coreStep_invokeName_foundExtern_of_declReady_reachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- A published mapped heap reference whose source cell is dead has a dead
target cell at the mapped location.  Closure invocation therefore terminates
with address-renamed `deadObject` faults on both sides. -/
theorem coreStep_invokeValue_dead_terminal
    (sourceState targetState : MachineState)
    (mapping : rho.forward sourceLocation = some targetLocation)
    (sourceCellFound : findCell? sourceState.runtime.heap sourceLocation =
      some sourceCell)
    (sourceDead : sourceCell.live = false)
    (runtime : ShadowRuntimeRel rho sourceState.runtime targetState.runtime
      ((.object (.heap sourceLocation) :: sourceArguments.toList) ++
        sourceFrameRoots)
      ((.object (.heap targetLocation) :: targetArguments.toList) ++
  ...
  rcases runtime.readMappedCell mapping sourceCellFound with
    ⟨targetCell, targetCellFound, cells⟩
  have targetDead : targetCell.live = false := by
    rw [← cells.2.2.1]
    exact sourceDead
  let sourceInvoke := {
    sourceState with
  ...
  have terminal := relatedFaultRel_terminal
    (externals := externals) invokeRuntime (.deadObject mapping)
    sourceFault targetFault (by simpa [sourceInvoke] using done)
  simpa [targetInvoke] using terminal
...
/-- Hereditary source-only rule for a deleted value-producing `let`.  The
dead source binding is invisible at the selected continuation's liveness set,
while exact provenance is preserved in that continuation, all live joins,
saved frames, and declaration bodies. -/
theorem coreStep_deletedLet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary form of deleted-constructor stuttering.  Constructor
allocation may add one source-only garbage cell, while every compiler-derived
code relation in the live machine remains exact. -/
theorem coreStep_deletedCtor_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary source-only rule for an unreachable object-field write. -/
theorem coreStep_deletedObjectSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary paired rule for a retained object-field write. -/
theorem coreStep_retainedObjectSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary paired rule for a retained absolute-slot write. -/
theorem coreStep_retainedUSizeSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary paired rule for a retained packed-scalar write. -/
theorem coreStep_retainedScalarSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary source-only rule for an unreachable absolute-slot write. -/
theorem coreStep_deletedUSizeSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary source-only rule for an unreachable packed-scalar write. -/
theorem coreStep_deletedScalarSet_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Hereditary certified rule for deleting an unreachable object-field
write. -/
theorem coreStep_deletedObjectSet_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  rcases runtime.setObjectFieldLeftUnreachable found live objectEq bounded
    ...
    ⟨nextRuntime, effect, next⟩
  exact ⟨nextRuntime,
    coreStep_deletedObjectSet_binderReadyReachableRelated
...
/-- Hereditary certified rule for deleting an unreachable absolute-slot
write. -/
theorem coreStep_deletedUSizeSet_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  rcases runtime.setUSizeSlotLeftUnreachable found live objectEq lower bounded
    ...
    ⟨nextRuntime, effect, next⟩
  exact ⟨nextRuntime,
    coreStep_deletedUSizeSet_binderReadyReachableRelated
      sourceState targetState programs frames continuation joins env
      objectRead fieldRead effect next⟩
...
/-- Hereditary certified rule for deleting an unreachable packed-scalar
write. -/
theorem coreStep_deletedScalarSet_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- No full application is locally deletable: after evaluating its arguments
it always produces an invocation action, never the value action required by
the generic runtime-neutral deletion certificate. -/
theorem DeletedLetReadyAt.not_fap
    (ready : DeletedLetReadyAt state roots {
      fvarId, binderName, type, value := .fap name arguments }) : False := by
  cases ready with
  | runtimeNeutral declaration value evaluated =>
      simp only [evalLetValue] at evaluated
      generalize argumentsEq :
        evalArgs state.env arguments = result at evaluated
      cases result with
      | error fault =>
          simp [argumentsEq, Bind.bind, Except.bind] at evaluated
      | ok values =>
...
/-- Hereditary source-only matcher used by deleted exact branches. -/
theorem match_sourceOnlyCoreStep_binderReady
    (transition : coreStep source = .next expectedSourceAfter)
    (related : BinderReadyReachableMachineRelated fuel rho
...
/-- Hereditary counterpart of `match_internalCoreSteps`.  Determinism is
independent of the compiler provenance retained by the post-step relation. -/
theorem match_internalCoreSteps_binderReady
    (sourceTransition : coreStep source = .next expectedSourceAfter)
    (targetTransition : coreStep target = .next targetAfter)
    (related : BinderReadyReachableMachineRelated fuel rho
      expectedSourceAfter targetAfter)
...
/-- Installing a retained value also preserves the exact hereditary graph of
the selected continuation and every code-bearing frame or live join body. -/
theorem retainedLetValue_binderReadyReachableRelated
    (sourceState targetState : MachineState)
    (sourceRuntime targetRuntime : RuntimeState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
...
/-- Exact-provenance form of the retained erased-let matcher.  The operational
transition is unchanged; only the post-state relation retains the selected
continuation's hereditary compiler certificate. -/
theorem match_retainedErasedLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  have afterRelated :
    ...
    exact retainedLetValue_binderReadyReachableRelated
      ...
      programs frames continuation joins env .erased runtime.prependErased
  exact ⟨targetExpected,
...
theorem match_deletedErasedLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
the audited compiler view to a strong related machine destination. -/
theorem ExactShadowCodeBinderReady.match_retainedErasedLetStep
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_retainedErasedLetStep_binderReady
    sourceState targetState programs frames
    (ready.letRetained_continuationGraph fuelBound usedBound)
    joins env runtime step

/-- Deleted counterpart of `match_retainedErasedLetStep`: exact branch
provenance supplies both ambient binder absence and the hereditary
continuation graph required after the source-only step. -/
theorem ExactShadowCodeBinderReady.match_deletedErasedLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId : FVarId} {binderName : Name} {type : Expr}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_deletedErasedLetStep_binderReady
    sourceState targetState programs frames
...
/-- A retained literal evaluates on both machines.  Immediate literals keep
the current address renaming, while heap-backed literals extend it with the
fresh allocation pair before binding the related results. -/
theorem match_retainedLiteralLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
  ...
  have targetTransition :
    ...
    simp [targetCurrent, targetExpected, targetRuntime, targetValue,
      coreStep, evalLetValue, Pure.pure, Except.pure]
  have afterRelated :
      ReachableMachineRelated fuel larger sourceExpected targetExpected := by
    exact retainedLetValue_reachableRelated sourceState targetState
      sourceRuntime targetRuntime programs (frames.monoRenaming extension)
      continuation joins (envRelOn_monoRenaming extension env) values
...
allocate only source-side unreachable garbage and stutter on the target. -/
theorem match_literalLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- Hereditary retained-constructor matcher.  Related live arguments either
produce the same immediate nullary constructor or paired heap allocations;
the latter extend the hidden address renaming and preserve exact compiler
provenance throughout the resulting machines. -/
theorem match_retainedCtorLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
theorem coreStep_deletedCtor_of_letReady_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  | runtimeNeutral declaration value evaluated =>
      exact ⟨sourceState.runtime, value,
        coreStep_deletedLet_binderReadyReachableRelated
          sourceState targetState programs frames continuation joins env
          absent evaluated runtime⟩
  | constructor fvarId binderName type info arguments ready =>
      exact coreStep_deletedCtor_of_ready_binderReady
        sourceState targetState programs frames continuation joins env absent
        runtime ready

/-- Hereditary deleted-constructor matcher.  The source performs its
certified value-producing step, the target stutters at the selected
continuation, and exact compiler provenance remains available afterwards. -/
theorem match_deletedCtorLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  rcases coreStep_deletedCtor_of_letReady_binderReady
      sourceState targetState programs frames continuation joins env absent
      runtime ready with
    ⟨nextRuntime, value, transition, afterRelated⟩
  rcases match_sourceOnlyCoreStep_binderReady
...
/-- Exact retained-constructor provenance supplies the hereditary
continuation graph, while its compiler liveness bound covers every
constructor argument used by the paired allocation. -/
theorem ExactShadowCodeBinderReady.match_retainedCtorLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId : FVarId} {binderName : Name} {type : Expr}
  ...
  exact match_retainedCtorLetStep_binderReady
    sourceState targetState programs frames
    (ready.letRetained_continuationGraph fuelBound usedBound)
...
theorem ExactShadowCodeBinderReady.match_deletedCtorLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId : FVarId} {binderName : Name} {type : Expr}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_deletedCtorLetStep_binderReady
    sourceState targetState programs frames
    (ready.letDeleted_continuationGraph fuelBound usedBound)
...
/-- A retained sharedness query reads related covered objects, obtains
related scalar results from equal live-cell metadata, and binds those results
without changing either runtime. -/
theorem match_retainedIsSharedLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
      ...
      | some values =>
          ...
          | ok sourceResult =>
              have sourceObjectRoot :
                ...
                exact List.mem_append_left _
                  (lookup_mem_envRootsOn objectMember sourceObjectLookup)
              rcases runtime.isSharedBoth_of_related sourceObjectRoot values
                  sourceSharedEq with
                ⟨targetResult, targetSharedEq, results, publishedRuntime⟩
              let sourceExpected := {
                sourceState with
                env := bind sourceState.env fvarId sourceResult
                control := .code sourceContinuation }
              let targetExpected := {
                targetState with
                env := bind targetState.env fvarId targetResult
                control := .code targetContinuation }
              have sourceTransition :
                  coreStep sourceCurrent = .next sourceExpected := by
                simp [sourceCurrent, sourceExpected, coreStep, evalLetValue,
              ...
              have targetTransition :
                  coreStep targetCurrent = .next targetExpected := by
                simp [targetCurrent, targetExpected, coreStep, evalLetValue,
                  targetObjectRead, targetSharedEq, Bind.bind, Except.bind,
                  Pure.pure, Except.pure]
              have afterRelated :
                  ReachableMachineRelated fuel rho
                    sourceExpected targetExpected := by
                exact retainedLetValue_reachableRelated
                  ...
                  programs frames continuation joins env results
                  publishedRuntime
              exact ⟨targetExpected,
                match_internalCoreSteps sourceTransition targetTransition
                  afterRelated (by simpa [sourceCurrent] using step)⟩

/-- Complete graph-level matcher for `isShared` lets.  Retained queries
transport successful metadata reads; deleted queries use their generic
runtime-neutral readiness certificate. -/
theorem match_isSharedLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- A runtime-neutral deleted let preserves hereditary compiler provenance:
the source advances through the certified value step while the target
stutters at the exact continuation selected by the compiler graph. -/
theorem match_deletedRuntimeNeutralLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
Heap-valued fields are promoted from child reachability to continuation
roots by the runtime theorem. -/
theorem match_retainedObjectProjectionLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
  ...
  | ok sourceObject =>
      ...
      | some values =>
          ...
          generalize sourceFieldEq :
            getObjectField sourceState.runtime sourceObject index =
              sourceFieldResult
          ...
          | ok sourceField =>
              have sourceObjectRoot :
                ...
                exact List.mem_append_left _
                  (lookup_mem_envRootsOn objectMember sourceObjectLookup)
              rcases runtime.getObjectFieldBoth_of_related sourceObjectRoot
...
/-- Hereditary object-projection matcher.  A successful related field lookup
publishes the selected child as a continuation root while retaining exact
compiler provenance in every code-bearing component of the machines. -/
theorem match_retainedObjectProjectionLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Exact retained object-projection provenance supplies the hereditary
continuation and promotes the compiler-collected object local to the ambient
live set. -/
theorem ExactShadowCodeBinderReady.match_retainedObjectProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
  ...
  exact match_retainedObjectProjectionLetStep_binderReady
    sourceState targetState programs frames
    (ready.letRetained_continuationGraph fuelBound usedBound)
...
runtime-neutral successful field-read case. -/
theorem ExactShadowCodeBinderReady.match_deletedObjectProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel index : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId object : FVarId} {binderName : Name} {type : Expr}
...
/-- Complete graph-level matcher for object-field projection lets. -/
theorem match_objectProjectionLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- Hereditary absolute-slot `USize` projection matcher.  Related live
objects yield the same unboxed word, and the exact compiler graph is retained
through the selected continuation. -/
theorem match_retainedUSizeProjectionLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Exact retained `USize` projection provenance supplies the hereditary
continuation and the ambient liveness proof for the projected object. -/
theorem ExactShadowCodeBinderReady.match_retainedUSizeProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel slot : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId object : FVarId} {binderName : Name} {type : Expr}
    {continuation :
      ExactShadowCodeRun nextFuel initial continuationUsed
    ...
    {keep :
      fvarId ∈ continuationUsed ∨
        safeToElim (LCNF.LetValue.uproj slot object :
          LCNF.LetValue .impure) = false}
    (ready :
      ExactShadowCodeBinderReady ambient
        (ExactShadowCodeView.letRetained
    ...
    (usedBound : UsedSubset
      (collectLetValue continuationUsed
        (LCNF.LetValue.uproj slot object : LCNF.LetValue .impure)) ambient)
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  have objectMember : ambient.contains object = true :=
    usedBound object (by simp [collectLetValue])
  exact match_retainedUSizeProjectionLetStep_binderReady
    sourceState targetState programs frames
    (ready.letRetained_continuationGraph fuelBound usedBound)
    joins env objectMember runtime step

/-- Exact deleted `USize` projection provenance supplies binder absence and
the hereditary continuation; its successful read is runtime-neutral. -/
theorem ExactShadowCodeBinderReady.match_deletedUSizeProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel slot : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId object : FVarId} {binderName : Name} {type : Expr}
...
/-- A retained packed-scalar projection reads related covered constructors,
transports the successful lookup, and binds the identical immediate scalar
on both machines. -/
theorem match_retainedScalarProjectionLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
    ...
    (step : Step externals
      { sourceState with
        control := .code (.let {
          fvarId, binderName, type, value := .sproj width offset object
        } sourceContinuation) }
      sourceAfter) :
    ∃ targetAfter,
      NonLockstep.Reaches externals
  ...
  | ok sourceObject =>
      ...
      | some values =>
          ...
          have targetObjectRead :
            ...
            lookupValue_eq_ok_iff.mpr targetObjectLookup
          generalize sourceFieldEq :
            getScalarField sourceState.runtime sourceObject width offset =
              sourceFieldResult
          cases sourceFieldResult with
          | error fault =>
              have done : coreStep sourceCurrent =
          ...
          | ok sourceField =>
              have sourceObjectRoot :
                ...
                exact List.mem_append_left _
                  (lookup_mem_envRootsOn objectMember sourceObjectLookup)
              rcases runtime.getScalarFieldBoth_of_related sourceObjectRoot
                  values sourceFieldEq with
                ⟨targetField, targetFieldEq, fields, publishedRuntime⟩
              let sourceExpected := {
                sourceState with
...
/-- Hereditary packed-scalar projection matcher.  Related live objects yield
the same immediate scalar, and exact compiler provenance is retained through
the selected continuation. -/
theorem match_retainedScalarProjectionLetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
/-- Exact retained packed-scalar projection provenance supplies the
hereditary continuation and the ambient liveness proof for the projected
object. -/
theorem ExactShadowCodeBinderReady.match_retainedScalarProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
    ...
    {keep :
      fvarId ∈ continuationUsed ∨
        safeToElim (LCNF.LetValue.sproj width offset object :
          LCNF.LetValue .impure) = false}
    (ready :
      ExactShadowCodeBinderReady ambient
        (ExactShadowCodeView.letRetained
          (declaration := {
            fvarId, binderName, type,
            value := .sproj width offset object
          }) continuation keep))
    (fuelBound : nextFuel + 1 ≤ fuel)
    (usedBound : UsedSubset
      (collectLetValue continuationUsed
        (LCNF.LetValue.sproj width offset object :
  ...
  have objectMember : ambient.contains object = true :=
    usedBound object (by simp [collectLetValue])
  exact match_retainedScalarProjectionLetStep_binderReady
    sourceState targetState programs frames
    (ready.letRetained_continuationGraph fuelBound usedBound)
    joins env objectMember runtime step

/-- Exact deleted packed-scalar projection provenance supplies binder absence
and the hereditary continuation; its successful read is runtime-neutral. -/
theorem ExactShadowCodeBinderReady.match_deletedScalarProjectionLetStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel width offset : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {fvarId object : FVarId} {binderName : Name} {type : Expr}
...
/-- Complete graph-level matcher for packed-scalar projection lets. -/
theorem match_scalarProjectionLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- A retained unbox reads related covered boxed values and binds their
related payloads. Tagged boxes produce identical immediate scalar values;
heap boxes promote their payload child to a continuation root. -/
theorem match_retainedUnboxLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
      ...
      | some values =>
          ...
          | ok sourceValue =>
              have sourceObjectRoot :
                ...
                exact List.mem_append_left _
                  (lookup_mem_envRootsOn objectMember sourceObjectLookup)
              rcases runtime.unboxBoth_of_related sourceObjectRoot
              ...
              have targetTransition :
                  coreStep targetCurrent = .next targetExpected := by
                simp [targetCurrent, targetExpected, coreStep, evalLetValue,
                  targetObjectRead, targetResultEq, Bind.bind, Except.bind,
                  Pure.pure, Except.pure]
              have afterRelated :
                  ReachableMachineRelated fuel rho
                    sourceExpected targetExpected := by
                exact retainedLetValue_reachableRelated
                  sourceState targetState
                  sourceState.runtime targetState.runtime
                  programs frames continuation joins env resultValues
                  publishedRuntime
              exact ⟨targetExpected,
                match_internalCoreSteps sourceTransition targetTransition
                  afterRelated (by simpa [sourceCurrent] using step)⟩
...
/-- A retained partial application evaluates related covered fixed arguments,
aligns the target declaration arity, and allocates related closure objects at
fresh mapped locations. -/
theorem match_retainedPapLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
remain observable as a call. -/
theorem match_retainedFapLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
  ...
  | ok sourceArguments =>
      ...
      | ok argumentsRelated =>
          rename_i targetArguments
          have published := runtime.publishEvalArgs covered sourceArgumentsEq
            targetArgumentsEq argumentsRelated
          let sourceExpected := {
            sourceState with
            frames := .bind fvarId sourceContinuation sourceState.env
              sourceState.joins :: sourceState.frames
            control := .invokeName name sourceArguments }
          let targetExpected := {
            targetState with
            frames := .bind fvarId targetContinuation targetState.env
              targetState.joins :: targetState.frames
            control := .invokeName name targetArguments }
          have sourceTransition :
              coreStep sourceCurrent = .next sourceExpected := by
            simp [sourceCurrent, sourceExpected, coreStep, evalLetValue,
              sourceArgumentsEq, pushBindFrame, Bind.bind, Except.bind,
              Pure.pure, Except.pure]
          have targetTransition :
              coreStep targetCurrent = .next targetExpected := by
            simp [targetCurrent, targetExpected, coreStep, evalLetValue,
              targetArgumentsEq, pushBindFrame, Bind.bind, Except.bind,
              Pure.pure, Except.pure]
          have afterRelated :
              ReachableMachineRelated fuel rho sourceExpected
                targetExpected := by
            unfold ReachableMachineRelated
            exact ⟨sourceArguments.toList, targetArguments.toList,
              envRootsOn used sourceState.env ++ sourceFrameRoots,
              envRootsOn used targetState.env ++ targetFrameRoots,
              programs, .invokeName name argumentsRelated,
              .cons (.bind continuation joins env) frames,
              by simpa only [List.append_assoc] using published⟩
          exact ⟨targetExpected,
            match_internalCoreSteps sourceTransition targetTransition
              afterRelated (by simpa [sourceCurrent] using step)⟩

/-- Complete graph-level matcher for full-application lets.  The transparent
pass always retains this effectful shape: `DeletedLetReadyAt` has no full-
application constructor, so the deleted residual is uninhabited. -/
theorem match_fapLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- A retained reset reads related covered constructor objects, performs the
paired reference-count/update/release transition, and binds related reuse
tokens in the two continuations. -/
theorem match_retainedResetLetStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
ownership-frame readiness rule and take the source-only branch. -/
theorem match_resetLetCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
/-- State-level named-call matcher for an external source transition.
All cache, fault, partial-application, and internal-body branches contradict
the external result; the sole surviving declaration branch exposes related
requests and waiting machines under one address renaming. -/
theorem SomeReachableMachineRelated.matchInvokeNameExternal
    (related : SomeReachableMachineRelated fuel source target)
    (sourceControl : source.control =
      .invokeName name sourceArguments)
    (sourceTransition :
...
external entry is a distinct `CoreResult`. -/
theorem SomeReachableMachineRelated.invokeName_terminal
    (related : SomeReachableMachineRelated fuel source target)
    (sourceControl : source.control =
      .invokeName name sourceArguments)
    (done : coreStep source = .done sourceObservation) :
      ...
      | invokeName name arguments =>
          ...
          have matchDeclaration
            ...
            cases sourceFound :
                source.program.findDecl? name with
            | none =>
                have terminal := coreStep_invokeName_unknown_terminal
                  (externals := externals) source target programs arguments
                  sourceReady sourceFound runtime done'
            ...
            | some sourceDeclaration =>
                ...
                · cases sourceBinding :
                  ...
                  | error fault =>
                      have terminal :=
                        coreStep_invokeName_bindingFault_terminal
                          (externals := externals) source target programs
                          arguments sourceReady sourceFound sourceTooFew
...
internal code entry contradict the terminal premise; external entry is a
distinct `CoreResult` and cannot terminate synchronously. -/
theorem SomeReachableMachineRelated.invokeValue_terminal
    (related : SomeReachableMachineRelated fuel source target)
    (sourceControl : source.control =
      .invokeValue sourceFunction sourceArguments)
    (done : coreStep source = .done sourceObservation) :
      ...
      | invokeValue function arguments =>
          ...
          have matchNonHeap
              ...
              ∃ targetObservation,
                EvaluatesState externals target targetObservation ∧
                ObservationRel sourceObservation targetObservation := by
            have terminal := coreStep_invokeValue_nonHeap_terminal
              (externals := externals) (targetArguments := targetArguments)
              source target function sourceNotHeap runtime done'
            simpa only [targetSame] using terminal
          cases function with
          | tagged payload =>
              exact matchNonHeap (by
...
/-- Hereditary case-table matcher.  The selected alternative retains exact
compiler provenance, so the one matched tag-dispatch step establishes the
strong machine relation directly. -/
theorem match_casesStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
...
provenance. -/
theorem ExactShadowCodeBinderReady.match_casesStep
...
/-- Hereditary semantic-step form of a retained object-field write. -/
theorem match_retainedObjectSetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  | ok objectValue =>
      ...
      | ok fieldValue =>
          ...
          | ok sourceNextRuntime =>
              rcases
                  coreStep_retainedObjectSet_binderReadyReachableRelated
                    sourceState targetState programs frames continuation joins
                    env objectMember fieldCovered objectRead fieldRead effect
                    runtime with
                ⟨targetObjectValue, targetFieldValue, targetNextRuntime,
...
/-- Hereditary semantic-step form of a certified deleted object-field
write. -/
theorem match_deletedObjectSetStep_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  rcases coreStep_deletedObjectSet_of_ready_binderReady
      sourceState targetState programs frames continuation joins env runtime
      ready with
    ⟨nextRuntime, transition, related⟩
  rcases match_sourceOnlyCoreStep_binderReady transition related step with
    ⟨targetAfter, _targetEq, targetPath, afterRelated⟩
  exact ⟨targetAfter, targetPath, afterRelated⟩

/-- An exact retained object-write view supplies the live object, covered
field argument, and hereditary continuation required by the paired runtime
step. -/
theorem ExactShadowCodeBinderReady.match_objectSetRetainedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel index : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  have objectMember : ambient.contains object = true :=
    usedBound object ((collectArg_subset continuationUsed field) object live)
  have fieldCovered : ArgCovered ambient field :=
    (argCovered_collectArg continuationUsed field).mono usedBound
  exact match_retainedObjectSetStep_binderReady
    sourceState targetState programs frames
    (ready.objectSetRetained_continuationGraph fuelBound usedBound)
    joins env objectMember fieldCovered runtime step

/-- An exact deleted object-write view combines its hereditary continuation
with the dynamic unreachability certificate for the source-only update. -/
theorem ExactShadowCodeBinderReady.match_objectSetDeletedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel index : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {object : FVarId} {field : LCNF.Arg .impure}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_deletedObjectSetStep_of_ready_binderReady
    sourceState targetState programs frames
...
/-- Hereditary semantic-step form of a retained absolute-slot write. -/
theorem match_retainedUSizeSetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  | ok objectValue =>
      ...
      | ok fieldValue =>
          ...
          | ok sourceNextRuntime =>
              rcases
                  coreStep_retainedUSizeSet_binderReadyReachableRelated
                    sourceState targetState programs frames continuation joins
                    env objectMember fieldMember objectRead fieldRead effect
                    runtime with
                ⟨targetObjectValue, targetFieldValue, targetNextRuntime,
...
/-- Hereditary semantic-step form of a certified deleted absolute-slot
write. -/
theorem match_deletedUSizeSetStep_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  rcases coreStep_deletedUSizeSet_of_ready_binderReady
      sourceState targetState programs frames continuation joins env runtime
      ready with
    ⟨nextRuntime, transition, related⟩
  rcases match_sourceOnlyCoreStep_binderReady transition related step with
...
/-- An exact retained absolute-slot view supplies both live locals and the
hereditary continuation required by the paired runtime step. -/
theorem ExactShadowCodeBinderReady.match_usizeSetRetainedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel index : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {object field : FVarId}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  have objectMember : ambient.contains object = true :=
    usedBound object ((usedSubset_insert continuationUsed field) object live)
  have fieldMember : ambient.contains field = true := by
    apply usedBound field
    simp
  exact match_retainedUSizeSetStep_binderReady
    sourceState targetState programs frames
    (ready.usizeSetRetained_continuationGraph fuelBound usedBound)
    joins env objectMember fieldMember runtime step

/-- An exact deleted absolute-slot view combines its hereditary continuation
with the dynamic unreachability certificate for the source-only update. -/
theorem ExactShadowCodeBinderReady.match_usizeSetDeletedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel index : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {object field : FVarId}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_deletedUSizeSetStep_of_ready_binderReady
    sourceState targetState programs frames
...
/-- Hereditary semantic-step form of a retained packed-scalar write. -/
theorem match_retainedScalarSetStep_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
  ...
  | ok objectValue =>
      ...
      | ok fieldValue =>
          ...
          | ok sourceNextRuntime =>
              rcases
                  coreStep_retainedScalarSet_binderReadyReachableRelated
                    sourceState targetState programs frames continuation joins
                    env objectMember fieldMember objectRead fieldRead effect
                    runtime with
                ⟨targetObjectValue, targetFieldValue, targetNextRuntime,
...
/-- Hereditary semantic-step form of a certified deleted packed-scalar
write. -/
theorem match_deletedScalarSetStep_of_ready_binderReady
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (BinderReadyShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : BinderReadyReachableFramesRelated fuel rho
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  rcases coreStep_deletedScalarSet_of_ready_binderReady
      sourceState targetState programs frames continuation joins env runtime
      ready with
    ⟨nextRuntime, transition, related⟩
  rcases match_sourceOnlyCoreStep_binderReady transition related step with
...
/-- An exact retained packed-scalar view supplies both live locals and the
hereditary continuation required by the paired runtime step. -/
theorem ExactShadowCodeBinderReady.match_scalarSetRetainedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel width offset : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
  ...
  have fieldMember : ambient.contains field = true := by
    apply usedBound field
    simp
  exact match_retainedScalarSetStep_binderReady
    sourceState targetState programs frames
    (ready.scalarSetRetained_continuationGraph fuelBound usedBound)
    joins env objectMember fieldMember runtime step

/-- An exact deleted packed-scalar view combines its hereditary continuation
with the dynamic unreachability certificate for the source-only update. -/
theorem ExactShadowCodeBinderReady.match_scalarSetDeletedStep
    {initial continuationUsed ambient : UsedLocals}
    {nextFuel fuel width offset : Nat}
    {sourceContinuation targetContinuation : LCNF.Code .impure}
    {object field : FVarId} {type : Expr}
    ...
    ∃ targetAfter,
      ...
      BinderReadyReachableMachineRelated fuel rho
        sourceAfter targetAfter := by
  exact match_deletedScalarSetStep_of_ready_binderReady
    sourceState targetState programs frames
...
theorem match_incrementCodeStep
    (sourceState targetState : MachineState)
    (programs : ProgramRelated (ShadowCodeRelated fuel)
      sourceState.program targetState.program)
    (frames : ReachableFramesRelated fuel rho
...
theorem match_deleteCodeStep
  ...
  | ok objectValue =>
      ...
      | error fault =>
          ...
          exact (noStep done).elim
      | ok sourceNextRuntime =>
          rcases coreStep_delete_reachableRelated sourceState targetState
              programs frames continuation joins env objectMember objectRead
              effect runtime with
            ⟨targetObjectValue, targetNextRuntime, targetObjectRead,
              targetEffect, sourceTransition, targetTransition,
...
/-- Terminal retained increments transport missing-variable and live-cell
faults. Persistent increment instructions cannot be terminal. -/
theorem SomeReachableMachineRelated.increment_terminal
    (related : SomeReachableMachineRelated fuel source target)
    (sourceControl : source.control =
                      ...
                      | some objects =>
                          have sourceRoot :
                            ...
                            exact List.mem_append_left _
                              (lookup_mem_envRootsOn objectMember sourceLookup)
                          have effects :=
                            runtime.incValueBoth_related sourceRoot objects
                              (amount := amount) (check := check)
                          generalize sourceEffectEq :
                            incValue source.runtime sourceObject amount check =
                              sourceEffect at effects
                          generalize targetEffectEq :
                            incValue target.runtime targetObject amount check =
                              targetEffect at effects
                          cases effects with
                          | error faults =>
                              rename_i sourceFaultValue targetFaultValue
                  ...
                  | some objects =>
                      have sourceRoot :
                        ...
                        exact List.mem_append_left _
                          (lookup_mem_envRootsOn objectMember sourceLookup)
                      have effects :=
                        runtime.deleteValueBoth_related sourceRoot objects
                      generalize sourceEffectEq :
                        deleteValue source.runtime sourceObject =
                          sourceEffect at effects
                      generalize targetEffectEq :
                        deleteValue target.runtime targetObject =
                          targetEffect at effects
                      cases effects with
                      | error faults =>
                          rename_i sourceFaultValue targetFaultValue
