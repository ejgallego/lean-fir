...
| `reset` | Yes | Tagged, nonunique, and unique protocol theorems | Dead-object source-address, unique-nonconstructor `expectedConstructor`, unique-constructor object-bounds, and non-`expectedObject` recursively mapped child faults exact; fallback decrement and release-fuel exclusion remain, with erased child mismatch carded | Concrete Talos host plus tagged/fallback/unique source/compiler/adapter composition, exact unary call, reuse-token local write, arbitrary continuation, unique/nonunique whole-module executions, and unique/shared Node/V8 plus browser-Worker concrete execution; recursive child-fault leaves preserve the first exact mapped failure after any successful prefix and trap before the token local write or continuation |
