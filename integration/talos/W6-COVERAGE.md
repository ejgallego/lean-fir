...
| `cacheSet` | Typed concrete globals, recursive graph persistence, and Talos host | Constructive for all non-heap lanes and ordinary boxed/natural leaf roots; explicit `CachePersistenceRefines` remains for recursive heap graphs | Partial | Concrete hit/miss control, witness-indexed source/compiler judgment, terminating declaration call, host call, both global writes, cached-value reload, and local write compose; recursive constructor/closure persistence, per-declaration body proofs, and artifact switch pending |
...
- lazy-cache hits and misses compose through the declaration-call termination
  ...
  while recursive constructor/closure graphs keep the explicit proof premise
