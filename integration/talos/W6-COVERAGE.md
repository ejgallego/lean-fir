...
- lazy-cache hits and misses compose through the exact compiler/adapter shape,
  ...
  natural-literal declaration bodies instantiate that package through a
  concrete return base rule and witness-growing literal allocation, while the
  remaining declaration families still need instances; scalar, erased/reuse,
  direct-tag, promoted-tag, and ordinary boxed/natural/string leaf roots
  discharge cache persistence constructively;
