...
| `cacheSet` | Typed concrete globals, recursive graph persistence, and Talos host | Constructive for all non-heap lanes and ordinary boxed/natural leaf roots; dead targets are exact no-ops; constructor/closure folds and the complete same-fuel graph theorem are proved; explicit public-fuel `CachePersistenceRefines` remains for complete recursive graphs | Partial | Concrete hit/miss control, witness-indexed source/compiler judgment, terminating declaration call, host call, both global writes, cached-value reload, and local write compose; successful-fuel monotonicity/public lift, per-declaration body proofs, and artifact switch pending; canonical dead-child gap fixed by `FIR-BUG-wasm-none-persistence-dead-child-refinement` |
...
- lazy-cache hits and misses compose through the declaration-call termination
  ...
  complete ordinary-live-count-bounded same-fuel graph theorem are proved;
  canonical dead children are all-fuel no-ops and malformed dead headers remain
  target faults; only the successful-fuel public lift keeps the explicit proof
