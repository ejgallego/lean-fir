...
| `dec` | Yes | Complete recursive heap theorem for either outer check bit plus stale and live-zero-count mapped-heap theorems for every positive amount | Positive-count dead-object and `referenceCountUnderflow` source-address faults exact with no post-state at their respective first-step boundaries; zero amount is the specified empty-fold no-op; recursive child-fault packaging remains partial | Concrete checked/unchecked ordinary recursive host, checked tagged/promoted no-op, exact stale-object and direct-underflow terminal leaves, source step, compiler/adapter, generated unary call, persistent elision, continuation, checked and unchecked constructor-graph whole-module release, and balanced/recursive Node/V8 plus browser-Worker ownership executions compose with explicit closure-descriptor identity |
