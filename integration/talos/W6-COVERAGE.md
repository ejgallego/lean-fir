...
| `scalarSet` | Four integer widths | Heap theorems, including same-coordinate replacement and disjoint retained-field framing for `UInt32`/`UInt64` writes | Partial | Concrete width dispatcher, FVar source step, compiler/adapter, generated binary call, continuation, and compiler-shaped `UInt8`/`UInt16`/`UInt32`/`UInt64` whole-module, Node/V8, and browser-Worker write/readback compose; repeated same-coordinate writes refine the source replacement filter and execute twice in one module; disjoint two-coordinate `UInt32` and `UInt64` Talos modules preserve the first coordinate after the second write; narrow-write retained-field framing remains pending; invalid hand fixture retains its exact external-engine failure under `FIR-BUG-wasm-none-scalar-slot-layout-contract`; floats share the runtime gap |
...
- all four supported packed-integer mutations compose through their exact
  ...
  the second value; disjoint `UInt32` and `UInt64` writes preserve retained
  scalar observations of every supported width, and corresponding
  ...
  mutation/readback execution; `UInt8`/`UInt16` write framing remains open; and
