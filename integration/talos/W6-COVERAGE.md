...
| `cacheSet` | Typed concrete globals, recursive graph persistence, and Talos host | Constructive for all non-heap lanes; explicit `CachePersistenceRefines` remains for heap graphs | Partial | Concrete hit/miss control, witness-indexed source/compiler judgment, terminating declaration call, host call, both global writes, cached-value reload, and local write compose; recursive heap persistence, per-declaration body proofs, and artifact switch pending |
...
- lazy-cache hits and misses compose through the declaration-call termination
  ...
  result-local write; scalar, erased/reuse, direct-tag, and promoted-tag lanes
  discharge cache persistence constructively, while recursive heap graphs keep
  the explicit proof premise rather than the former heap-unchanged assumption;
  and
