...
| `reuse` | Yes | Fresh empty/nonempty and in-place theorems; one branch-independent successful operation theorem derives zero versus retained tokens from fitting capacity evidence, selects tagged/fresh-heap/in-place execution internally, and returns exact post-capacity, witness, descriptor, runtime/value, and header transport | Dead-object source-address and live-nonconstructor `expectedConstructor` faults exact; token/arity gates exclude earlier semantic faults; `reuseCapacitySafeProgram` excludes unknown or undersized retained tokens and exposes the in-place layout inequality; provenance-sensitive empty-layout result-kind validation remains open as `FIR-BUG-wasm-none-reuse-retained-result-kind` | Concrete Talos host plus all three source/compiler/adapter branches, arbitrary-arity token/field call, descriptor transport, result-local write, arbitrary continuation, in-place/fresh whole-module executions, and in-place/fresh Node/V8 plus browser-Worker concrete execution; a generic arbitrary-arity trap combinator proves result-local write and continuation unreachable. The branch-independent operation theorem is certificate-free, but its zero-token allocation/budget and ordinary retained-token provenance still need compiler-state derivations before it can replace the older branch-specific whole-program boundary |
...
- compiler-level coverage is now tracked separately from operation coverage.
  ...
  Successful reuse now has the analogous branch-independent lower operation
  theorem: fitting static evidence plus its dynamic relation determines zero
  ...
  missing retained-evidence/tagged-result relation case. Compiler-facing
  composition remains pending a constructive zero-token allocation budget,
  derivation of ordinary retained-token provenance, and the coordinated
  provenance-sensitive result-kind validator fix tracked by
