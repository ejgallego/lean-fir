...
The static fact map and its concrete meaning advance together. The separate
ordinary-token relation records the source ownership protocol, while local
frame alignment and the wasm32 address-space budget provide the two
constructive resources needed by the production step.
-/
def ConcreteReuseCapacityFrame
    (sourceFunction : Fir.Wasm.Function)
...
/--
The facts-indexed reuse frame strengthened with the immutable closure
descriptor agreement required by recursive ownership operations.
-/
def ConcreteReuseCapacityOwnershipFrame
    (sourceFunction : Fir.Wasm.Function)
    (facts : ReuseCapacityFacts) (remainingBytes : Nat)
    (sourceRuntime : RuntimeState) (sourceEnv : Env)
    (targetStore : Wasm.Store Host) (targetLocals : Wasm.Locals)
    (witness : RefinementWitness) : Prop :=
  ConcreteReuseCapacityFrame sourceFunction facts remainingBytes sourceRuntime
      sourceEnv targetStore targetLocals witness ∧
    targetStore.host.closureDescriptors = witness.closureDescriptors
...
advance the same authoritative fact transfer and threaded invariant.
-/
theorem ReuseCapacityDirectLetRuntimeRefinesWithCost.or
    {context : Fir.Wasm.Context}
    {sourceModule : Fir.Wasm.Module}
    {sourceFunction : Fir.Wasm.Function}
    {labels : List FVarId}
...
/--
A facts-indexed direct law lifts through closure-descriptor agreement. Every
successful direct step already exposes preservation of the host and witness
descriptor tables independently.
-/
theorem
...
The authoritative validator transfer erases only the destination's shadowed
...
theorem ConcreteSupportedExport.reuseCapacityDirectLetRuntimeRefinesWithCost_localAlias
  ...
  rcases sourceFacts with ⟨rfl, sourceLookup⟩
  obtain ⟨sourceIndex, sourceFound, sourceKindAt⟩ :=
    spec.localsAligned sourceCompiled
  obtain ⟨alignedResultIndex, alignedResultFound, resultKindAt⟩ :=
    spec.localsAligned resultCompiled
  rw [resultFound] at alignedResultFound
  injection alignedResultFound with resultIndexEq
  subst alignedResultIndex
  obtain ⟨physical, sourcePhysical, physicalRelated⟩ :=
    related.stateRelated.resolve sourceLookup sourceFound sourceKindAt
  have expectedCompiled :
      Fir.Wasm.compileLetValue context decl =
        .ok [.localGet sourceId] := by
    have emptyCompiled :
  ...
  have step :
      LetStepSimulates context sourceFunction target.wasmModule hosts.env decl
        [.localGet sourceIndex] nextRuntime nextRuntime sourceEnv
        sourceValue targetStore targetStore targetLocals updated resultIndex
        witness witness :=
    letStepSimulates_localAlias valueEq sourceLookup related.stateRelated
      resultFound resultKindAt sourcePhysical physicalRelated targetSet
  have nextRelated :
      ReuseCapacityStateRelated
        (eraseReuseCapacityFact facts decl.fvarId) sourceFunction nextRuntime
        (bind sourceEnv decl.fvarId sourceValue) targetStore updated witness :=
    related.ofLetStepEraseOfSet step resultFound targetSet
      (WitnessTransport.refl witness)
      (HeaderCapacityTransport.refl targetStore.host.runtime.heap witness)
  have nextOrdinary :
...
theorem
  ...
  | cons objectFound noMore =>
      ...
      | word32 objectRelated =>
          ...
          obtain ⟨scalar, unboxObjectRelated, concreteRead, sourceValueEq⟩ :=
            FirTalos.Concrete.ConcreteRuntimeRel.unboxFacts_of_sourceCompatible
              related.stateRelated.1 objectRelated compatible unboxed
          subst sourceValue
          obtain ⟨updated, targetSet, nextFrameAligned⟩ :=
            frameAligned.set?
              (nextRuntime := nextRuntime)
          ...
          obtain ⟨imp, imported, inBounds, contracted, params, results⟩ :=
            spec.unboxCall callFound
          have step :=
            letStepSimulates_unbox (context := context) valueEq resultTypeEq
              sourceLookup unboxed related.stateRelated resultFound
              resultKindAt hObject unboxObjectRelated concreteRead imported
              spec.hostsSatisfy inBounds contracted params results targetSet
          have nextRelated :
              ReuseCapacityStateRelated
                (eraseReuseCapacityFact facts decl.fvarId) sourceFunction
                nextRuntime
                (bind sourceEnv decl.fvarId scalar.semanticValue)
                (clearFailure targetStore) updated witness :=
            related.ofHeapPreservingLetStepErase step resultFound targetSet
              (WitnessTransport.refl witness) (by simp [clearFailure])
          have nextOrdinary :
...
/--
Successful sharing observations preserve the facts-indexed reuse frame. They
  ...
  | cons objectFound noMore =>
      ...
      | word32 objectRelated =>
          ...
          obtain ⟨imp, imported, inBounds, contracted, params, results⟩ :=
            spec.isSharedCall callFound
          have step :=
            letStepSimulates_isShared (context := context) valueEq sourceLookup
              evaluated related.stateRelated resultFound resultKindAt hObject
              objectRelated imported spec.hostsSatisfy inBounds contracted
              params results targetSet
          have nextRelated :
              ReuseCapacityStateRelated
                (eraseReuseCapacityFact facts decl.fvarId) sourceFunction
...
This is a source semantics relation, not a translation certificate: each
constructor contains only source admission, the executable source step, and
the validator's own fact-transfer equation.
-/
inductive ReuseCapacityCodeEvaluates (context : Fir.Wasm.Context)
    (Supported : ReuseCapacityFacts → LCNF.LetDecl .impure → Prop)
    (letCost : LCNF.LetDecl .impure → Nat) :
  ...
  | ret
      (sourceLookup : lookup sourceEnv result = some sourceValue) :
      ReuseCapacityCodeEvaluates context Supported letCost facts sourceRuntime
        sourceEnv (.return result) facts sourceRuntime sourceEnv sourceValue 0
  | letValue
      (supported : Supported facts decl)
      (sourceStep :
        SourceLetResult context sourceRuntime sourceEnv decl nextRuntime
          sourceValue)
      (transfer :
        reuseCapacityLetFacts? facts decl = some nextFacts)
      (continued :
        ReuseCapacityCodeEvaluates context Supported letCost nextFacts
      ...
      ReuseCapacityCodeEvaluates context Supported letCost facts sourceRuntime
        sourceEnv (.let decl continuation) resultFacts resultRuntime resultEnv
        resultValue (letCost decl + continuationCost)

/-- The fact-indexed finite relation denotes an exact interpreter run. -/
theorem ReuseCapacityCodeEvaluates.execEvaluates
    {context : Fir.Wasm.Context}
    {Supported : ReuseCapacityFacts → LCNF.LetDecl .impure → Prop}
    {letCost : LCNF.LetDecl .impure → Nat}
...
The theorem follows the source-selected finite evaluation, inverts the actual
production compiler/adapter output at each declaration, and threads the exact
fact map, ordinary-token relation, local frame, and address-space budget to
the return. Nontermination is outside this first finite theorem and will use
the same step law in the later trace/simulation layer.
-/
theorem
    ConcreteSupportedExport.codeWP_of_reuseCapacityCodeEvaluates_exactReturn_of_runtimeRefines
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    {resultValue : Value} {requiredBytes : Nat}
    (evaluation :
      ReuseCapacityCodeEvaluates context Supported directLetAllocationCost facts
    ...
    (runtimeRefines :
      ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
        sourceFunction labels target.wasmModule hosts.env Supported
        directLetAllocationCost
...
theorem
  ...
  | @letValue facts decl letSourceRuntime letSourceEnv letNextRuntime
      ...
      subst producedFacts
      have continuationInvariant :
          Frame nextFacts continuationCost letNextRuntime
...
-/
theorem ConcreteSupportedExport.codeWP_of_reuseCapacityCodeEvaluates_exactReturn
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    {resultValue : Value} {requiredBytes : Nat}
    (evaluation :
      ReuseCapacityCodeEvaluates context (ReuseSupported context)
        directLetAllocationCost facts sourceRuntime sourceEnv code resultFacts
        resultRuntime resultEnv resultValue requiredBytes)
    (adapted :
      CodeAdapted context sourceModule sourceFunction labels code targetCode)
  ...
  spec.codeWP_of_reuseCapacityCodeEvaluates_exactReturn_of_runtimeRefines
    evaluation adapted localsAligned invariant
    spec.reuseCapacityDirectLetRuntimeRefinesWithCost

/--
Public function-body form of the generic facts-indexed structural theorem.
-/
theorem
    ConcreteSupportedExport.codeWP_of_reuseCapacityCodeEvaluates_of_runtimeRefines
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  obtain ⟨resultStore, resultLocals, resultWitness, resultKind, physical,
    ...
    spec.codeWP_of_reuseCapacityCodeEvaluates_exactReturn_of_runtimeRefines
      evaluation adapted localsAligned invariant runtimeRefines
  refine ⟨resultStore, resultLocals, resultWitness, resultKind, physical, ?_,
    resultInvariant, failureClear, valueRelated⟩
...
/-- Reuse-only public function-body specialization. -/
theorem ConcreteSupportedExport.codeWP_of_reuseCapacityCodeEvaluates
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    ∃ resultStore resultLocals resultWitness resultKind physical,
      CodeWP context sourceModule sourceFunction labels target.wasmModule
        ...
        resultStore.host.failure? = none ∧
        PhysicalValueRel resultWitness resultKind physical resultValue :=
  spec.codeWP_of_reuseCapacityCodeEvaluates_of_runtimeRefines evaluation adapted
    localsAligned invariant spec.reuseCapacityDirectLetRuntimeRefinesWithCost
    parameterCount

/--
Whole-export finite partial correctness for the certificate-free,
facts-indexed reuse fragment.

The source evaluation fixes only the successful finite path and its validator
facts. The generated target body, runtime calls, concrete branches, and local
indices are all recovered from the production compiler and resolver.
-/
theorem
    ConcreteSupportedExport.correctReuseCapacityCode_of_runtimeRefines
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    (runtimeRefines :
      ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
        sourceFunction [] target.wasmModule hosts.env Supported
        directLetAllocationCost
        (ConcreteReuseCapacityFrame sourceFunction))
    (parameterCount :
      parameters.length = spec.targetFunction.numParams) :
...
/-- Reuse-only specialization of the generic facts-indexed export theorem. -/
theorem ConcreteSupportedExport.correctReuseCapacityCode
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  spec.correctReuseCapacityCode_of_runtimeRefines evaluation invariant
    spec.reuseCapacityDirectLetRuntimeRefinesWithCost parameterCount

/--
First mixed facts-indexed source fragment: successful validated reuse may
interleave with cost-zero local aliases.
-/
def ReuseAliasSupported (context : Fir.Wasm.Context)
...
theorem ConcreteSupportedExport.reuseCapacityDirectLetRuntimeRefinesWithCost_reuseAlias
  ...
  exact ReuseCapacityDirectLetRuntimeRefinesWithCost.or
    (Left := ReuseSupported context)
    (Right := fun _ decl => LocalAliasSupported context decl)
    spec.reuseCapacityDirectLetRuntimeRefinesWithCost
    spec.reuseCapacityDirectLetRuntimeRefinesWithCost_localAlias

/--
Finite whole-export partial correctness for arbitrary interleavings of
successful validated reuse and local aliases.

This is the first mixed operation-family endpoint for the facts-indexed path.
It is parameterized only by source evaluation and the initial resource
relation; target code and per-program translation certificates remain absent.
-/
theorem ConcreteSupportedExport.correctReuseAliasCode
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    {resultValue : Value} {requiredBytes : Nat}
    (evaluation :
      ReuseCapacityCodeEvaluates context (ReuseAliasSupported context)
        directLetAllocationCost facts sourceRuntime sourceEnv sourceCode
        resultFacts resultRuntime resultEnv resultValue requiredBytes)
    (invariant :
      ConcreteReuseCapacityFrame sourceFunction facts requiredBytes
  ...
  (ReuseAliasSupported context facts decl ∨
    (ImmediateLiteralSupported context decl ∨
      (USizeProjectionSupported context decl ∨
        (ObjectProjectionSupported context decl ∨
          ScalarProjectionSupported context decl)))) ∨
    (UnboxSupported context decl ∨ IsSharedSupported context decl)

/--
Reuse, aliases, immediate literals, all three successful projection families,
typed unboxing, and sharing observations share one facts-indexed production
runtime law.
-/
theorem
  ...
  have reuseAndReaders :
      ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
        ...
        (ConcreteReuseCapacityFrame sourceFunction) :=
    ReuseCapacityDirectLetRuntimeRefinesWithCost.or
      (Left := ReuseAliasSupported context)
      (Right := fun _ decl =>
        ImmediateLiteralSupported context decl ∨
          (USizeProjectionSupported context decl ∨
            (ObjectProjectionSupported context decl ∨
              ScalarProjectionSupported context decl)))
      spec.reuseCapacityDirectLetRuntimeRefinesWithCost_reuseAlias
      immediateAndProjections
  have unboxAndShared :
      ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
        sourceFunction labels target.wasmModule hosts.env
        (fun _ decl =>
        ...
        (ConcreteReuseCapacityFrame sourceFunction) :=
    ReuseCapacityDirectLetRuntimeRefinesWithCost.or
      (Left := fun _ decl => UnboxSupported context decl)
      (Right := fun _ decl => IsSharedSupported context decl)
      spec.reuseCapacityDirectLetRuntimeRefinesWithCost_unbox
      spec.reuseCapacityDirectLetRuntimeRefinesWithCost_isShared
  intro facts sourceRuntime nextRuntime sourceEnv decl sourceValue valueCode
    targetValue targetStore targetLocals resultIndex remainingBytes witness
    supported stepFits invariant sourceStep valueCompiled valueAdapted
    resultFound
  cases supported with
  | inl reuseOrReader =>
      exact reuseAndReaders reuseOrReader stepFits invariant sourceStep
        valueCompiled valueAdapted resultFound
  | inr unboxOrShared =>
      exact unboxAndShared unboxOrShared stepFits invariant sourceStep
        valueCompiled valueAdapted resultFound
...
theorem
    ...
    ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
      sourceFunction labels target.wasmModule hosts.env
      (ReuseBudgetedDirectSupported context) directLetAllocationCost
      (ConcreteReuseCapacityOwnershipFrame sourceFunction) := by
  change
    ReuseCapacityDirectLetRuntimeRefinesWithCost context sourceModule
      sourceFunction labels target.wasmModule hosts.env
      (ReuseBudgetedDirectSupported context) directLetAllocationCost
  ...
  exact
    ReuseCapacityDirectLetRuntimeRefinesWithCost.preservingClosureDescriptorAgreement
      ...
      (Supported := ReuseBudgetedDirectSupported context)
...
/--
Ordinary increment also preserves the ownership-strengthened reuse frame.
The concrete step changes only heap memory and retains the same witness, so
...
theorem
  ...
  | inc sourceRuntime nextRuntime sourceEnv objectId amount check continuation
      ...
      have nextBudget : heap.AddressSpaceBudget remainingBytes := by
        ...
        · simpa [cursor] using budget.endWithinAddressSpace
      have nextAgreement :
          (replaceHeap targetStore heap).host.closureDescriptors =
            witness.closureDescriptors := by
        simpa [replaceHeap, clearFailure] using invariant.2
      exact ⟨targetRest, replaceHeap targetStore heap, witness,
...
/-- One uniform effect law for the currently admitted ownership prefix. -/
theorem
    ConcreteSupportedExport.effectRuntimeRefines_reuseOwnershipThroughDecrement
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  · apply EffectRuntimeRefines.or
    · exact
        spec.effectRuntimeRefines_ordinaryIncrement_reuseCapacityOwnership
    · exact
        spec.effectRuntimeRefines_ordinaryDecrement_reuseCapacityOwnership

/-- One uniform facts-indexed runtime law for the complete ownership family. -/
theorem ConcreteSupportedExport.effectRuntimeRefines_reuseOwnership
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
...
/-- Both FVar and erased object-field forms preserve the facts-indexed frame. -/
theorem ConcreteSupportedExport.effectRuntimeRefines_objectField_reuseCapacityOwnership
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    EffectRuntimeRefines context sourceModule sourceFunction labels
      ...
      (ConcreteReuseCapacityOwnershipFrame sourceFunction facts) := by
  apply EffectRuntimeRefines.or
  · exact spec.effectRuntimeRefines_objectFieldFVar_reuseCapacityOwnership
  · exact spec.effectRuntimeRefines_objectFieldErased_reuseCapacityOwnership

/--
One facts-indexed runtime law for ownership, tag mutation, and both
object-field argument forms.
-/
theorem
    ConcreteSupportedExport.effectRuntimeRefines_reuseOwnershipTagAndObject
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
...
/--
Finite whole-export partial correctness through recursive ownership release:
the complete direct/reuse fragment may interleave persistent ownership,
ordinary increment, and successful recursive decrement.
-/
theorem
    ConcreteSupportedExport.correctReuseBudgetedDirectOwnershipThroughDecrementCode
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  spec.correctReuseCapacityEffectCode evaluation invariant
    ...
    (fun _ => spec.effectRuntimeRefines_reuseOwnershipThroughDecrement)
    parameterCount

/--
Finite whole-export partial correctness for the complete current ownership
family interleaved with every facts-indexed direct/reuse operation.
-/
theorem ConcreteSupportedExport.correctReuseBudgetedDirectOwnershipCode
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
    ...
    (evaluation :
      ReuseCapacityEffectCodeEvaluates context
        (ReuseBudgetedDirectSupported context)
        (OwnershipEffectSupported context) directLetAllocationCost facts
        sourceRuntime sourceEnv sourceCode resultFacts resultRuntime resultEnv
        resultValue requiredBytes)
    (invariant :
      ConcreteReuseCapacityOwnershipFrame sourceFunction facts requiredBytes
...
/--
Finite whole-export partial correctness for every facts-indexed direct/reuse
operation interleaved with ownership, tag mutation, and either successful
object-field argument form.
-/
theorem
    ConcreteSupportedExport.correctReuseBudgetedDirectOwnershipTagAndObjectCode
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
...
/--
Finite whole-export partial correctness for arbitrary nesting of default-only
cases around every facts-indexed direct/reuse operation and the complete
current ownership, tag, object, `USize`, and packed-integer field mutation
families.

The source evaluation selects each sole default branch. The production
compiler erases that wrapper to the selected branch, while the authoritative
reuse fact map and allocation budget remain unchanged.
-/
theorem
    ConcreteSupportedExport.correctReuseBudgetedDirectOwnershipTagAllFieldMutationDefaultCases
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceCode : LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
