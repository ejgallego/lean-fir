...
/-- A related resident Array exposes exactly its live owned prefix as both
semantic ownership values and contiguous physical payload lanes.  Capacity
remains a framing fact: spare retained slots never enter the relation. -/
theorem ResidentArrayObjectRel.readOwnedPayloadWords
    {state : MemoryState} {witness : RefinementWitness} {address : Word32}
    ...
    ∃ words,
      readOwnedReferences state address header = .ok words ∧
      OwnershipValuesRel witness words elements.toList ∧
      OwnedPayloadWordsRel state address header
        (List.range elements.size) words := by
  obtain ⟨words, wordsRead, ownership⟩ := related.readOwnedReferences
  have headerValid :
    ...
    simp [opaqueEq, related.headerKind, related.marker, related.reserved,
      related.logicalSize, related.physicalCapacity, related.sizeCapacity,
      related.allocationBytes]
  have decoded := wordsRead
  unfold readOwnedReferences at decoded
  rw [related.headerKind] at decoded
  simp only at decoded
  rw [if_pos (by simp [related.marker])] at decoded
  unfold readResidentArrayOwnedReferences at decoded
  simp only [headerValid, ↓reduceIte] at decoded
  rw [related.logicalSize] at decoded
  have payload : OwnedPayloadWordsRel state address header
      (List.ofFn (fun position : Fin elements.size => position.val)) words :=
    OwnedPayloadWordsRel.ofFnM decoded (by
      intro position word operation
      have rawRead :
          state.memory.readWord32
              (address.value + headerBytes +
                target.semanticSlotBytes * position.val) = .ok word := by
        unfold liftMemory at operation
        cases read : state.memory.readWord32
            (address.value + headerBytes +
              target.semanticSlotBytes * position.val) <;>
            simp_all
      have withinLayout :
          headerBytes + target.semanticSlotBytes * position.val + 4 ≤
        ...
        simp [residentArrayAllocationBytes, target] at aligned ⊢
        omega
      have within :
          headerBytes + target.semanticSlotBytes * position.val + 4 ≤
      ...
      have inBounds :
        ...
        have cursorInBounds := valid.cursorInBounds
        omega
      exact ⟨within, inBounds, rawRead⟩)
  have indicesEq :
      List.ofFn (fun position : Fin elements.size => position.val) =
        List.range elements.size := by
    apply List.ext_getElem?
    intro index
    by_cases inRange : index < elements.size
    · simp [inRange]
    · simp [inRange]
  rw [indicesEq] at payload
  exact ⟨words, wordsRead, ownership, payload⟩

/-- Physical capture indices selected by the closure ownership filter. -/
def closureOwnedCaptureIndices (index : Nat) : List Fir.Wasm.AbiKind →
    List Nat
  | [] => []
  | kind :: rest =>
      if kind.isObjectField then
        index :: closureOwnedCaptureIndices (index + 1) rest
      else
...
theorem readClosureCapture_word_of_eq_ok
    {memory : LinearMemory} {address : Nat} {kind : Fir.Wasm.AbiKind}
    {word : Word32} (admissible : kind.isObjectField = true)
    (operation :
      memory.readClosureCapture address kind = .ok (.word32 word)) :
    memory.readWord32 address = .ok word := by
  cases kind <;> simp [Fir.Wasm.AbiKind.isObjectField] at admissible
  all_goals
    unfold LinearMemory.readClosureCapture at operation
    simp only [Fir.Wasm.AbiKind.valueType] at operation
    cases rawRead : memory.readWord32 address with
    | error failure =>
        rw [rawRead] at operation
        contradiction
    | ok actual =>
        rw [rawRead] at operation
        simp only [Bind.bind, Except.bind] at operation
        cases paddingRead : memory.readUInt32 (address + 4) with
        | error failure =>
            rw [paddingRead] at operation
            contradiction
        | ok padding =>
            rw [paddingRead] at operation
            by_cases zero : padding = 0
...
/-- Descriptor-filtered closure decoding retains the exact physical payload
indices consumed by W7.  Scalar captures advance the physical slot without
adding a recursive child. -/
theorem readClosureOwnedReferences_payload
    {state : MemoryState} {address : Word32} {header : Header}
    {index captureCount : Nat} {kinds : List Fir.Wasm.AbiKind}
    {words : List Word32}
    (operation : readClosureOwnedReferences state address index kinds =
      .ok words)
    (fits : index + kinds.length ≤ captureCount)
    (allocationFits :
      headerBytes + target.semanticSlotBytes * captureCount ≤
        header.allocationBytes.toNat)
    (extent :
      closureCaptureAddress address.value captureCount ≤ state.heapCursor)
    (valid : state.FrontierInvariant) :
    OwnedPayloadWordsRel state address header
      (closureOwnedCaptureIndices index kinds) words := by
  induction kinds generalizing index words with
  | nil =>
      simp only [readClosureOwnedReferences] at operation
      have wordsEq := Except.ok.inj operation
      subst words
      exact .nil
  | cons kind rest ih =>
      simp only [List.length_cons] at fits
      unfold readClosureOwnedReferences at operation
      by_cases admissible : kind.isObjectField = true
      · rw [if_pos admissible] at operation
        cases captureRead : state.memory.readClosureCapture
            (closureCaptureAddress address.value index) kind with
        | error failure =>
            rw [captureRead] at operation
            contradiction
        | ok lane =>
            simp only [liftMemory, captureRead, Bind.bind, Except.bind]
              at operation
            cases lane with
            | word32 word =>
                cases tailRead : readClosureOwnedReferences state address
                    (index + 1) rest with
                | error failure =>
                    rw [tailRead] at operation
                    contradiction
                | ok tailWords =>
                    ...
                    have wordsEq := Except.ok.inj operation
                    subst words
                    have rawRead := readClosureCapture_word_of_eq_ok
                      admissible captureRead
                    have tailPayload := ih tailRead (by omega)
                    have currentLt : index < captureCount := by omega
                    have within :
                        headerBytes + target.semanticSlotBytes * index + 4 ≤
                          header.allocationBytes.toNat := by
                      have allocationFits' := allocationFits
                      simp [target] at allocationFits' ⊢
                      omega
                    have inBounds :
                        address.value + headerBytes +
                              target.semanticSlotBytes * index + 3 <
                            state.memory.size := by
                      have cursorInBounds := valid.cursorInBounds
                      have extent' := extent
                      simp [closureCaptureAddress, target] at extent' ⊢
                      omega
                    simp only [closureOwnedCaptureIndices, admissible,
                      if_true]
                    exact .cons within inBounds
                      (by simpa [closureCaptureAddress] using rawRead)
                      tailPayload
            | word64 value | float32Bits value | float64Bits value =>
                simp at operation
      · have rejected : kind.isObjectField = false := by
          cases found : kind.isObjectField <;> simp_all
        rw [if_neg (by simpa using rejected)] at operation
        simpa [closureOwnedCaptureIndices, rejected] using
          ih operation (by omega)

/-- A related closure's public ownership decoder supplies the filtered
semantic captures and the exact physical indices consumed by W7. -/
theorem ClosureObjectRel.readOwnedPayloadWords
    ...
    (valid : state.FrontierInvariant) :
    ∃ words,
      readClosureOwnedReferences state address 0 captureKinds.toList =
        .ok words ∧
      OwnershipValuesRel witness words
        (closureOwnedValues captureKinds.toList captures.toList) ∧
      OwnedPayloadWordsRel state address header
        (closureOwnedCaptureIndices 0 captureKinds.toList) words := by
  obtain ⟨words, wordsRead, ownership⟩ :=
    related.readClosureOwnedReferences
  obtain ⟨metadata, metadataRead, _, _, _, _⟩ := related.metadata
  have heap :=
  ...
  have requiredFits :
      align8 (headerBytes +
        target.semanticSlotBytes * header.aux2.toNat) ≤
          header.allocationBytes.toNat := by
    have check := decodedRead
    unfold readClosureHeader at check
    simp only [heap, if_true] at check
    ...
    split at check
    · rename_i validCheck
      have validProp : header.aux2.toNat < header.aux1.toNat ∧
          align8 (headerBytes +
            target.semanticSlotBytes * header.aux2.toNat) ≤
              header.allocationBytes.toNat := by
  ...
  have captureKindsSize : captureKinds.toList.length = captures.size := by
    simpa using related.captureKindsSize
  have payload := readClosureOwnedReferences_payload wordsRead
    (captureCount := captures.size) (by simp [captureKindsSize])
    allocationFits extent valid
  exact ⟨words, wordsRead, ownership, payload⟩
...
/-- Common successful outcome of one fuel-indexed decrement across the
concrete heap and resident Wasm memory.  Constructor, closure, and Array
admission theorems differ only in how they establish this relation; the
installed call boundary can therefore be proved once. -/
def DecrementOnceSuccess
    {host : Type} (fuel : Nat) (state : MemoryState)
    (witness : RefinementWitness)
    (nextRuntime : Fir.LeanIR.Impure.RuntimeState)
    (address : Word32) (check : Bool)
...
/-- Turn a relational return proof for the installed decrement body into the
public, fuel-free call predicate.  The generated helper has no result lanes,
so the caller tail is preserved while the final store remains related to an
existential concrete result. -/
theorem DecrementOnceInstallation.terminatesWith_of_success_wp
    {host : Type} {sourceModule : Fir.Wasm.Module}
    {module : Wasm.Module} {env : Wasm.HostEnv host}
    {descriptors : Array (Array Fir.Wasm.AbiKind)}
    {state : MemoryState} {witness : RefinementWitness}
    ...
    (bodyWP : Wasm.wp module installation.targetFunction.body
      ...
      store (decrementEntry (UInt32.ofNat address.value) checkWord) env) :
    Wasm.TerminatesWith env module installation.index store
  ...
  apply FirTalos.Correctness.terminatesWith_of_wp_body_at
    installation.notImport installation.installed
  rw [installation.entry (UInt32.ofNat address.value) checkWord tail]
  apply Wasm.wp.conseq _ bodyWP
  intro continuation completed
  obtain ⟨result, finalStore, success, rfl⟩ := completed
  obtain ⟨params, _locals, results⟩ := installation.signature
  simp [FirTalos.Correctness.FunctionBodyPost, Wasm.Function.numParams,
    params, results]
...
/-- A closure header skips the constructor arm and selects exactly the
supplied descriptor-dispatch body. -/
theorem wp_ownedReleaseProgram_closure
    {host : Type} {module : Wasm.Module} {env : Wasm.HostEnv host}
    {Q : Wasm.Assertion host} {store : Wasm.Store host}
    {object check flags descriptor refCount kind marker count
      captureCount : UInt32}
    {constructorBody closureBody opaqueBody : Wasm.Program}
    (notConstructor : kind ≠ ObjectKind.constructor.code)
    (closureKind : kind = ObjectKind.closure.code)
    (closureWP : Wasm.wp module closureBody (TerminalPost Q) store
      (ownedEntry object check flags descriptor refCount kind marker count
        captureCount) env) :
    Wasm.wp module
      (ownedReleaseProgram constructorBody closureBody opaqueBody) Q store
  ...
  apply Wasm.wp_iff_cons rfl
  rw [if_pos (by decide : (1 : UInt32) ≠ 0)]
  apply Wasm.wp.conseq _ closureWP
  intro continuation terminal
  cases continuation <;> simp_all [TerminalPost]

/-- A resident generic Array header skips constructor and closure dispatch and
selects exactly the supplied opaque-marker body. -/
theorem wp_ownedReleaseProgram_opaque
    {host : Type} {module : Wasm.Module} {env : Wasm.HostEnv host}
    {Q : Wasm.Assertion host} {store : Wasm.Store host}
    {object check flags descriptor refCount kind marker count
      captureCount : UInt32}
    ...
    (notConstructor : kind ≠ ObjectKind.constructor.code)
    (notClosure : kind ≠ ObjectKind.closure.code)
    (opaqueKind : kind = ObjectKind.opaque.code)
    (opaqueWP : Wasm.wp module opaqueBody (TerminalPost Q) store
      (ownedEntry object check flags descriptor refCount kind marker count
        captureCount) env) :
    Wasm.wp module
      (ownedReleaseProgram constructorBody closureBody opaqueBody) Q store
  ...
  apply Wasm.wp_iff_cons rfl
  rw [if_pos (by decide : (1 : UInt32) ≠ 0)]
  apply Wasm.wp.conseq _ opaqueWP
  intro continuation terminal
  cases continuation <;> simp_all [TerminalPost]
...
/-- Complete fuel-indexed three-semantics refinement for a count-one closure.
The static descriptor filters scalar captures before the shared ownership
fold; the remaining physical indices are released in source order. -/
theorem LiveHeapRel.decrementOnceProgram_closure_refines
    {host : Type} {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
    {runtime nextRuntime : Fir.LeanIR.Impure.RuntimeState}
    ...
    (releaseRun : Wasm.TerminatesWith env module releaseHeaderIndex store
      ...
      (fun final values =>
        final = releaseHeaderStore store (UInt32.ofNat address.value) ∧
          values = []))
    (closureBodyCorrect : ∀ {releasedStore finalStore : Wasm.Store host},
      ReleaseChildrenRun env module decrementIndex
          (UInt32.ofNat address.value)
          (closureOwnedCaptureIndices 0 captureKinds.toList)
          releasedStore finalStore →
      Wasm.wp module closureBody
        (TerminalPost (fun continuation =>
          continuation = .Return finalStore [])) releasedStore
        (ownedEntry (UInt32.ofNat address.value) checkWord header.flags
          header.aux3 header.refCount header.kind.code header.aux0
          header.aux1 header.aux2) env) :
    ∃ result finalStore,
      decrementReferenceOnceFuel (fuel + 1) state address check
          witness.closureDescriptors = .ok result ∧
      LiveHeapRel result witness nextRuntime ∧
      ...
      Wasm.wp module
        ...
        (decrementEntry (UInt32.ofNat address.value) checkWord) env := by
  obtain ⟨words, ownedRead, ownershipRelated, payloadRelated⟩ :=
    ClosureObjectRel.readOwnedPayloadWords objectRelated headerRead headerKind
      fixedCount extent related.frontier
  ...
  have parentPayloadFrame :=
    related.mappedPayloadFrame_of_headerWrite objectRelated.descriptor rawRead
  ...
  have releasedMemory : ResidentMemoryRel released
    ...
    simpa [releaseHeaderStore] using
      ResidentMemoryRel.releaseHeader memoryRelated exact headerInBounds
        headerWrite
  have releasedCanonical : CanonicalMappedHeadersRel released witness :=
    canonicalHeaders.ofHeaderWrite related objectRelated.descriptor rawRead
      releasedEq headerInBounds headerWrite
  let releaseChild : Fir.LeanIR.Impure.RuntimeState →
      Fir.LeanIR.Impure.Value →
      Except Fir.LeanIR.Impure.RuntimeFault
        Fir.LeanIR.Impure.RuntimeState := fun next value =>
  ...
  obtain ⟨result, finalStore, concreteFold, finalRelated, finalMemory,
      finalCanonical, finalFrame, childRuns⟩ :=
    OwnershipValuesRel.releaseChildrenRun_refines ownershipRelated
      payloadRelated mapped rawRead localClosure.headerOwned parentRelated
      releasedMemory releasedCanonical parentPayloadFrame step
  ...
  have notPromoted : header.isPromotedTag = false := by
    have different :
        (ObjectKind.closure == ObjectKind.natural) = false := by decide
    simp [Header.isPromotedTag, headerKind, headerOrdinary, different]
  have headerNonzero : header.refCount ≠ 0 := by
    intro zero
    rw [zero] at refCount
  ...
  have ownedReadWithDescriptors :
    ...
    simpa [readOwnedReferences, headerKind, descriptorLookup,
      objectRelated.captureKindsSize, fixedCount] using ownedRead
  have concreteOperation :
      decrementReferenceOnceFuel (fuel + 1) state address check
        witness.closureDescriptors = .ok result := by
    simp only [decrementReferenceOnceFuel]
  ...
  have ownedExecution :
      Wasm.wp module
        ...
        (ownedEntry (UInt32.ofNat address.value) checkWord header.flags
          header.aux3 header.refCount header.kind.code header.aux0
          header.aux1 header.aux2) env := by
    apply wp_ownedReleaseProgram_closure
      (by simp [headerKind]; decide) (by simp [headerKind])
    apply Wasm.wp.conseq _ (closureBodyCorrect childRuns)
    intro continuation terminal
    cases continuation <;> simp_all [TerminalPost]
  have physicalExecution :
      Wasm.wp module
...
/-- Lift count-one constructor refinement through the exact production
installation.  The postcondition is deliberately relational: it records the
concrete result and every invariant needed by the next recursive decrement,
without choosing a final store outside the Wasm execution. -/
theorem LiveHeapRel.wp_installedDecrementConstructor
    {host : Type} {sourceModule : Fir.Wasm.Module}
    {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
  ...
  · intros
    simp
  · intro opaqueTarget
    obtain ⟨result, finalStore, concreteOperation, finalRelated, finalMemory,
        finalCanonical, finalFrame, execution⟩ :=
      LiveHeapRel.decrementOnceProgram_constructor_refines
        ...
        (closureBody := closureDescriptorReleaseProgram installation.index
          witness.closureDescriptors.toList 0)
        (opaqueBody := opaqueTarget) related memoryRelated canonicalHeaders
          mapped found live ordinary one descriptor objectEq objectRelated
          headerRead headerKind refCount persistent withinLimit check checkWord
...
/-- Lift count-one closure refinement through the same installed production
body.  The descriptor table used by generation is definitionally the table in
the refinement witness, so no body certificate or table-coherence premise is
needed. -/
theorem LiveHeapRel.wp_installedDecrementClosure
    {host : Type} {sourceModule : Fir.Wasm.Module}
    {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
  ...
  · intro opaqueTarget
    obtain ⟨result, finalStore, concreteOperation, finalRelated, finalMemory,
        finalCanonical, finalFrame, execution⟩ :=
      LiveHeapRel.decrementOnceProgram_closureDescriptor_refines
        (decrementIndex := installation.index)
        (releaseHeaderIndex := installation.releaseHeaderIndex)
        (constructorBody := constructorReleaseProgram installation.index)
        (opaqueBody := opaqueTarget) related memoryRelated canonicalHeaders
          mapped found live ordinary one objectEq objectRelated headerRead
          headerKind descriptorLookup descriptorTableFits fixedCount extent
          refCount persistent check checkWord semanticOperation step releaseRun
    apply Wasm.wp.conseq _ execution
    intro continuation returned
    subst continuation
    exact ⟨result, finalStore,
...
/-- Complete fuel-indexed three-semantics refinement for a count-one resident
generic Array.  Only the logical live prefix participates in recursive
ownership; retained spare capacity is preserved solely by the mapped-payload
frame. -/
theorem LiveHeapRel.decrementOnceProgram_array_refines
    {host : Type} {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
    {runtime nextRuntime : Fir.LeanIR.Impure.RuntimeState}
    ...
    (releaseRun : Wasm.TerminatesWith env module releaseHeaderIndex store
      ...
      (fun final values =>
        final = releaseHeaderStore store (UInt32.ofNat address.value) ∧
          values = []))
    (opaqueBodyCorrect : ∀ {releasedStore finalStore : Wasm.Store host},
      ReleaseChildrenRun env module decrementIndex
          (UInt32.ofNat address.value) (List.range elements.size)
          releasedStore finalStore →
      Wasm.wp module opaqueBody
        (TerminalPost (fun continuation =>
          continuation = .Return finalStore [])) releasedStore
        (ownedEntry (UInt32.ofNat address.value) checkWord header.flags
          header.aux3 header.refCount header.kind.code header.aux0
    ...
    ∃ result finalStore,
      ...
      Wasm.wp module
        ...
        (decrementEntry (UInt32.ofNat address.value) checkWord) env := by
  obtain ⟨words, ownedRead, ownershipRelated, payloadRelated⟩ :=
    ResidentArrayObjectRel.readOwnedPayloadWords objectRelated related.frontier
  let localArray : LiveCellRel state witness address cell :=
  ...
  have parentPayloadFrame :=
    related.mappedPayloadFrame_of_headerWrite descriptor rawRead releasedEq
      headerInBounds headerWrite rfl
  have exact := canonicalHeaders mapped header objectRelated.headerRead
  have releasedMemory : ResidentMemoryRel released
      (releaseHeaderStore store (UInt32.ofNat address.value)).mem := by
    rw [releasedEq]
    simpa [releaseHeaderStore] using
  ...
  have notAboveOne : ¬1 < cell.rc := by omega
  have semanticFoldArray :
      Array.foldlM releaseChild parentRuntime elements = .ok nextRuntime := by
    simp only [Fir.LeanIR.Impure.decLocationFuel,
      Fir.LeanIR.Impure.getLiveCell, found, live, ↓reduceIte,
      Bind.bind, Except.bind] at semanticOperation
    rw [if_neg (by simp [ordinary])] at semanticOperation
    ...
    exact semanticOperation
  have semanticFoldList :
      elements.toList.foldlM (init := parentRuntime) releaseChild =
        .ok nextRuntime := by
    simpa only [Array.foldlM_toList] using semanticFoldArray
  obtain ⟨result, finalStore, concreteFold, finalRelated, finalMemory,
      finalCanonical, finalFrame, childRuns⟩ :=
  ...
  have notPromoted : header.isPromotedTag = false := by
    have different :
        (ObjectKind.opaque == ObjectKind.natural) = false := by decide
    simp [Header.isPromotedTag, objectRelated.headerKind, headerOrdinary,
      different]
  have headerNonzero : header.refCount ≠ 0 := by
    intro zero
    rw [zero] at refCount
  ...
  have ownedExecution :
      Wasm.wp module
        ...
        (ownedEntry (UInt32.ofNat address.value) checkWord header.flags
          header.aux3 header.refCount header.kind.code header.aux0
          header.aux1 header.aux2) env := by
    apply wp_ownedReleaseProgram_opaque
      (by simp [objectRelated.headerKind]; decide)
      (by simp [objectRelated.headerKind]; decide)
      (by simp [objectRelated.headerKind])
    apply Wasm.wp.conseq _ (opaqueBodyCorrect childRuns)
    intro continuation terminal
    cases continuation <;> simp_all [TerminalPost]
  have physicalExecution :
      Wasm.wp module
...
outcome, and the caller operand tail is unchanged. -/
theorem LiveHeapRel.terminatesWith_installedDecrementConstructor
    {host : Type} {sourceModule : Fir.Wasm.Module}
    {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
    ...
    Wasm.TerminatesWith env module installation.index store
      ...
      (fun final values =>
        ...
        values = tail) := by
  apply installation.terminatesWith_of_success_wp checkWord tail
  exact LiveHeapRel.wp_installedDecrementConstructor installation related
    memoryRelated canonicalHeaders mapped found live ordinary one descriptor
    objectEq objectRelated headerRead headerKind refCount persistent
    withinLimit check checkWord semanticOperation step releaseRun

/-- Public installed-call refinement for a count-one closure.  Static
descriptor admission and recursive capture release are hidden behind the same
relational postcondition used by constructors. -/
theorem LiveHeapRel.terminatesWith_installedDecrementClosure
    {host : Type} {sourceModule : Fir.Wasm.Module}
    {module : Wasm.Module} {env : Wasm.HostEnv host}
    {store : Wasm.Store host}
    {state : MemoryState} {witness : RefinementWitness}
    ...
    Wasm.TerminatesWith env module installation.index store
      ...
      (fun final values =>
        ...
        values = tail) := by
  apply installation.terminatesWith_of_success_wp checkWord tail
  exact LiveHeapRel.wp_installedDecrementClosure installation related
    memoryRelated canonicalHeaders mapped found live ordinary one objectEq
