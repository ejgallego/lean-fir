...
/-- Preserve a specified checked caller spine through frame reindexing instead
of existentially hiding it. Used by direct entry to expose its actual push. -/
theorem ConcreteStructuredValidatedStackAgreement.reindex
    {program : Fir.LeanIR.ImpureProgram}
    {sourceModule : Fir.Wasm.Module}
    {targetModule : AdaptedModule}
    {hosts : ResolvedHosts}
    ...
    (targetValidation : ConcreteStructuredSuspendedValidation program
      functionResult expectedResult targetSourceFrames) :
    ConcreteStructuredValidatedStackAgreement spine targetAgrees targetValidation := by
  subst targetSourceFrames
  subst targetTargetFrames
  exact aligned
...
the named callee only after the exact direct push and entry have been exposed. -/
theorem
  ...
  obtain ⟨targetAfter, targetPath, _calleeSpec, calleeResultAt, nextOutcome,
...
theorem ConcreteStructuredValidatedReturnedOutcome.advance_lazyCacheWithSpine_of_step
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {functionCode : Lean.Compiler.LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  | @case spine _ _ _ _ _ sourceFrames targetFrames belowStack targetRest
      ...
      obtain ⟨tailCount, targetAfter, tailPath, tailPositive,
          ...
          callerEntryRuntime, callerEntryStore, callerEntryWitness, callerResult,
          tailResult, callerFacts, callerEnv, callerLocals, declaration, result,
          continuation, callerJoins, sourceTail, restoredRest, targetTail, kind,
          cacheIndex, cacheSetId, resultIndex, runtimeAfter, publishedStore,
          tailCaseFrames, tailSpine, callerSpec, nextBind, nextChecked, spineEq,
          callerSourceFramesEq, sourceFramesAfter, targetFramesAfter,
      ...
      refine ⟨testCount + tailCount, targetAfter,
        unwindTarget.trans tailPath, by omega,
          ...
          callerSourceFramesEq, sourceFramesAfter, targetFramesAfter,
          ?_, sourceRuntimeAfter, operationEq, nextStoreEq, ?_⟩
      · rw [callerTargetFramesEq, List.append_assoc]
      · have caseLength :
            (structuredWasmCaseLabels belowStack targetRest testCount).length =
              testCount := by
  ...
  | @lazy spine activeEntryRuntime callerEntryRuntime activeEntryStore
      ...
      have nextChecked :
          ConcreteStructuredValidatedStackAgreement spine
            bindValidated.agrees bindValidated.frames.validation := by
        simpa only [bindValidated, validatedFrames, nextAgrees] using tailAligned
      exact ⟨7, targetAfter, targetPath, by omega,
        callerContext, callerCode, callerFunction, callerLabels,
        callerEntryRuntime, callerEntryStore, callerEntryWitness, callerResult,
        tailResult, callerFacts, callerEnv, callerLocals, declaration, result,
        continuation, callerJoins, sourceFrames, targetRest, targetFrames, kind,
        cacheIndex, cacheSetId, resultIndex, runtimeAfter, nextStore, [], spine,
        callerSpec, bindValidated, nextChecked, rfl, rfl,
        bindFocus.sourceFramesEq, bindFocus.targetFramesEq, rfl,
        bindFocus.sourceRuntimeEq, operationEq, rfl, rfl⟩

/-- Compatibility endpoint: hide the actual published bind only after the
same checked input spine and its popped caller tail have been exposed. -/
theorem ConcreteStructuredValidatedReturnedOutcome.advance_lazyCache_of_step
  ...
  obtain ⟨targetCount, targetAfter, targetPath, positive,
          _callerContext, _callerCode, _callerFunction, _callerLabels,
          _callerEntryRuntime, _callerEntryStore, _callerEntryWitness,
          _callerResult, _tailResult, _callerFacts, _callerEnv, _callerLocals,
          _declaration, _result, _continuation, _callerJoins, _sourceTail,
          _targetRest, _targetTail, _kind, _cacheIndex, _cacheSetId, _resultIndex,
          _runtimeAfter, _nextStore, _caseFrames, _tailSpine, _callerSpec,
          nextBind, _nextChecked, _spineEq, _callerSourceFramesEq,
          _sourceFramesAfter, _targetFramesAfter, _callerTargetFramesEq,
          _sourceRuntimeAfter, _operationEq, _nextStoreEq, _countEq⟩ :=
    related.advance_lazyCacheWithSpine_of_step aligned lazyCaller sourceStep
  exact ⟨targetCount, targetAfter, targetPath, positive,
    ConcreteStructuredValidatedCodeGlobalOutcomeAt.externalBind nextBind⟩
...
theorem ConcreteStructuredValidatedExternalBindOutcome.advance_code_of_step
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {functionCode : Lean.Compiler.LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
...
/-- Compatibility endpoint: hide the named active-code successor only after
exposing the exact destination bind, one-step path and saved-frame equations. -/
theorem ConcreteStructuredValidatedExternalBindOutcome.advance_of_step
    ...
    ∃ targetAfter,
      FinitePath (StructuredWasmStep targetModule.wasmModule hosts.env) 1
        ...
        ConcreteStructuredValidatedCodeGlobalOutcomeAt program sourceModule
          targetModule hosts externals witness sourceAfter targetAfter := by
  obtain ⟨targetAfter, _resumedLocals, targetPath, nextActive,
      _sourceFramesEq, _targetFramesEq⟩ :=
...
/-- Residual executable validation and the successful source selection derive
the complete production case family.

The object branch needs no caller-provided tag-range invariant: generated
object comparisons preserve the exact `UInt64` tag, while the concrete heap
refinement proves that every live `tobject` tag fits that representation.  The
validated discriminator mode, normalized alternative order, compiled local,
and static alternative bounds are therefore sufficient for all three case
families. -/
theorem ConcreteStructuredValidatedCodeCoreRel.productionCasesSupported_of_validation
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceModule : Fir.Wasm.Module}
    {sourceFunction : Fir.Wasm.Function}
...
/-- Outcome-level spelling of compiler-derived case admission. Suspended-frame
validation is irrelevant to classifying the current case node, so the proof
delegates to the residual validated code core. -/
theorem ConcreteStructuredValidatedCodeOutcome.productionCasesSupported_of_validation
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {functionCode : Lean.Compiler.LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
  ...
  related.core.productionCasesSupported_of_validation sourceResult

/-- One validated source case step has a finite concrete Wasm path and
preserves the closed validated relation.
...
theorem ConcreteStructuredValidatedCodeOutcome.advance_cases_of_validated_step
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {functionCode : Lean.Compiler.LCNF.Code .impure}
    {sourceModule : Fir.Wasm.Module}
...
/-- A validated current case node constructs its exact zero-allocation
admission directly from the successful source selection.

This is the case-family PA2 boundary: it has no source invariant, semantic
classifier, schema premise, target execution, or future-step certificate. -/
theorem ConcreteStructuredValidatedCodeCoreRel.admit_cases_of_validated_step
    {program : Fir.LeanIR.ImpureProgram}
    {context : Fir.Wasm.Context}
    {sourceModule : Fir.Wasm.Module}
    {sourceFunction : Fir.Wasm.Function}
    ...
    (sourceStep : executeStep externals source = .next sourceAfter) :
    ConcreteStructuredCodeStepAdmission context sourceModule externals
      functionResult facts sourceRuntime sourceEnv 0 (.cases cases) := by
  obtain ⟨selected, sourceResult, _sourceAfterEq⟩ :=
    related.core.focus.caseResult_of_step sourceStep
  rcases related.productionCasesSupported_of_validation sourceResult with
      defaultOnly | tested
  · exact .defaultOnlyCase defaultOnly
  · rcases tested with objectCases | scalarCases
    · exact .objectCases objectCases
    · exact .scalarUInt8Cases scalarCases
...
theorem ConcreteStructuredValidatedCodeCoreRel.admit_of_source_safe_step
  ...
  | cases =>
      exact ⟨0, related.admit_cases_of_validated_step sourceStep⟩
  | incPersistent =>
