...
| 2026-07-21 | A0 reusable `prettyM` client | integration, proof, and concrete-runtime owners | carded | `call-pretty-format.mjs` ignores the fixture invocation, starts with an empty semantic heap, instantiates the artifact once, and repeatedly passes caller-allocated raw Lean `Format` handles. Independent nested-line calls and the native-oracle all-constructor case pass. A longer standalone group exposed `FIR-BUG-impure-none-cached-heap-persistence`: both shared FIR caching and the V8 `cacheSet` mirror retain a heap constant as ordinary `rc = 1`, while generated persistent ownership operations require the cached cell to be persistent. The external-engine reproducer is checked as an expected failure; no host-only workaround or shared contract change was made. |
