...
`LazyCacheValidatorSound` is now discharged once for every successfully
...
refinements rather than executions stored in admission. Constructor-tag and
field mutations are now branches of that same relation. The tag case derives
the exact two-step unary prefix; FVar and erased object fields, `USize`, and
...
continuations remain admission-free under arbitrary join labels. Arbitrary
normalized object-constructor and scalar-`UInt8` dispatch are pointwise too.
The source step determines the selected arm; production compiler inversion
derives the tested prefix, which costs exactly five target steps per object
tag or four per scalar byte. Each executed test records one target-only case
layer in both the resource and supported stacks. Return-pop unwinds every such
...
resolved import, the imported-call state transports the full facts/cache/
closure resource scope while leaving the destination fact untouched, and the
generated `local.set` erases only the shadowed destination fact before
...
The split makes the proof-audit result enforceable in Lean: the compiler law
...
longer masquerade as source typing; they are consumed through the independent
finite-runtime safety law. The return
...
The first source-typing extraction distinguishes storage typing from use-site
typing. `SemanticBindingAtAbi` indexes a source environment binding by the ABI
required at the current use. This is intentionally more precise than the
compiler local kind: a coarse `.tobject` local does not by itself justify a
precise `.object` or `.tagged` return. The active-return theorem now consumes
that reusable binding judgment directly.

The same audit found that constructor descriptors need a relational component,
not merely a stronger predicate on `MachineState`. The existing
`ConcreteObjectFieldKindAligned` quantifies over arbitrary refinement
witnesses; `concreteObjectFieldKindAligned_not_of_sourceLocation_alone` proves
that a semantic heap location alone cannot establish it, because another
witness can attach a conflicting proof-only descriptor to the same location.
The correct next abstraction retains constructor-schema provenance in the
final-LCNF typing invariant and relates it to the active witness inside the
validated source/target relation. This is the sharpened obligation tracked by
