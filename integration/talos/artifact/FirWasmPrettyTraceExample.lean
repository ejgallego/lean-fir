...
run_cmd do
  ...
  let stackSafeArtifact ← match
    ...
    | .ok artifact => pure artifact
    | .error error =>
        throwError "failed to eliminate resident styled self-tail calls: {repr error}"
  unless stackSafeArtifact.module.imports.isEmpty &&
    ...
    throwError
      "resident styled tail-call elimination changed imports, source, or closure metadata"
