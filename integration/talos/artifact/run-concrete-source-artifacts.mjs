...
export async function checkConcreteStringInitialRuntimeGate(wasmPath) {
  const manifest = JSON.parse(await readFile(`${wasmPath}.json`, "utf8"));
  assert.equal(manifest.fixture, "Fir.Wasm.Emit.SourceFixture.acceptString");
  assert.throws(() => new ConcreteHost(manifest.imports, manifest.initialRuntime),
    /unsupported concrete initial-runtime heap object: string/,
    "string initial runtime unexpectedly crossed its concrete layout gate");
  console.log("PASS concrete initial-runtime string layout gate");
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  await checkConcreteStringInitialRuntimeGate(stringWasmPath);
}
