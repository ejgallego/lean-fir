...
run_cmd do
  ...
  let baseResult ← liftCoreM VersoFirHtml.Compile.compileBaseModule
  let base ← match baseResult with
    | .ok artifact => pure artifact
    | .error error => throwError "failed to compile Verso HTML source: {repr error}"
  match ← base.write "_build/prettyM-html-base.wasm" with
  | .ok () => pure ()
  | .error error => throwError "failed to write base HTML module: {repr error}"
  let artifact ← match VersoFirHtml.Compile.linkResidentRuntime base with
    | .ok artifact => pure artifact
    | .error error => throwError "failed to link HTML resident runtime: {repr error}"
  match ← artifact.write "_build/prettyM-html-resident.wasm" with
  | .error error => throwError "failed to write resident HTML module: {repr error}"
  | .ok () =>
      unless artifact.module.runtimeOperations.isEmpty do
        throwError "HTML module retained unresolved runtime operations"
      ...
      IO.FS.writeFile "_build/prettyM-html-resident.inventory.json"
        (inventory.pretty ++ "\n")
      logInfo m!"wrote {artifact.bytes.size} HTML bytes, {retainedSourceFunctions.size} source functions, and {residentHelpers.size} resident helpers"
