import IlluminateFirNative.SelectionCompile
...
run_cmd do
  ...
  let base ← match baseResult with
    ...
    | .error error => throwError "failed to compile Illuminate selection player: {repr error}"
  match ← base.write "_build/illuminate-selection-player-base.wasm" with
  | .ok () => pure ()
  ...
  | .ok () =>
      unless artifact.module.initializers.isEmpty do
        throwError "selection module retained lazy-cache globals"
      unless artifact.module.globals.size == 1 do
        throwError "selection module must retain only the allocator frontier global"
      unless artifact.module.runtimeOperations.isEmpty do
        throwError "selection module retained unresolved runtime operations"
      ...
      IO.FS.writeFile
        "_build/illuminate-selection-player-resident.wasm.inventory.json"
        (inventory.pretty ++ "\n")
  ...
  | .error error => throwError "failed to write selection player: {repr error}"
