...
run_cmd do
  ...
  let baseResult ← liftCoreM IlluminateFirSpatialHitScene.Compile.compileBaseModule
  let base ← match baseResult with
    | .ok artifact => pure artifact
    | .error error => throwError "failed to compile HitScene source: {repr error}"
  match ← base.write "_build/illuminate-spatial-hit-scene-base.wasm" with
  | .ok () => pure ()
  ...
  match ← frontier.write "_build/illuminate-spatial-hit-scene-frontier.wasm" with
  ...
  | .ok () =>
      ...
      IO.FS.writeFile "_build/illuminate-spatial-hit-scene-frontier.inventory.json"
        (inventory.pretty ++ "\n")
