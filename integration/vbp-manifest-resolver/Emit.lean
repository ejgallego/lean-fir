...
run_cmd do
  ...
  let base ← match ← liftCoreM FirVbpManifestResolver.Compile.compileBaseModule with
    | .ok artifact => pure artifact
    | .error error => throwError "resolver base compilation failed: {repr error}"
  match ← base.write "_build/vbp-manifest-resolver-base.wasm" with
  | .ok () => pure ()
  | .error error => throwError "resolver base write failed: {repr error}"
  let artifact ←
    match FirVbpManifestResolver.Compile.linkResidentRuntime base with
    | .ok artifact => pure artifact
    | .error error => throwError "resolver resident link failed: {repr error}"
  match ← artifact.write "_build/vbp-manifest-resolver.wasm" with
  | .error error => throwError "resolver resident write failed: {repr error}"
  | .ok () =>
      unless artifact.module.runtimeOperations.isEmpty do
        throwError "resolver retained unresolved runtime operations"
      unless artifact.module.imports.isEmpty do
        throwError "resolver retained Wasm imports"
      ...
      IO.FS.writeFile "_build/vbp-manifest-resolver.inventory.json"
        (inventory.pretty ++ "\n")
      logInfo m!"wrote {artifact.bytes.size} resolver bytes, {retainedSourceFunctions.size} source functions, and {residentHelpers.size} resident helpers"
