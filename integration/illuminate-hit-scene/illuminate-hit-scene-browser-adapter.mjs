...
const SCENE = Symbol("fir.illuminate-hit-scene.scene");
...
function fail(message, options) {
  throw new Error(`FIR Illuminate HitScene adapter: ${message}`, options);
}
...
export async function fetchIlluminateHitSceneAdapter(artifactUrl, {
  ...
  requireCondition(typeof fetchImpl === "function",
    "fetchIlluminateHitSceneAdapter requires fetch");
  ...
  const adapter = await createIlluminateHitSceneAdapter({
