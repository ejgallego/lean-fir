...
generated_scalar_array_c="$out_dir/WasiScalarSmoke.c"
...
native_scalar_array_generated_o="$out_dir/WasiScalarSmoke.native.o"
native_scalar_array_host_o="$out_dir/native_wasi_scalar.o"
native_scalar_array_executable="$out_dir/WasiScalarSmoke.native"
...
for symbol in \
  ...
  lean_mk_empty_byte_array; do
  if ! grep -q "$symbol" "$generated_scalar_array_c"; then
    echo "WASI scalar-array fixture no longer references $symbol" >&2
    exit 1
  fi
done
...
lake env leanc \
  ...
  -ffp-contract=off \
  -c "$generated_scalar_array_c" \
  -o "$native_scalar_array_generated_o"
cc \
  -O3 \
  -DNDEBUG \
  -fomit-frame-pointer \
...
lake env leanc \
  ...
  "$native_scalar_array_host_o" \
  -o "$native_scalar_array_executable"
"$native_scalar_array_executable" > "$native_scalar_array_results"
...
link_flags=(
  ...
  "-Wl,--export=fir_lcnf_c_wasi_scalar_checksum"
  ...
  "-Wl,--export=fir_lcnf_c_wasi_scalar_array_deallocations"
