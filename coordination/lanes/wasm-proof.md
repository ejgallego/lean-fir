...
bug-cards: none
blockers: none
handoff: none; the singleton object-case slice landed on main at 0a95c890 and the lane is active on ordered case chains.
next: Generalize the fixed dispatcher proof to ordered object-constructor chains, starting with two arms plus a default. Reuse the exact five-step head-test boundary for a hit and prove the false-branch continuation boundary for a miss. Then connect tagged scalar/UInt8 chains through the same recursive relation.
```
