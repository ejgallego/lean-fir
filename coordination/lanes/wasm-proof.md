...
bug-cards: none
blockers: none
handoff: none; the ordinary increment slice landed on main at c025081e and the lane is active on ordinary decrement.
next: Add successful ordinary decrement using the same exact unary-host prefix boundary and its recursive release/transport theorem. First factor the shared same-witness heap-effect reconstruction for the complete entry-relative cache frame. Keep delete, heap-valued lazy publication, and saturated closure calls as separate slices.
```
