...
checks: No system /tmp input was used; the exact comparison lives under ignored integration/lean-zip/.deps/experiments/closure-dispatch-v2. Lean Beam PASS with zero diagnostics for Lower, WellFormed, Source, and SourceExamples. After rebasing on 63204d2e, the combined SourceExamples/ResidentLinker dependency cone PASS, including the accepted generic call-site and sparse constructor paths. git diff --check PASS and make check PASS with 713 unique cases and 2121/2121 comparisons. make talos-setup PASS at pinned Talos 0e05edbc. make talos-check again reports exactly one failure, the W6-owned ConcreteClosureDispatch theorem at line 450, because its candidatesEq premise is hard-coded to context.program.decls rather than the new context-selected declarations; every other reported target passes
