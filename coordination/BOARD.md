...
| `LCNF-TOOLCHAIN-MODULE-INPUTS` | integration/W7 | VBP preview source capture, future installed-Lean owners | active | decision `ROOT-W7-20260915-020`; bootstrap-input clarification `ROOT-W7-20260915-021` | Supplies a faithful, explicit compiler input for an installed Lean module only when its exact source, toolchain revision, module identity, imports/options and compiler-artifact provenance are recoverable from the pinned toolchain. Releases omit original bootstrap setup/options, so the provider may instead derive a **new capture input** from the exact pinned upstream bootstrap template, CMake stage options and recorded release module artifacts; that provenance must remain labeled derived, never recovered release metadata. It rejects unavailable, inconsistent or ambiguous records; it does not synthesize a Lake setup, import a target into itself, seed generated declarations, alter the selected toolchain, or change importer/reset/source-unit policy. Initial scope is exactly the five owners of the seven renderer-frontier gaps; successful input recovery enables a later separately leased recursive capture, not lowering, linking, Wasm execution, ABI/profile admission, or W6 work. |
