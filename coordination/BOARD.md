...
| `LCNF-OWNING-MODULE-CAPTURE` | W7/integration | VBP renderer probe, future ordinary-source capture callers | active | decision `W7-ROOT-20260915-014`; W7 lease follows root review | Replaces the failing fresh per-root final-LCNF reconstruction only by capturing through the target owning ordinary module's original header and command-frontend context. The adapter retains upstream declaration groups, target-private/import visibility, generated-specialization context and final-LCNF recorder semantics. It does not erase/reseed compiler mappings, import a target into itself, inject prebuilt readers, change importer or source-unit policy, migrate to IR, weaken checks, or change semantic/ABI/layout/runtime/W6 contracts. The first acceptance is the actual `Verso.Doc` shared-specialization/`Renderer.render` frontier; broader module coverage is separately decided. |
