...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through the conservative nullary policy graph and strict compiler-client correctness endpoint `5322ccf`; the isolated `FLOAT-SCALAR-RUNTIME` producer remains ready at `c901e0d`, and its validation stack is rebased on current `main` through the carded tagged-float repair and 655-case checkpoint `bcbc1a9` | The validation stack exposes one exact pass-proof adaptation plus the already-recorded W6/W7 exhaustive consumers; all must be green before the shared contract and dependent fixtures land on `main` |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | `NullarySafeShadowCodeRun` now proves an actual transparent run while rejecting precisely deleted nullary `.fap`, and `ElimDeadCompilerAdmissibleRun.loweringCorrect` combines that policy with the independent runtime/ownership certificate; positive and counterexample-rejection fixtures are landed through `5322ccf`, and the branch is aligned with `main` | No shared contract changed. The next proof slice derives the conservative policy from compiler provenance and states the ownership-to-runtime-admissibility bridge. The queued float-contract adaptation remains to extend `scalarFromType_ok_eq_immediate` with `float32` and `float` after rebasing onto validation contract commit `62de0d4` |
