...
- W6 checkpoint `f72fb1a9` is landed on functional `main`. The integration
  worktree independently passes `git diff --check`, the complete `make check`
  gate (633 native/LCNF cases, 9 direct cases, 601 V8 cases, 1,844/1,844
  comparisons), and `make talos-setup && make talos-check` (3,123 jobs). The
  W6 branch is clean and now one landed proof commit behind functional `main`. W7 remains
  clean at `7638f71f`, twenty-one commits ahead and one hundred thirty-eight
  behind functional `main`; its latest checkpoint executes exact floats through
  the manifest facade. Integration must not absorb W7 until its owner rebases,
  runs the required full/Talos/artifact checks, and sends the prescribed
  handoff.
...
| Validation | validation owner | `validation/float-corpus` | active | Clean coordination head `cfa17d81`, with validated functional head `96eec154` on semantic base `fff91175`, is one hundred ninety-four commits behind and one hundred twelve ahead of functional `main` at `40aebeeb`. Its 1,008-case native/LCNF run and 9-case direct-machine run verify offline with zero findings; the composed long-stack index retains 1,017 comparisons, 9,939 interpreter steps, 193 semantic-tag floors, and 152 semantic domains. Current-main extractions through `c05e85d9` independently land retained-corpus attribution, immutable evidence/attestations, Bool plus unsigned/signed entry ABIs, and the signed result ABI. The current root gate covers 633 native/LCNF cases, 601 V8 cases, 642 unique cases, 1,844 comparisons, 5,750 interpreter transitions, 51 semantic-tag floors, and 142 conjunctive domains. | The next long-branch rebase drops duplicated mechanisms from `a2907a66`, `7c87e6ec`, `bb387042`, `9171fdd6`, and `cb03e9ab`, already superseded on `main`; it retains long-only Float/IO domains as an additive post-contract calibration. Shared float, closure, alias, termination, IO, and stream-capture contracts remain isolated and require the recorded proof/W6/W7 handoffs before integration. |
