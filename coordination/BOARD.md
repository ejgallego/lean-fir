...
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, FIR lower/link | active | primitives `422caea9`, handoff `690e7940`; provider linking `fd77cba8`; official routing `ROOT-W7-20260915-036` | One outcome-driven investigation establishes whether real VBP `Renderer.render` compiles through FIR using its faithful upstream Lean/Lake module contexts. It follows one deterministic owner worklist from the renderer through complete source closure, FIR lowering, and a checked artifact/import frontier; an external native/VIR requirement remains explicit rather than fabricated. Each owner retains exact source/setup/plugin/toolchain/body/interface identity and the accepted worker boundary prevents conflicting native images from coexisting. Accepted generic `@[extern]` → exact upstream `@[export]` linkage closes 16 edges without custom String helpers; existing arbitrary-limb natural allocation and exact `UInt64.ofNatLT` coverage close both corresponding imports, leaving 15 imports and zero runtime operations in the checked renderer lowering. Remaining native String primitives and the JS boundary remain explicit. The next full renderer validation uses a W7-owned child of official `tooling/lean-4.34`; no support may change layout, helper signature, ownership/effect rule, host import, or consumer. It reports concise traversal progress and stops only for a genuine shared semantic/ABI contract, consumer-owned change, unrepresentable upstream boundary, or the final result. No reset, generated-name seeding, weakened identity check, plugin suppression, durable/untrusted transport, W6 work, or implicit runtime/consumer change is authorized. |
