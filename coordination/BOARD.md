...
- Integration owner: `root`, holding the serial landing lease through accepted
  integration head `4fca13b0`. `wasm-gen` remains released to the W7-1
...
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | released | Planning checkpoint `4fca13b0`, functional head `0f2b8f0f`, aligns W6, backward-pass composition, and provenance around PA0--PA4. PA0 audits guarded schema-aware source admission; PA3 removes the arbitrary source-invariant premise before another pass hop. | Documentation only; no semantic, ABI, layout, lowering, proof, or generated-code contract changed. The 730-case/2,172-comparison root gate and 3,189-job Talos gate pass. |
...
|---|---|---|---|---|---|
| `WASM-NORMALIZED-CASE-TABLE-ADMISSION` | W6/integration | W6, W7, validation | active | isolated contract `bd43f7f0`; proof consumer pending | Requires production-accepted case tables to contain a constructor prefix followed by at most one final default, matching the source interpreter's first-match semantics and the existing structured case simulation. The former default-before-constructor example is retained as a fail-closed whole-program regression. W6 derives `ConcreteStructuredCaseAltsNormalized` directly from residual validation and removes it from the source-safety premise. Existing compiler-produced final LCNF is expected to be normalized; W7 and validation rebase after the green proof stack lands and report any rejected artifact. No lowering instruction, runtime, ABI, layout, helper signature, ownership rule, or emitted code changes for previously accepted normalized programs. |
