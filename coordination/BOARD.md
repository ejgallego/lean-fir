...
|---|---|---|---|---|---|
| Integration | integration owner | `integration/wasm-lanes` | active | Integrated W7's closed resident `prettyM` runtime through `071fcbc`, the complete exact signed validation family as `5305358`, and whole-program exact/runtime-ownership pass proofs through `361f95e`; the isolated `FLOAT-SCALAR-RUNTIME` producer is ready at `c901e0d`, with its rebased validation stack ready through `67c6532` | The root Lean and pass-proof cones are green; W6 and W7 must adapt the exact exhaustive consumers recorded below before the float contract and dependent validation stack can land on green `main` |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Complete exact machine dispatch plus closed whole-program proofs for writes, failed/concrete/owned-child reset-reuse, PAP closure allocation, and heap-backed boxing are landed through `361f95e`; branch and `main` are aligned and green | No shared contract change. The remaining general theorem needs an explicit source runtime/ownership and purity contract; compiler well-formedness alone is insufficient because of the registered nullary-`.fap` discrepancy |
...
| Validation | validation owner | `validation/float-corpus` | active | The rebased float stack is ready through `67c6532`: `5e08833` preserves ten exact identity patterns and `67c6532` adds 51 width-polymorphic arithmetic, comparison, classification, bit-reinterpretation, and conversion cases plus 28 duplicate-checked LCNF handlers. All 642 source cases compare native == LCNF with zero findings in evidence run `d57481b901a4f8de8417a3d17915d7e1eccbeef2eaba9a45d4c449919e862309` | Consumes `FLOAT-SCALAR-RUNTIME` without modifying W6/W7-owned files. The branch is ready to rebase after both consumer adaptations land; only then does validation activate these cases in compiler-generated Wasm/V8 with raw-bit transport |
...
| `FLOAT-SCALAR-RUNTIME` | integration/validation | pass proof, W6, W7, validation | active | `c901e0d` | Adds bit-exact `float32Bits`/`float64Bits`, typed validation schemas, and LCNF codecs. Root Lean, source examples, pass proofs, 642 native/LCNF cases, and nine direct-machine cases pass on dependent validation head `67c6532`. W7 must extend exhaustive matches in `Fir/Wasm/Emit/Manifest.lean` at lines 149, 255, and 280; W6 must extend `Fir/Wasm/Concrete/HeapRefinement.lean` at lines 35, 116, and 136. `make check` reaches V8 and fails only at the three W7 sites; producer-side `make talos-check` additionally identifies only the three W6 sites. Once both adaptations rebase onto the contract stack, validation can land its native-oracle corpus and add compiler-generated Wasm/V8 raw-bit transport, boxing, and field-mutation cases. |
