...
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, later FIR lower/link | active | decision `ROOT-W7-20260915-022` | Resolves the real `Renderer.render` source closure through a deterministic worklist of recorded defining modules. Each owner is captured exactly once through the Lake or released derived-input provider; a signature becomes local only after exact signature agreement, while duplicate bodies, unknown/ambiguous provenance, unavailable inputs, or new compiler diagnostics stop the run. It retains module boundaries and never falls back to fresh root reset or generated-name seeding. Initial output is a source-only closure product and remaining classified boundaries; lowering, linking, Wasm/profile admission, ABI/runtime/W6 and consumer changes remain separately authorized. |
