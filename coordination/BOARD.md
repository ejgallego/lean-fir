...
| `VBP-VIR-SESSION-HOST-PROTOTYPE` | W7/integration | VIR host semantics, VBP preview acceptance | released | decision `ROOT-W7-20260915-040`; functional `4163e27a`; immutable handoff `0c2eaa62` on official 4.34 child | The bounded bridge executes the linked renderer with exactly twelve signature-checked VIR imports, module-owned memory, explicit String/Bool/Float/IO conversions, and a strong per-session JS table. Its typed `Callback.ofUnary` trampoline keeps the transferred Lean closure live and borrows it on later calls; disposal after consumer unmount rejects new calls/callbacks and clears adapter-held references. The Node checks exercise all imports, Unicode/NUL, negative zero, retained callbacks across allocation/render, independent sessions, missing bindings, signature rejection, host-failure poisoning and repeated disposal. It deliberately exposes only fixture-built structured Documents and recording React doubles: genuine VBP SSR/browser behavior, one-root DOM identity, arbitrary typed Document/Options transport, and the full consumer acceptance campaign remain separate. Session growth remains accepted prototype debt. There is no main migration, W6/VIR/VBP source edit, generalized host ABI, or unreviewed runtime claim. |
