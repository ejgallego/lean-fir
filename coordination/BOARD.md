...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through structured symbolic Wasm loops, Talos lowering, and ledger-aware partial-application matching at `e3bb2ba`. Validation head `34c33b4` is rebased onto the earlier Wasm boundary and advances the ready stack to 671 native/LCNF cases with typed process supervision and a real divergence witness | Four isolated shared contracts are ready but not yet eligible as one stack for `main`: the older float/closure/argument-alias consumers and the native-termination consumer boundary below must be acknowledged. Dependent validation-only effect, mutation, panic, and divergence fixtures add no proof-lane work |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Ledger-aware partial-application matching is integrated at `e3bb2ba` after the literal and constructor checkpoints; the rebased suite has 590 unique cases, 1752/1752 equal comparisons, 4790 interpreter steps, zero findings, and exactly one registered trusted axiom | Continue with boxes, failed-token reuse, existing-address, and external-response branches, then assemble the unified ledger-aware dispatcher. Afterwards define the entry-indexed exact ownership invariant, adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172`, and prove the queued `takeClosureApplication` consumers |
