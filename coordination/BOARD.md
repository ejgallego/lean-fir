...
| `VBP-RENDERER-DOCUMENT-JSON-ENTRY` | W7/integration | designated VIR widget-backend consumer owner | released | functional `2d6aac03`; immutable producer handoff `a48bbb4a` on official 4.34 child | The renderer-only, synchronous `renderDocumentJson(string)` entry compiles the existing `VersoBlueprint.Experimental.VirPreview.Document.decode`, calls actual `Renderer.render` with default options, and returns the genuine React node. It has no JS schema copy, general Document/Options transport, Preview/RPC wire-format migration, or options transport. Two native-encoded documents render; four malformed JSON/Document inputs visibly reject; ordinary provider-error recovery, retained callbacks, and later rendering remain live. The release has 60 modules/2,186 bodies/18 native-provider links, exactly the same twelve deliberate typed VIR imports, module-owned memory, and no native/runtime-op import. The shared owner independently confirmed the predecessor’s real React SSR/Chromium error recovery and has full-FLT `Document.encode` input captures (6,598,303/6,598,333 bytes) plus a frozen VIR control matching `c4430bfe`; it now owns rich-document/extension/details and matched-renderer acceptance of this exact new artifact. This remains a producer release, not full-document/browser acceptance, timing evidence, or an optimization assignment. |
