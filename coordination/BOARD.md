...
- Validation's validated pre-record coordination head is `3ae6c37d`, with
  ...
  `6ab2efed`; the combined proof/validation head is `3723e145`, so the long
...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Landed head `3723e145` adds a current-state source-owned exact invariant relation and checked-pass contract. Client predicates preserve only static/source-target facts; readiness may consume the concrete source heap/environment/frame carrier maintained by the operational proof. The complete allocation-plus-three-writes fixture reaches `LoweringCorrect` through the new endpoint. | Migrate an ownership-sensitive readiness proof so it genuinely consumes the maintained carrier, then use that interface to replace finite fixture enumeration with auditable static compiler typing/ownership preservation. After lane 4 rebases its shared contracts, also re-probe the focused `SimpCaseRelation` and `AlphaEqvCode` consumers rather than assuming their historical locations remain exact. |
