...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through closed reset/PAP/box proof checkpoint `6104100`; the isolated `FLOAT-SCALAR-RUNTIME` producer remains ready at `c901e0d`, and its validation stack is rebased on current `main` through the carded tagged-float repair and 655-case checkpoint `bcbc1a9` | The validation stack exposes one exact pass-proof adaptation plus the already-recorded W6/W7 exhaustive consumers; all must be green before the shared contract and dependent fixtures land on `main` |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Complete exact machine dispatch plus closed whole-program proofs for writes, failed/concrete/owned-child reset-reuse, PAP closure allocation, and heap-backed boxing are landed through `6104100`; a new proof slice is in progress | After rebasing onto validation contract commit `62de0d4`, extend `scalarFromType_ok_eq_immediate` in `Fir/LeanIR/Passes/ElimDeadRuntimeRel.lean` with `float32` and `float` scalar cases; its focused dependency cone reports this single unsolved goal at line 4120 |
