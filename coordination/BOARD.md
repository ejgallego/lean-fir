...
- W7's float-only integration candidate is `2b4d9d23` on base `8a8d1387`.
  It deliberately excludes queued closure commits `db02897c` and `9173e1e9`:
...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | This landing advances the green shared float runtime/proof base `8a8d1387` through the accepted W7 functional head `2b4d9d23` and this coordination snapshot. The root gate covers 633 native/LCNF cases, 9 direct cases, and 601 native/LCNF/V8 cases: 642 unique cases and 1,844 equal comparisons. | Float representation, reinterpretation, manifest transport, and the zero-import package are released. Seven isolated shared-contract domains remain queued: closure ownership, argument aliases, process termination, effectful native-oracle actions, source-entry result boundaries, Lean IO-error normalization, and native source-stream capture. Closure commits `db02897c`/`9173e1e9` remain outside `main` until pass-proof and W6 adaptations are green. |
| Lean pass proof | pass-proof owner | `proof/simpcase` | blocked | Clean checkpoint `ae995ba8` contains the float-box proof adaptation now landed on `main` through `8a8d1387`. The source-owned/ledger simulation remains checked through `LoweringCorrect`; the integrated float stack passes all required gates. | Deliberately stopped; resume only on explicit direction. The next shared-contract task is the queued closure-application adaptation exposed by `AlphaEqvCode`; general reset/reuse proof work remains separate. |
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | released | Clean handoff `8a8d1387` on base `ae995ba8` is landed on `main`. Float boxes, packed scalar storage/projection/mutation, resolver support, concrete refinement, raw-bit normalization, and reuse-capacity proof adaptation pass all 3,123 Talos jobs. | Float runtime work is complete. Closure-application refinement remains a separate W6-owned queue item and is not part of the landed float stack. |
| W7 generation | generation owner | `wasm/generation` | released | Accepted functional head `2b4d9d23` on base `8a8d1387` contains seven W7 float commits plus one split-boundary test correction. It executes exact Float32/Float64 arguments and results through the integer-lane facade and publishes the zero-import `prettyM` package. The original branch remains clean at `c0a18a22`. | The accepted slice passes `make check`, all 3,130 Talos jobs, and the full artifact/browser/native-oracle gate. Integration excludes closure commits `db02897c`/`9173e1e9`; rebase the original W7 branch onto the new `main` before its next slice. |
...
| `CLOSURE-APPLICATION-OWNERSHIP` | integration/validation | pass proof, W6, W7, validation | active | current rebased contract `db02897c`; W7 adapter transfer `9173e1e9` | Matches Lean's `lean_apply_*` boundary: an exclusive closure transfers fixed arguments and is freed non-recursively; a shared closure drops one reference and retains each fixed heap argument. The current unsplit W7 branch demonstrates the executable behavior but fails pass-proof `AlphaEqvCode` cases and W6/Talos `Correctness.Semantics` frame cases. Keep both commits out of `main` until the pass-proof and W6 owners provide separate green adaptations. |
