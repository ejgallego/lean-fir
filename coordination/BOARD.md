...
| `LCNF-OWNING-MODULE-CAPTURE` | W7/integration | VBP renderer probe, future ordinary-source capture callers | active | capture `ca0b0a40`, handoff `c57742d`; next first dependency-module assembly | Replaces the failing fresh per-root final-LCNF reconstruction only by capturing through the target owning ordinary module's original header and command-frontend context. The adapter retains upstream declaration groups, target-private/import visibility, generated-specialization context and final-LCNF recorder semantics. It does not erase/reseed compiler mappings, import a target into itself, inject prebuilt readers, change importer or source-unit policy, migrate to IR, weaken checks, or change semantic/ABI/layout/runtime/W6 contracts. The first acceptance captures actual `Verso.Doc` shared specialization and the `Renderer.render` 151-local/41-signature frontier; dependency modules are assembled one defining module at a time under later root leases. |
