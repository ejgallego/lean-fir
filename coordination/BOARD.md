...
| `VBP-VIR-SESSION-HOST-PROTOTYPE` | W7/integration | VIR host semantics, VBP preview acceptance | active | initial functional `4163e27a`; handoff `0c2eaa62`; error-alignment lease `ROOT-W7-20260915-042` on official 4.34 child | The bounded bridge executes the linked renderer with exactly twelve signature-checked VIR imports, module-owned memory, explicit String/Bool/Float/IO conversions, and a strong per-session JS table. Its typed `Callback.ofUnary` trampoline keeps the transferred Lean closure live and borrows it on later calls; explicit disposal after consumer unmount rejects new calls/callbacks and clears adapter-held references. The initial Node checks cover all imports, Unicode/NUL, negative zero, retained callbacks across allocation/render, independent sessions, missing bindings, signature rejection and repeated disposal. A user-provided VIR raw-host policy input (`03ffaa13`, draft PR189, not landed) found one material mismatch: an **ordinary provider exception** must produce Lean `IO.Result.error`, preserve original JS exception identity for the owning export/callback boundary, and leave the session plus escaped callbacks live. Only genuine Wasm/ABI corruption may poison the session. W7 is aligning that focused behavior and nested/reentrant checks; this does not adopt the draft as a permanent ABI or alter pins. The prototype still uses fixture-built structured Documents and recording React doubles: arbitrary typed Document/Options transport, genuine SSR/browser checks, one-root DOM identity and the full consumer campaign remain separately scoped. Session growth remains accepted prototype debt; no main migration, W6/VIR/VBP source edit or generalized host framework is included. |
