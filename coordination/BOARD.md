...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | active | request `ROOT-W7-20260916-009`; VBP request `VBP-FIR-20260916-OPTIONS-001` | Assesses whether VBP's new once-per-runtime `createEncodedDocumentComponent (mathComponent? := none)` can be captured through the current typed FIR adapter, preserving native `Document.encode` String input and session-owned controls/identity/content. The first slice is an import/source-identity and feasibility inventory only. It reads the current graph rather than a base archive but may rely only on the supplied immutable `Preview/Component.lean` / `Component/Shell.lean` identities until VBP supplies a complete closure snapshot or explicitly authorizes the other mutable inputs. It stops at that provenance boundary, or—if an authorized package runs—at the first missing generic primitive or typed import. Optional native math-handle support follows only after `none` is feasible. No renderer fork, adapter/ABI/options-schema/transport change, source/pin mutation, consumer acceptance, timing result, main migration, W6 work, or change to preserved `4776f86e`/`a48bbb4a` artifacts. |
