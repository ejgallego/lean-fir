...
| `USIZE-BOXING-UPSTREAM-ALIGNMENT` | integration/W7 | semantic LCNF runtime, LCNF ElimDead proof, W6 concrete layout/proofs, W7 resident scalar boxing, validation fixtures | active | refreshed base `e2c2f8c4`; bug card `9841a7a1`; semantic/fixture candidate `edaea293`; proof-boundary record `afeb9d11`; operational threads `W7-LCNF-20260826-021` and `W7-W6-20260826-026` require exact-head refresh after their current checkpoints | Aligns FIR with upstream Lean's unconditional `lean_box_usize` allocation and constructor-only `lean_unbox_usize`. Small and maximal source-generated boxes follow the same owned heap-release trace. The semantic runtime and provider-backed native/LCNF/V8 triangle are green. Talos isolates exactly two stale `ElimDeadRuntimeRel` equivalence sites and three stale `BoxingCorrectness` policy/unbox arguments; LCNF and W6 adapt those surfaces before W7 adds zero-import `.box/.unbox .usize` helpers. Other scalar policies, layout constants, semantic ABI signatures, and symbolic Wasm remain unchanged. |
