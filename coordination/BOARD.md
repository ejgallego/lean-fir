...
| `VBP-VIR-SESSION-HOST-PROTOTYPE` | W7/integration | VIR host semantics, VBP preview acceptance | active | decision `ROOT-W7-20260915-040`, VBP acceptance `VBP-FIR-20260915-001`, source-to-Wasm base `655ee202` | Implements the smallest executable host for the linked renderer's twelve typed VIR imports: one Wasm instance and a strong JS value table per preview session, typed String/Bool/Float and IO-result conversion, plus a synchronous typed callback trampoline whose escaped Lean closures are retained for the session. Session growth/leaks are intentional prototype debt; dispose after React root unmount invalidates callbacks and releases adapter-held instance/table references, without claiming immediate reclamation of outside JS references. Traps are visible and invalidate an unjustifiable continuation. Acceptance invokes actual typed `Renderer.render`, checks the existing SSR renderer cases, retains one React root across unchanged/changed document updates, tests a repeated callback after another render, then rejects callbacks/new calls after unmount-and-dispose. No hooks/effects/RPC, per-render rewind, recycling, finalizers, per-node tracking, untyped/general host framework, permanent cross-backend ABI, main migration, W6/VIR/VBP source edit, or new consumer contract is included. Consultations inform validation but do not block the bounded prototype. |
