...
|---|---|---|---|---|---|
| `NATIVE-FLOAT-TOSTRING-PROVIDER` | W7 prototype/root contract owner | callback-rooted VBP options component; future standard-runtime consumers | active | progress `W7-ROOT-20260916-016`; decision `ROOT-W7-20260916-018`; no clean provider checkpoint | SDK roots/nine compiled callback signatures and accepted renderer controls pass; component linking now stops at opaque `Float.toString`. Root chooses a thin native provider faithful to Lean `6a10ac8c` runtime `object.cpp` (NaN normalization and `std::to_string`), with an explicit existing-layout FIR String/allocator bridge and native byte-for-byte differential controls. Private feasibility remains within the probe/.deps lease; W7 also has a narrow lease for new Float-format source/contract/build/test files in `integration/wasm-runtime/`, naming files in its next meaningful status. Reusable capability/String linkage must be isolated for root acceptance before production dependency; no silent `fir.standard-libm/v2` expansion or broad shared-linker lease. Preserve exact intentional VIR imports, callback exports, memory/ownership and source authority. No JS formatting fallback, removed debug branch, full Lean runtime, W6/consumer source edit, pin change, component/browser acceptance, migration or publication is claimed. Continue the same options execution objective; actual shared signatures/layout/helper changes escalate. |
...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | active | investigation `f055e0ff`, report `a32f8b58`; source/setup `2c1ef0f6`/`6bbd2b32`; continuing request `ROOT-W7-20260916-017` | Source-preserving official Lake `--packages` mapping and active Git mutation guard pass before capture; all frozen source bytes remain intact. Real `createEncodedDocumentComponent none` captures/assembles/lowers/links. Root accepts this investigation via `ROOT-W7-20260916-016`, independently verifying all 404 evidence payload hashes/sizes/modes and actual factory Wasm. The 64,858-byte factory has one `Js.Function.ofLean` import and no callback invocation export; it is not component execution. Full captured host frontier: 31 rows, 19 added/one removed/12 unchanged against `4776f86e`, no signature/borrow changes. W7 now owns one continuing additive SDK/fixture slice under `integration/vbp-native-session-probe/` through generic typed callbacks and explicit invocation/reachability roots to actual `none` React execution, with compiled conversion/error/retention/disposal and focused lifecycle checks. No approval per ordinary diagnostic; actual shared ABI/helper/compiler changes or new source/consumer authority escalate to root. Consumer retains full parity/campaign ownership and received `FIR-VBP-20260916-002`. Optional math, publication, timings, optimization, main migration, W6/VIR/VBP source edits and all live pins remain out of scope; `4776f86e`/`a48bbb4a` outputs unchanged. Four known official-child 4.34 audit findings remain, with no full green/Talos claim. |
