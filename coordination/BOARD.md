...
- Validation's validated pre-record coordination head is `3ae6c37d`, with
  ...
  `8ea9f75b`; the combined proof/validation head is `3c071252`, so the long
  validation branch is forty-eight commits behind and one hundred twelve
...
- Main checkpoint `41f40efd` independently passes its complete 581-case
  ...
  frontier, not the total real-engine baseline. Functional `main` at `3c071252`
...
- W6 has advanced its clean committed head to `13958eb4`, one hundred
  thirty-four commits ahead and three behind functional `main`; no formal
  integration handoff has been published. W7 is clean at `e5c67f54`,
  twenty-two commits ahead and thirty-three behind functional `main`; its exact
...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Functional proof head `3c071252` is on `main`, and the clean proof worktree is aligned. The combined source-owned/ledger simulation remains checked through `LoweringCorrect`. The target side of the retained-prefix contract uses a step-preserved three-phase allocation/control invariant rather than any finite target execution graph. Its post-allocation facts now project through the program-independent `TargetSingletonLiveReturnAt` interface; a generic ledger theorem excludes any distinct source owner from that target prefix for both reset and reuse. The integrated gate passes 602 unique cases, 1,764/1,764 comparisons, all coverage floors, 84 bug cards, and exactly one registered trusted axiom. | Generalize target-ledger exclusion beyond singleton prefixes, derive the retained-prefix interface from arbitrary exact checked compiler residuals, and replace the remaining finite source reset/reuse classification with generic local operation-shape/ownership premises. After lane 4 rebases its shared contracts, also re-probe focused `SimpCaseRelation` and `AlphaEqvCode` consumers. |
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | active | Clean committed head `13958eb4`, one hundred thirty-four commits ahead and three behind functional `main`, separates recursive callee contexts in the concrete compiler-correctness stack. | Rebase, run/report the required full and Talos checks, and publish a formal handoff before integration. Float32/Float boxed-kind codes, packed storage/projection/mutation support, resolver support, `HeapRefinement` cases, and the closure-application audit remain W6-owned; W6 owns implementation-to-concrete-host theorems, not validation adapters or the W7 compiler. |
| W7 generation | generation owner | `wasm/generation` | active | Clean committed head `e5c67f54`, thirty-three commits behind and twenty-two ahead of functional `main`, executes exact Float32/Float64 results through the manifest's integer-lane facade. Earlier clean checkpoints expose the versioned `bitExactFloatTransport` capability. | Rebase before continuing and publish the exact check results and formal artifact handoff; the commit is observed, not yet accepted. The twelve scalar closure cases are only future `wasm-generation-pending` admission probes and require no overlap now. The queued Lean float/closure stack still needs proof-owned `AlphaEqvCode` adaptation and W6 concrete refinement before the final artifact becomes `linked/accepted`. |
