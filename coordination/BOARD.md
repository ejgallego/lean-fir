...
| `LCNF-EXTENSION-LIFECYCLE-CAPTURE` | W7/integration | VBP module-product closure | active | decision `ROOT-W7-20260915-025` | Isolates whether `VersoManual.Basic`'s duplicate `inlineExtensionExt` arises in a fresh owning frontend, from prior owning-module contexts, or from a repeated plugin/dylib load. It compares unchanged actual source/setup in fresh and serial processes and records lifecycle state only. It does not reset persistent extensions, suppress initializers, modify plugins/importer/source-unit policy, serialize/inject an in-memory closure across processes, or resume source closure. A production process-boundary/transport design requires a separate root decision after this causality result. |
