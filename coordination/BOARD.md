...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | blocked | inventory functional `914f5d87`; clean handoff `c2945976`; VBP request `FIR-VBP-20260916-001` | Assesses whether VBP's new once-per-runtime `createEncodedDocumentComponent (mathComponent? := none)` can be captured through the current typed FIR adapter, preserving native `Document.encode` String input and session-owned controls/identity/content. The read-only current-graph inventory resolves 256 modules and 99 installed-toolchain boundaries, but stops before capture: its first unapproved direct input is mutable `Preview/Component/Content.lean` (`2cf21217…`); the current setup also has cross-reference-workspace paths and a dirty Verso `Parser.lean` dependency. Both supplied Component/Shell hashes match. VBP must supply a content-addressed closure snapshot or explicitly authorize exact mutable inputs before a reproducible capture can run. Then FIR stops at the first missing generic primitive or typed import; optional native math follows only after `none` is feasible. No renderer fork, adapter/ABI/options-schema/transport change, source/pin mutation, consumer acceptance, timing result, main migration, W6 work, or change to preserved `4776f86e`/`a48bbb4a` artifacts. |
