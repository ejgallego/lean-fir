...
|---|---|---|---|---|---|
| Integration | integration owner | `integration/wasm-lanes` | active | Integrated W7's closed resident `prettyM` runtime through `071fcbc`, exact-provenance pass proofs through `a29d480`, and the complete exact signed validation family as `5305358`; the isolated `FLOAT-SCALAR-RUNTIME` producer is ready at `c901e0d`, with its rebased validation stack ready through `67c6532` | The root Lean and pass-proof cones are green on the producer; W6 and W7 must adapt the exact exhaustive consumers recorded below before the contract and dependent validation stack can land on green `main` |
