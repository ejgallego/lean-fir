...
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, later FIR lower/link | active | decision `ROOT-W7-20260915-022`; diagnostic `31067950`, handoff `8db61f63` | Resolves the real `Renderer.render` source closure through a deterministic worklist of recorded defining modules. Each owner is captured exactly once through the Lake or released derived-input provider; a signature becomes local only after exact signature agreement, while duplicate bodies, unknown/ambiguous provenance, unavailable inputs, or new compiler diagnostics stop the run. The first run admits 26 modules/1,201 bodies and stops at `VersoManual.Basic` duplicate persistent-extension registration. It retains module boundaries and never falls back to fresh root reset or generated-name seeding. The next lifecycle-isolation investigation precedes any continuation; lowering, linking, Wasm/profile admission, ABI/runtime/W6 and consumer changes remain separately authorized. |
...
| `LCNF-ISOLATED-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP renderer source closure, later FIR lower/link | active | decision `ROOT-W7-20260915-030` | Applies the accepted trusted-local worker/assembler mechanism to the deterministic renderer source worklist: each owning module is captured in its own native context, and the assembler admits its verified product only after exact owner/body/interface agreement. Every worker identity, product digest and loaded-region lifetime is explicit; native/VIR boundaries remain external. It stops at complete source closure or the first source/provenance/signature/transport diagnostic. No cache reuse policy, durable/untrusted format, lowering/linking/Wasm/profile, runtime/ABI/W6 or consumer change is included. |
