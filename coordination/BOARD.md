...
- W6: ROOT-W6-20260909-107 is accepted at exact checkpoint `a5076b0f`
  ...
  ROOT-W6-20260910-018 is accepted at exact saturated/lazy staging checkpoint
  `79022194e90483fd0186e331932edbb9eda1127a`, functional `f0170471`, base
  `7529041f`. Independent diff-check, make check (730/2172, 38 mailbox tests)
  and make talos-check (3204 jobs, 3165 audit cone, 110 endpoints) pass.
  Exact verified receipt:
  `42d7b8199eb16de801507caaa20dd616a6a600e720efd9f8f6ed0ec5b382c657`.
  Logs: root worktree `.deps/root-saturated-lazy-stage-{check,talos}.log`.
  The original staging/ready-relation file is unchanged. Direct reindex keeps
  caller root on both actual ready outcomes, zero-step target identity and
  strict source rank. Exact retain-capacity and both lazy hit/miss admission
  ...
  ROOT-W6-20260910-032 is accepted at exact lazy-miss entry checkpoint
  `c6d2e27090a9b217810a6938a4909073fcc02292`, functional `d5a54d06`, base
  `17de790b`. Independent diff-check, make check (730/2172, 38 mailbox tests)
  and make talos-check (3204 jobs, 3165 audit cone, 130 forced endpoints) pass.
  Exact verified receipt:
  `82648131d8d2d1e308f9ba3051376942445fb853a1e449bd2334f873d04a7ead`.
  Logs: root worktree `.deps/root-lazy-miss-entry-{check,talos}.log`.
  Original signature, core/generated/push and complete callee construction are
  byte-identical. The same checked lazy push preserves root on the actual
  initializer, exact three-step path, non-heap admission and suspended publication
...
- Accepted W6 proof-architecture planning checkpoint: functional documentation
  head `0f2b8f0f` is linked through clean tracked handoff `4fca13b0`, based on
  accepted `sumTo` semantic-typing head `f7e46bc3`. The W6, backward-pass, and
  object-provenance roadmaps now state the same intended result: finite-prefix
  behavioral preservation from compiler-produced final LCNF to generated Wasm,
  with resident-runtime linking and earlier-pass simulations composed as
  separate reviewable links. PA0--PA4 first audit every guarded schema-aware
  source-admission branch, add only semantic provenance that the existing
  relation cannot derive, eliminate the arbitrary caller-supplied source
  invariant from the public W6 theorem, and only then resume backward pass
  composition. `sumTo` remains a smoke regression rather than a program proof;
  finite reference-count/capture and wasm32 allocation bounds remain explicit
  resource premises. `git diff --check`, `make check` at 730 unique cases and
  2,172/2,172 comparisons, and all 3,189 Talos jobs pass with receipt
  `99b4ae3d28fd90e22fb9e5d780b608ce629d9141a31a376ccce96ce09b3f89dd`.
  No semantic, ABI, layout, lowering, proof, or generated-code contract changed;
  bug cards: none. W6 next executes PA0's branch-by-branch derivability audit.
...
- Accepted W6 active-schema field-successor checkpoint: functional proof head
  `b424927d` is linked through clean tracked handoff `7a65e036`, based on the
  accepted constructor-schema agreement head `a4543bd6`. Active-witness FVar
  and erased object-field admission now consume descriptor alignment only from
  the current simulation witness. Their closed validated-code successors
  reconstruct compiler/source facts, derive that alignment from source schema
  typing plus `WitnessAgrees`, and preserve the continuation across the exact
  three-step target prefix. One factored mutation transport serves both paths
  and the legacy witness-independent wrappers. Direct batch elaboration of all
  four edited modules, the forced 3,128-job cone, `make check` at 730 unique
  cases and 2,172/2,172 comparisons, and all 3,182 Talos jobs pass with receipt
  `25254d510e767bb17b0a293cbed7a92f641b9935ac6e64ef4b1dddabec75604e`.
  No shared contract changed. The remaining work is global schema/agreement
  threading and dispatcher selection; the object-field admission bug therefore
  remains confirmed, not fixed.
- Accepted W6 schema-aware code-dispatch checkpoint: functional proof head
  `bd03de5b` is linked through clean tracked handoff `82b10e74`, based on
  accepted active-schema integration head `2693f593`. A three-way current-node
  admission retains established operation proofs while giving FVar and erased
  object-field writes dedicated source-schema alternatives. The compatibility
  arm has a syntax guard that is false for exactly those two `oset` shapes, so
  it cannot reintroduce their old arbitrary-witness premises. Residual
  validation derives this admission, and the validated ordinary-code
  dispatcher combines schema typing with `WitnessAgrees` for the active
  witness; its two field branches have exact zero allocation cost and use the
  active successors directly. Lean Beam, direct batch elaboration of both
  edited modules, the forced 3,128-job cone, `make check` at 730 unique cases
  and 2,172/2,172 comparisons, and all 3,182 Talos jobs pass with receipt
...
- Accepted W6 witness-preserving administrative checkpoint: functional proof
  head `bd908f8e` is linked through clean tracked handoff `d7cd31af`, based on
  accepted witness-indexed integration head `2e5779aa`. Direct and saturated
  callee entry, lazy-cache hit and miss, destination bind, and both ordinary
  and lazy return-pop now close in
  `ConcreteStructuredValidatedCodeGlobalOutcomeAt` at exactly the incoming
  witness. The established unindexed dispatcher is only a projection through
  `toValidatedGlobal`, so no ghost witness is recovered from a `Prop` proof.
  Resolved external execution is correctly left for the witness-extension
  slice because its concrete host refinement produces `nextWitness`. Lean
  Beam, the forced 3,128-job cone, `make check` at 730 unique cases and
  2,172/2,172 comparisons, and all 3,182 Talos jobs pass with receipt
  `cc63d78b6eb09e2e873e7fcd0116e00760e64bd6c1c3a7d4747a891a99680b08`.
...
- Accepted W6 schema-changing direct-let checkpoint: functional proof head
  `34e7b302` is linked through clean tracked handoff `a02a9941`, based on
  accepted constructor-schema update integration head `132c8bac`. The additive
  `ReuseCapacityDirectLetRuntimeRefinesWithSchema` law retains one exact
  `DirectLetUpdate` for nonempty constructor allocation and every successful
  reuse branch. The law crosses pure-external contracts, closure ownership,
  the whole lazy-cache table, and entry-relative transports without erasing
  its next schema or witness. Production structured execution and residual
  validation now return that same successor pair, and
  `DirectLetUpdate.agrees` closes
  `ConcreteStructuredSchemaValidatedCodeGlobalOutcome`. Lean Beam, the
  3,127-job structured-validation cone, `make check` at 730 unique cases and
  2,172/2,172 comparisons, and all 3,182 Talos jobs pass with receipt
  `f1fbfffe54952cf6cdd7bad202870953d91713d13c68ccf7bedb199c9faa8437`.
...
- Accepted W6 schema-indexed finite-trace framework: functional proof head
  `d20971e6` is linked through clean tracked handoff `7a5d1be3`, based directly
  on accepted schema-global validation head `9301c0e0`. `DirectLetShape` now
  retains the compiler's exact `compileArgs` result, and
  `ConstructorSchema.SourceStep` records either that exact constructor/reuse
  transition or a proof that no schema-changing direct shape is present. One
  factored source-schema dichotomy drives every ordinary-code branch, while
  the witness-indexed global theorem retains the actual transition across all
  seven administrative outcome families. Pairing current-schema readiness
  with preservation under that transition now constructs the ranked weak
  simulation and `ConcreteFiniteTraceCorrect` directly, without a universal-
  schema premise, target execution certificate, or termination hypothesis.
  Lean Beam, the targeted 3,128-job cone, `git diff --check`, `make check` at
  730 unique cases and 2,172/2,172 equal comparisons, `make talos-setup`, and
  all 3,182 Talos jobs pass. No source semantics, ABI, concrete runtime,
  lowering, or emitted code changed. The remaining W6 obligation is an
  invariant instance derived from final-LCNF semantic type/provenance
  soundness plus the independent finite-runtime and wasm32 address-space laws;
  `FIR-BUG-wasm-none-object-field-kind-admission` therefore remains confirmed.
- Accepted W6 schema-indexed export theorem: functional proof head `a0e9f1fe`
  is linked through clean tracked handoff `2e0dff75`, based directly on the
  accepted framework head `8020cac5`.
  `ConcreteSupportedExport.validatedCodeGlobalRootAt` retains the exact initial
  refinement witness constructed by the canonical export frame, and
  `finiteTraceCorrect_of_schemaSourceInvariant` composes that compiler-derived
  root with one agreeing constructor schema, its hereditary source invariant,
  and the independent finite-runtime/address-space laws. A caller supplies no
  target relation, execution path, future transition, termination evidence,
  universal-schema premise, or translation certificate. Lean Beam, the
  targeted 3,128-job cone, `git diff --check`, `make check` at 730 unique cases
  and 2,172/2,172 equal comparisons, `make talos-setup`, and all 3,182 Talos
  jobs pass. No semantics, ABI, concrete runtime, lowering, emitted code, or
  compatibility theorem changed. W6 next defines and preserves the final-LCNF
  semantic type/provenance invariant that instantiates this public surface.
...
- Accepted heap-only USize convergence: the semantic contract and source
  ...
  are linked through the exact clean checkpoint `de7c03ab`. FIR now follows
  upstream `lean_box_usize`/`lean_unbox_usize`: every boxed USize is an owned
  40-byte heap object and a physical tagged word is rejected. The W7 helper is
  zero-import and uses the existing marker 5, eight-byte payload, zeroed
  reserved lanes, reference count 1, and non-persistent ownership. Lean Beam,
  the focused LCNF and W6 cones, `make check` at 730 unique cases and
  2,172/2,172 comparisons, all 3,174 Talos jobs, and the complete deterministic
  artifact/concrete/V8 gate pass. `FIR-BUG-impure-none-usize-box-tagged` is
  fixed. No semantic ABI signature, layout constant, allocator, ownership
  rule, symbolic Wasm surface, or unrelated scalar policy changed.
- Accepted scalar-box result-kind lease: shared contract head `29aff571`
  derives exact boxed result annotations from upstream `Lean.Expr.boxed`,
  preserves the generic `tobject` path, and admits no unrelated object-family
  kind. W7 functional head `bf4c9f4f` is linked through clean tracked handoff
  `3953370c`: `fir_box_uint16_tagged` and `fir_box_uint64_object` are exact
  aliases derived from the already-reviewed generic bodies by retagging only
  symbolic object-family result metadata. The physical Wasm instructions,
  generic helper implementations, concrete layout, ownership, and ABI are
  unchanged. Lean Beam, 728 unique repository cases and 2,166/2,166
  comparisons, all 3,174 Talos jobs, and the complete deterministic W7
  artifact gate pass. The zero-import resident fixture is 2,789 bytes with
  SHA-256
  `0a41bb1b5d4f6facdcee75ff3a23af42e932757226b4aeac378ba3ee18a4b357`;
  its Node client exhaustively checks all 65,536 UInt16 values and seven
  UInt64 boundaries through both generic and exact exports. The scoped lease
  is released; W6's small alias-refinement bridge is an independent follow-up.
- Accepted Float32 resident-box generation: functional head `1697038a` is
  linked through clean tracked handoff `1d723b62`. The generic source closure
  and compiler-generated `_boxed` wrapper now close with zero imports and zero
  residual runtime operations. W7 aligns the executable marker inventory to
  the already accepted concrete host (`Float32 = 6`, `Float = 7`) and checks
  canonical 40-byte owned boxes, zero Float32 padding, exact frontier growth,
  rewind, malformed-layout traps, and bit-exact signed-zero/subnormal/NaN
  round trips. The deterministic 7,248-byte standalone Wasm has SHA-256
  `fa806f59e3c1c0e56d52a6fe35fb5a3a5496f7e09b7183084b2a13af697a6c58`.
  This is generation-ready only; W6 promotion/refinement of the stable helper
  descriptor remains an independent contract-queue item. USize is now the
  sole unresolved family in the seven-family source ratchet.
- Accepted heap-only floating scalar descriptor/refinement boundary: isolated
  shared contract `ca25935e`, reusable memory proof boundary `f492bfeb`, and W6
  functional proof head `48e340b7` are linked through exact clean tracked
  handoff `b605cd9d`, based directly on `main` at `97474d5b`. Float32 uses
  marker 6 with four exact payload bytes and four zero padding bytes; Float
  uses marker 7 with eight exact payload bytes. Both are canonical owned
  40-byte heap boxes and neither admits a tagged representation. The concrete
  allocator, physical lane decoder, resolver, and compiler extraction proofs
  preserve the exact raw bits. Pure 64-bit word assembly/projection laws are
  factored into the concrete memory boundary, removing a Lean 4.33 generated
  enum-helper import diamond exposed by forced source recompilation. Lean Beam,
...
- Accepted native-order trusted Array replacement repair: W7 source-shape head
  `d701c32e`, W6-authored discrepancy card `05a93b2f`, and executable repair
  `56e25d73` are linked in that exact order. The three private production
  helpers now have public closed function/body witnesses plus name, signature,
  and body equations for W6, without changing their generated definitions.
  The common exclusive replacement arm now matches upstream Lean and FIR's
  semantic model by loading and decrementing the displaced value before
  storing the replacement; checked/shared behavior and the copy-on-write tail
  are unchanged. Lean Beam and the focused 28-job cone pass, `make check`
  remains at 730 unique cases and 2,172/2,172 comparisons, all 3,183 Talos
  jobs pass with receipt `60c00c05`, and the complete deterministic artifact
  and 44/44 concrete-readiness gate passes. The trusted Array fixture remains
  13,807 bytes and prettyM remains 87,595 bytes with 322 functions. No helper
  name/signature, ABI, layout, import, export, allocator, or package contract
  changed. W6 continues the installed-body proof in
  `W7-W6-20260828-003`; bug card
  `FIR-BUG-wasm-none-array-exclusive-release-order` records the repaired
  semantic discrepancy. The disjoint fixed-width checkpoint `e77c54d2`
  remains frozen until that proof lands.
- Accepted trusted Array installed refinement: W6 head `614980cb` proves the
  production `Array.uset`, `Array.set`, and `Array.set!` entries against the
  repaired native-order resident implementation. One common semantic theorem
  carries the reference-count-one path through exact displaced-value release,
  replacement write, rebuilt live-heap/resident-memory relations, and
  same-address return; alias-specific proofs discharge only decoding and local
  frames. The common nonexclusive theorem reaches the unchanged
  shared/persistent copy frame after loading capacity and proves that deferring
  that load until after the exclusivity probe is observationally irrelevant.
  Lean Beam is diagnostic-clean, the focused 3,094-job cone passes, `make
  check` remains at 730 unique cases and 2,172/2,172 comparisons, and all
  3,180 Talos jobs pass with receipt `a7720c8f`. No W7 helper, source
...
- Accepted fixed-width proof-visible body surface: W7 checkpoint `be49211e`
  publishes complete explicit body constants and closed `rfl` equations for
  production `UInt8.toNat`, `UInt8.toBitVec`, `UInt16.toNat`, and
  `UInt32.toNat`. The private helper constructors remain private; W6 can now
  attach its generic range/WP proofs to the actual installed functions without
  duplicating their implementations. Lean Beam is diagnostic-clean, the
  focused 19-job cone passes, `make check` remains at 730 cases and
  2,172/2,172 comparisons, all 3,180 Talos jobs pass with receipt `d9fb773f`,
  and the complete artifact gate passes. The fixed-width artifact remains
  byte-identical at 17,812 bytes and SHA-256
  `cff779e93097de7c94d15c568d16ba15785042b9aa4a5a9dd85316f591cd794a`.
  No executable helper, signature, ABI, layout, ownership, linker, import,
  export, package, or semantic contract changed. W6 continues thread
  `W7-W6-20260828-004`; bug cards: none.
- Accepted production fixed-width Natural refinement: W6 functional head
  `e6d3c2a5` is linked through exact clean tracked handoff `d3366493`, based
  directly on the accepted W7 body surface. `UInt8.toNat`, `UInt8.toBitVec`,
  and `UInt16.toNat` share the exact canonical immediate-word theorem and
  preserve the caller store and operand tail. `UInt32.toNat` has exact
  installed low/high termination theorems: the low arm returns the canonical
  immediate word, while the high arm composes the existing resolved
  `fir_numeric_make_natural(value, 0)` execution and preserves its result
  store, returned word, extended refinement witness, and restored caller tail.
  W7's four closed equations are used only at the final production attachment
  boundary; no named semantic shim duplicates the generic bounded proof. Lean
  Beam is diagnostic-clean, the focused 3,126-job cone passes, `make check`
  remains at 730 cases and 2,172/2,172 comparisons, and all 3,182 Talos jobs
  pass with receipt `36c2023e`. No helper body, signature, ABI, layout,
  ownership rule, source semantics, linker inventory, or artifact changed.
  Thread `W7-W6-20260828-004` is complete; bug cards: none.
- Rejected bounded `ByteArray.get` caller expansion: W7-2's ephemeral typed
  provider and generic two-part persistent-plan registration passed Lean Beam,
  the immediate/promoted real-engine boundary, zero-import differentials, and
  flat-frontier ownership, but expanding all 425 sites added 9,894 bytes
  (+2.578%). Two balanced 24-pair timing campaigns were neutral/slower at
  +0.32% and +2.88%, so the candidate was discarded without moving a package
  ratchet or artifact pointer. Thread `W72-W7-20260828-007` is closed with
  disposition `rejected`; the shared planner protocol remains valid and no W6
  request was opened. A revisit should test hot/cold helper placement or a
  generic size-aware policy rather than duplicate the branch at every caller.
- Accepted precise direct-call result proof: W6 functional head `99a3c03e` is
  ...
  the effective callee result kind chosen by production local-row refinement
  through entry, frames, resource/validation stacks, and caller resumption;
  public source-result compatibility remains an independent refinement fact.
  The strict `object -> tobject` regression proves that compiler and validator
  locals stay `object` while the public annotation remains `tobject`.
  `FIR-BUG-wasm-none-direct-call-validation-result-kind-drift` is fixed. Lean
  Beam, the 3,127-job forced cone, `make check` at 728 unique cases and
  2,166/2,166 comparisons, and all 3,174 Talos jobs pass. No shared runtime,
  ABI, layout, helper, symbolic Wasm, or W7 artifact contract changed.
- Accepted deleted-constructor allocation provenance: LCNF functional head
  `8d8cfcb1` is linked through atomic clean checkpoint `bdc4e232` after the
  mailbox cutover. Heap-backed deleted-constructor evaluation now returns both
  the normal exact target-ledger relation and the durable allocated source-only
  lifecycle carrier after binding the dead source result; failed-token reuse
  forwards the same stronger carrier. The focused 34-job importer cone and
  `git diff --check` pass on the final rebase; the earlier functional checkpoint
  passed Lean Beam and the complete 728-case, 2,166/2,166-comparison root gate.
  No runtime, interpreter, compiler-pass, target-ledger, or shared semantic
  contract changed. The proof lane may now begin its separately tracked
  carrier-dispatch successor, while USize adaptation remains a distinct queue.
- Accepted deleted-allocation semantic carrier: LCNF functional head
  `3d811853` is linked through clean tracked checkpoint `8788603c`, based
  directly on `main` at `f20cc40a`. A generic deleted-let theorem now preserves
  an existing allocated source-only location while the source frontier moves
  monotonically and the target stutters with its exact ledger; the semantic
  deleted-constructor matcher returns the same durable heap capability beside
  the ordinary ledger machine relation. Lean Beam, the focused 34-job cone,
  `make check` at 728 unique cases and 2,166/2,166 comparisons, and the final
...
- Accepted Talos anonymous-control-depth repair: isolated shared-contract head
  `8cb20ce7` makes the proof adapter count every structured control frame while
  retaining symbolic names only for branch-resolvable block and loop frames.
  An `if` pushes an anonymous entry, so a branch from its arm to the enclosing
  loop is `br 1`, exactly as in `Fir.Wasm.Emit.encodeWithOrigins`. The label
  context is threaded through code adaptation, active and suspended structured
  states, runtime refinement, and resident-helper proofs. Generated Wasm,
  symbolic instructions, runtime signatures, ABI/layout, and ownership are
  unchanged. `FIR-BUG-wasm-none-adapter-if-branch-depth` is fixed; Lean Beam,
  `make check` at 726 unique cases and 2,160/2,160 comparisons, and all 3,172
  Talos jobs pass. W6 rebases on this boundary before completing resident Array
  installed-call provenance; operational thread `W6-W7-20260826-015` records
  the exact consumption instructions.
...
- Accepted inhabited-monad fallback removal: W7 functional head `f5e29648`
  is linked through clean tracked handoff `0dd9dbd1`, on the released dynamic
  closure-underapplication admission contract `a823686b`. The real Lean
  `instInhabitedOfMonad._redArg` declaration is now retained and compiled from
  final LCNF through the generic source path; its trap-backed resident
  substitute and external declaration are gone. A production-shaped generic
  monad probe returns 42 in native Lean, the FIR LCNF interpreter, and a
  zero-import external-engine Wasm module. The fail-closed fallback family now
  contains only `panicCore`. Lean Beam reports zero diagnostics; `make check`
  passes 717 native/LCNF, 9 direct-machine, and 717 V8 cases with all 2,160
  comparisons equal; all 3,172 Talos jobs and the complete deterministic W7
  artifact, package, browser, ownership, stack-safety, concrete-readiness, and
  16-source-probe gates pass. `FIR-BUG-wasm-none-generic-inhabited-fallback-trap`
  is fixed. The separately observed upstream/FIR UInt64 boxing mismatch is
  recorded as `FIR-BUG-impure-none-uint64-box-tagged` and queued below rather
  than being hidden by this slice.
...
- Current production release proof surfaces: `decrementOnceFunction` is public
  ...
  `fc61301d946b1596ad08c9b20d51d2e1f68d60ca0c04b0f9d56e298c2ec6408d`.
- Accepted production decrement decomposition: W7 functional head `f35913d4`
  is linked through exact clean handoff `4a390625` and operational checkpoint
  `W7-W6-20260825-008`. The installed `decrementOnceFunction` now consumes the
  public `decrementOnceBody`; its persistent, last-reference, constructor,
  closure-descriptor, opaque/Array, and outer admission builders are
  reduction-visible to W6. Descriptor dispatch is total structural recursion
  over `descriptors.toList`, and `decrementOnceFunction_body_of_ok` extracts
  the actual installed body without a duplicate generator or caller-supplied
  certificate. Production helper name, signature, locals, layout, ownership,
  installation order, and instructions are unchanged. The focused release
  artifact remains byte-identical at 1,850 bytes and SHA-256
  `c2edc88d12400714aa5268b37e61dd55d11d2fb6073c888d2c332565378c7d0a`.
  Lean Beam, the focused 54-job linker cone, `make check` at 726 cases and
...
- Landing order:
  ...
  25. W7's rebased proof-visible impure-hygiene contract lands at `818a1be1`,
      immediately followed by W6's three-consumer adaptation at `1d44bfe5` and
...
- `USIZE-BOXING-UPSTREAM-ALIGNMENT` is an active isolated contract candidate
  on base `e2c2f8c4`: bug-card commit `9841a7a1` precedes semantic/fixture head
  `edaea293`. It aligns FIR with upstream `lean_box_usize` /
  `lean_unbox_usize`: every `USize` box is heap allocated and a tagged word is
  rejected by `unbox USize`. Source-generated small and maximal owned-release
  fixtures execute the exact `box, fap, lit, return, dec, return` trace. Lean
  Beam, the focused final build, the focused native/LCNF/V8 triangle (six of
  six comparisons), and full `make check` (730 unique cases and 2,172/2,172
  comparisons) pass. The semantic provider therefore validates the new
  contract, but the zero-import resident linker still lacks dedicated
  `.box/.unbox .usize` functions. The rebased LCNF provenance checkpoint is
  already below this candidate. After worktree-local `make talos-setup`, the
  Talos gate reaches 3,171/3,178 targets and identifies exactly two dependent
  proof modules: `ElimDeadRuntimeRel` has two stale tagged-unbox equivalence
  cases, while `BoxingCorrectness` has three stale heap-only policy/unbox
  arguments. LCNF-proof and W6 adapt those owned surfaces; W7 then adds and
  ratchets the executable helpers before this queue item can be released. Do
  not confuse the provider-backed V8 result with resident-linker readiness.
...
- W6's production resident `UInt64` helper refinement is linked/accepted
  through functional head `45989deb` and tracked handoff `991b3774`.  The
  exact installed allocator, box, unbox, trap, layout, and bit-preservation
  paths are in the default Talos umbrella cone; all 3,172 jobs passed on its
  handoff base.  No runtime implementation or shared contract changed.

- LCNF's aligned source-only deleted-operation carrier is linked/accepted
  through rebased functional head `75b896fe` and tracked handoff `67474d9f`.
  Historical target allocations are handled without a duplicate target-phase
...
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | released | Clean tracked checkpoint `1ee832cb`, functional head `30bc5522`, closes ordinary direct-call result admission from residual validation and leaves only directional argument ingress at that call boundary. | Non-cached named calls now require directional result refinement; nullary cache calls retain object-family compatibility. Product census: 0 rejected result edges across 11,487; full root and 3,190-job Talos gates pass. |
...
| Bounded fixed-width `toNat` boxing | W7 functional `d87dc9bd`; tracked handoff `e8b89fce`; closed body surface `be49211e` | unchanged tagged/promoted Natural and fixed-width scalar layouts; W6 functional `e6d3c2a5`, tracked `d3366493` | contract-proved | W6 complete | raw lean-zip `2c01e190`, 383,816 bytes, zero imports, 502 functions; boundary artifact `cff779e9`, 17,812 bytes |
...
|---|---|---|---|---|---|
| `WASM-PROOF-FREE-RUNTIME-LAYOUT` | integration/W7 | W7 native generators, W6 concrete runtime/proofs | linked/accepted | isolated contract `fe7b3de06` | Extracts `heapBase`, resident representation markers, and canonical natural-limb decomposition into the lightweight `Fir.Wasm.Concrete.RuntimeLayout` module. Resident generation now imports only `Concrete.Layout` plus this contract instead of pulling `Concrete.Runtime` → `Concrete.Memory` → `Concrete.Refinement` and `Std.Tactic.BVDecide` into native executables. Public names, values, concrete runtime behavior, helper signatures, symbolic Wasm and emitted artifacts are unchanged. The resident-linker FIR cone drops from 50 to 48 project modules; its native linker response contains `RuntimeLayout` and none of the three concrete model/proof objects. Full `make check`, 3,212-job Talos build/proof-trust, and the complete deterministic artifact/V8 gate pass. |
| `QUIET-COMMAND-FAILURE-STATUS` | tooling/root | all quiet command callers and CI | linked/accepted; lease released | `ed77b41f4aee8e6ac7e3e8d68ea3efd9de5c4a77` | Root independently reviewed nine direct regressions and passed full make check, then fast-forwarded main. Actual child status, concise output/full failure logs/FIR_VERBOSE preserved. Initial review crash was upstream Lake setup-tempfile SIGSEGV with root's nonexistent TMPDIR; creating .deps/tmp restored the unchanged gate. No broad refactor or weakened 4.34 expectations. Official tooling child backport remains separate. |
| `NATIVE-FLOAT-TOSTRING-PROVIDER` | W7 prototype/root contract owner | callback-rooted VBP options component; future standard-runtime consumers | prototype accepted; production parked | isolated `a642e356`, rounding successor `1c45f00d`; accepted private checkpoint `5fefee0c` via `ROOT-W7-20260916-019` | Root reviewed pinned Lean `6a10ac8c` native formatting, separate `fir.native-float-format/v1`, fresh existing-layout FIR String allocation/copy and bounded native scratch/stack/262,144-byte low reservation. Independent 6,288 native-oracle comparisons and String/padding/growth/scratch/invalid allocator checks pass; native build-input and output hashes verified. Complete component has no residual native import. Existing libm/v2, shared linker and W6 layout/helper/ABI remain unchanged. Native proposal file lease is released; no universal libc equality/refinement theorem, combined-provider or general production adoption. Future production integration needs root review/gates; current use remains the qualified private session-retained prototype. |
| `TOOLCHAIN-LEAN-4.34-COMPATIBILITY` | tooling/root | W7 and other user-project compiler consumers | active | official lane smoke `05a2e64e`; tracked overlay `66cafb6f3`; RC2 trust profile `fe1a21bc7` | Establishes the official FIR 4.34 compatibility branch and `.worktrees/tooling-lean-4.34`, pinned to `leanprover/lean4:v4.34.0-rc2` / `6a10ac8c`. Workers whose user-project Lake setup requires 4.34 use this lane or a track-owned child rebased on it, preserving the project’s actual Lake module setup. The tracked Talos overlay pins both Lake manifests, Talos/mathlib revisions and the isolated W6 normalization; its focused/full 3,231-job FirTalos/TrustAudit cone passes. The version-indexed UInt8 trace audit and exact-RC2 trusted-assumption profile now pass fail-closed negatives, while the 721-case differential remains green. The default no-argument route and 4.33 setup remain unchanged; the explicit RC2 profile is audited compatibility, not a kernel proof or `main` migration. Tooling owns its pin, Lake/CI compatibility and migration evidence; root still owns any eventual migration. No expectation is weakened and no ABI/runtime/proof/consumer change is included. |
| `VBP-VIR-SESSION-HOST-PROTOTYPE` | W7/integration | VIR host semantics, VBP preview acceptance | released | initial functional `4163e27a`; error-aligned functional `852149f0`; immutable handoff `8ec770fd` on official 4.34 child | The bounded bridge executes the linked renderer with exactly twelve signature-checked VIR imports, module-owned memory, explicit String/Bool/Float/IO conversions, and a strong per-session JS table. Its typed `Callback.ofUnary` trampoline keeps the transferred Lean closure live and borrows it on later calls; explicit disposal after consumer unmount rejects new calls/callbacks and clears adapter-held references. A user-provided VIR raw-host policy input (`03ffaa13`, draft PR189, not landed) exposed and W7 repaired the key error distinction: an **ordinary provider exception** becomes an actual compiled Lean `IO.Result.error`, stops the bind, preserves the original arbitrary thrown JS value for rethrow outside the owning structural guard, and leaves the session plus escaped callbacks live. Nested/reentrant frames preserve caught/uncaught identity and prior side effects remain performed. Only malformed values/signatures/envelopes and real Wasm/ABI failures poison the session. This adopts no VIR pin or permanent ABI. The prototype still uses fixture-built structured Documents and recording React doubles; input coverage advances separately under `VBP-RENDERER-DOCUMENT-JSON-ENTRY`. Session growth remains accepted prototype debt; no main migration, W6/VIR/VBP source edit or generalized host framework is included. |
| `VBP-RENDERER-DOCUMENT-JSON-ENTRY` | W7/integration | designated VIR widget-backend consumer owner | linked/accepted | functional `2d6aac03`; immutable producer handoff `a48bbb4a` on official 4.34 child; consumer evidence `VBP-FIR-RICH-ACCEPTANCE-20260916-001` | The renderer-only, synchronous `renderDocumentJson(string)` entry compiles the existing `VersoBlueprint.Experimental.VirPreview.Document.decode`, calls actual `Renderer.render` with default options, and returns the genuine React node. It has no JS schema copy, general Document/Options transport, Preview/RPC wire-format migration, or options transport. The shared owner verified matched real React SSR and Chromium against source-matched VBP `c4430bfe` / VIR `9fafe9cf`: four exact rich/full-FLT SSR equalities, rich and two full-FLT browser DOM equalities, extensions/details, malformed recovery, retained callbacks, provider-error identity, later renders, disposal, and six identity-preserving detail updates per engine all pass without React warnings. Full-FLT `Document.encode` inputs remain byte-preserved. This establishes matched renderer acceptance, not timings, optimization, general options/RPC transport, or the newer live-widget shell. |
| `PTR-ADDR-UNSAFE-RESIDENT` | W7/integration | JSON renderer entry, later W6 refinement | released | functional `2d6aac03`; immutable handoff `a48bbb4a` | The source-complete JSON closure's sole unresolved primitive, borrowed `ptrAddrUnsafe : erased × tobject → usize`, is now internalized only after exact upstream final-LCNF name/type/borrow/native-symbol (`lean_ptr_addr`) validation. Its resident Wasm32 body is `local.get; i64.extend_i32_u; return`: no locals, memory access, allocation, retain/release, tag inspection, host fallback, or new pointer API. Nine tagged/high-bit pointer words and unchanged memory/live-object RC pass; eight malformed source declarations and three malformed symbolic signatures reject. The JSON closure has no residual native primitive/runtime-op import beyond its existing twelve VIR hosts. No shared semantic/ABI/layout/USize/instruction change or W6 theorem is claimed. |
| `VBP-FIR-LIVE-WIDGET-SHELL` | designated VIR widget-backend consumer owner | VBP VS Code preview | active | consumer-only live observation, 2026-09-16 | The newer VBP RPC/shell can display the frozen FIR renderer live and now forwards the server `Document.encode` string directly to `renderDocumentJson`, removing the prior shell decode/re-encode detour. It uses default renderer options and is intentionally outside the matched renderer control: no timing, highlighting, KaTeX, live-edit qualification, producer change, pin change, or optimization conclusion is claimed. Automated live-widget edit qualification is a separate consumer task; FIR has no active producer lease here. |
| `VBP-FIR-RENDERER-VERSION-ALIGNMENT` | W7/integration | designated VIR widget-backend consumer owner | linked/accepted | diagnostic `d504fb4a`; functional `65eeaa61`; immutable handoff `4776f86e` on official 4.34 child; consumer acceptance `VBP-FIR-CURRENT-ACCEPTANCE-20260916-001` | Immutable VBP `92db6325` / VIR `36d26bc2` sources, manifest and five hashes verify; default JSON capture/resident linking succeeds (52 modules/1,985 bodies/18 native providers) with a 2,219,310-byte module and exactly 13 typed VIR-only imports. The three-operation delta from the old adapter—`Js.Object.get`, `Js.UndefinedOr.isUndefined`, and `JsValue.toBool`—now executes against real compiled Wasm. The shared consumer verified matched real React SSR and Chromium equality for rich and full-FLT before/after documents, malformed recovery, provider-error identity, retained details/callbacks, and disposal. The live ProofWidgets full-FLT gate also passes: mount plus two edits gives three FIR renders, one RPC/edit, retained DOM and unchanged Document-string delivery, with no warnings. This is an official 4.34-child acceptance, not a `main` migration, timing result, or broad widget-parity claim. |
| `VBP-FIR-CURRENT-GENERIC-ADAPTER` | W7/integration | current renderer alignment | linked/accepted | request `ROOT-W7-20260916-007`; functional `65eeaa61`; handoff `4776f86e`; consumer review `VBP-FIR-TYPED-ADAPTER-FOLLOWUP-20260916-001`; acceptance `VBP-FIR-CURRENT-ACCEPTANCE-20260916-001` | Adds only the three signature-checked current generic VIR bindings needed by the retarget. `Object.get` returns the exact raw JS property value—including a rooted `undefined` and distinct `null`—while retaining its borrowed parameter behavior; `UndefinedOr.isUndefined` and `JsValue.toBool` use their current strict provider behavior and construct the actual compiled Lean Boolean/result representation at the ABI edge. The compiled producer and independent consumer establish payloads `1`/`3`; swapped-value controls reject the wrong representation. Ordinary provider errors preserve identity and leave session/callbacks live, while malformed ABI values poison the session. The producer and matched SSR/Chromium/live-widget consumer gates cover property/missing/undefined/null, strict Boolean rejection, valid/malformed JSON, callbacks/later renders after errors, provider identity, disposal, rich documents and retained DOM. It is not a generic dispatch framework, permanent host ABI/pin adoption, package/main migration, or performance result. |
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | prototype accepted; local packaging active | immutable `5fefee0c`, functional `618a67d2`; source/setup `2c1ef0f6`/`6bbd2b32`; package successor `ROOT-W7-20260916-020` | Source-preserving upstream Lake setup and actual frozen `createEncodedDocumentComponent none` plus explicit callback roots now execute through Wasm/real React SSR/Chrome. Complete module: 1,968,612 bytes, `98504ee7…`, 31 deliberate VIR function imports, zero memory/native imports, owned memory, fifteen function exports. Native options/state/DOM identity across updates, six effect setups/cleanups, malformed recovery, error identity/reentry, retained callbacks and unmount-before-dispose pass. Root verifies bound outputs/native build inputs, independently replays native formatter and actual SSR/value/error/callback/disposal controls, and reviews Chrome evidence; not an independent Chrome/full-parity campaign. `ROOT-W7-20260916-019` accepts this bounded session-retained prototype. Consumer notified for full matched parity; W7 next builds a minimal fresh-output self-contained local package using existing utilities, with no producer-tree runtime dependency or public registry. Optional math, nullable/form-value branch campaign, GC/refinement, general/combined native production adoption, timings, publication, canonical/live pins, main migration and W6/VIR/VBP edits remain out of scope. Old 4776/a48 bytes and four known 4.34 audit failures unchanged. |
| `FIR-WASM-HOST-BINDING-RENDER-CORE-V0` | integration / W7 | VBP preview, future browser consumers | active | draft `docs/wasm-host-binding-profile-v0.md` on current main | Defines the fail-closed versioned physical host-binding boundary for synchronous render cores: exact Wasm import inventory, checked scalar/UTF-8 codecs, opaque adapter-owned resource tokens with explicit borrow/take/return ownership, and manifest conformance fixtures. It deliberately excludes retained callbacks, components, hooks/effects, LeanRef, promise/RPC and browser mounting. VIR metadata is a logical-API crosswalk, not FIR's raw ABI. W7's `8ad8a645` feasibility baseline inventories the original 151-local/41-signature renderer frontier (24 Lake-resolvable, 6 native, 4 VIR, 7 installed-toolchain setup gaps) and composes one owning module into a deterministic 512-local/103-signature capture product. This is source-capture evidence only: toolchain-module input provenance must be decided before recursive closure, and no ABI/profile admission has occurred. No Lean semantic, symbolic Wasm, concrete-layout, resident-helper or W6 proof contract changes. |
| `WASM-TRANSITIVE-ERASED-CLOSURE-DISPATCH` | integration/W7 | W6, W7, validation | released | shared lowering `f8954036`; resident/widget stack through `5f448e39`; W6 proof `ce30b6ce`; clean evidence handoff `cf5e335e`; mailbox `W7-ROOT-20260901-003` / `W6-ROOT-20260901-001` | Follows compiler-generated erased lanes transitively through statically named declaration parameters. Raw `erased`/`void` parameters remain the only terminal sinks; eligible `tobject` and explicit-boxing `tagged` carriers recurse through one bounded, visited-node query shared by both structural predicates. Cycles, exhaustion, unknown declarations, arity mismatch, and external carrier parameters fail closed. No generated-name match or global erased/object compatibility is added. W6's declaration-aware proof cone, the resident physical-zero persistence refinement, the complete artifact gate, and two deterministic exact VBP widget generations pass. Wasm SHA-256 is `d738584215ea9c4b3eabef3aa290245c49fc54e2128d0dddf303db82cdc26a66`; bug `FIR-BUG-wasm-none-transitive-erased-closure-dispatch` is fixed. |
| `LCNF-OWNING-MODULE-CAPTURE` | W7/integration | VBP renderer probe, future ordinary-source capture callers | active | capture `ca0b0a40`, feasibility/one-module assembly `7b60a8da`, handoff `8ad8a645` | Replaces the failing fresh per-root final-LCNF reconstruction only by capturing through the target owning ordinary module's original header and command-frontend context. The adapter retains upstream declaration groups, target-private/import visibility, generated-specialization context and final-LCNF recorder semantics. It does not erase/reseed compiler mappings, import a target into itself, inject prebuilt readers, change importer or source-unit policy, migrate to IR, weaken checks, or change semantic/ABI/layout/runtime/W6 contracts. Actual `Verso.Doc`, `Renderer.render`, and one `VersoReact.Renderer` owner compose deterministically; the next accepted prerequisite is a fail-closed installed-toolchain input provider for the observed seven signatures, after which recursive capture remains separately leased. |
| `LCNF-TOOLCHAIN-MODULE-INPUTS` | integration/W7 | VBP preview source capture, future installed-Lean owners | released | derived provider `6afe2ef8`, handoff `e236fbfc` | Supplies a faithful, explicit compiler input for an installed Lean module only when its exact source, toolchain revision, module identity, imports/options and compiler-artifact provenance are recoverable from the pinned toolchain. Releases omit original bootstrap setup/options, so the provider derives a **new capture input** from the exact pinned upstream bootstrap template, CMake stage options and recorded release module artifacts; that provenance is labeled derived, never recovered release metadata. All five owners/seven observed entries pass deterministic capture and negative provenance controls. It rejects unavailable, inconsistent or ambiguous records; it does not synthesize a Lake setup, import a target into itself, seed generated declarations, alter the selected toolchain, or change importer/reset/source-unit policy. This releases the source-input prerequisite for later recursive capture, not lowering, linking, Wasm execution, ABI/profile admission, or W6 work. |
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, FIR lower/link | released | official source-to-Wasm `39fb3d6f`, checkpoint `655ee202` | The outcome-driven investigation establishes that real VBP `Renderer.render` compiles and links through FIR under its faithful Lean 4.34/Lake contexts. The official child has 52 modules, 1,535 bodies, 158 selections and 16 generic native-export links; it emits deterministic 1,388,311-byte Node-valid Wasm with module-owned memory, zero native/memory/runtime-operation imports, and exactly 12 intentional typed VIR function imports. String raw/internal queries reuse established String and arbitrary-Nat helpers; the repaired multi-limb position behavior is oracle-tested. This is a compile/link result on the official 4.34 child, not a `main` toolchain migration, browser execution, package publication, W6 theorem, or host admission. A typed VIR host adapter—including callbacks and resource lifetimes—is a separate shared host-policy decision. |
| `LCNF-EXTENSION-LIFECYCLE-CAPTURE` | W7/integration | VBP module-product closure | released | lifecycle diagnosis `f23f7411`, handoff `b7baf3df` | Isolates whether `VersoManual.Basic`'s duplicate `inlineExtensionExt` arises in a fresh owning frontend, from prior owning-module contexts, or from a repeated plugin/dylib load. It establishes that overlapping aggregate `libverso_VersoManual.so` and standalone `verso_VersoManual_Ext.so` images register the same persistent extension in one process; fresh/same-context controls pass. No reset, initializer suppression, plugin/source mutation or source-closure continuation was adopted. |
| `LCNF-PROCESS-CAPTURE-TRANSPORT` | W7/integration | VBP module-product closure | released | transport probe `c3575453`, handoff `7adfe8d` | Tests one real conflicting owning module in an isolated compiler process and determines whether an exact, integrity-checked final-LCNF compiler product can be transported and rehydrated by a separate assembler process without source recompilation, plugin overlap, or generated-name seeding. The upstream CompactedRegion path succeeds for trusted-local same-toolchain producer/consumer children with full identity, body and interface checks. It is not an untrusted/cross-version/durable interchange format. |
| `LCNF-TRUSTED-LOCAL-CAPTURE-WORKER` | W7/integration | VBP module-product closure | released | worker assembly `fb08ada2`, handoff `4db558a3` | Builds one bounded, trusted-local worker/assembler pair for the conflicting renderer and `VersoManual.Basic` owners. Each child receives exact source/setup/plugin/toolchain identity, produces or consumes only a validated CompactedRegion product, and keeps loaded regions alive through assembler use. The exact two-module product passes with 158 bodies/45 signatures and no overlapping native images. It remains same-toolchain/schema/local-only and is not a durable or untrusted protocol. |
| `LCNF-ISOLATED-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP renderer source closure | absorbed | worker basis `fb08ada2`; continuation `ROOT-W7-20260915-032` | The fixture-only owner-worklist request is absorbed into the continuing end-to-end renderer investigation. Its trusted-local worker/assembler invariants remain required, but a diagnostic is no longer a mandatory approval boundary. |
...
| `FLOAT32-RESIDENT-BOX-REFINEMENT` | W7 | W6 concrete runtime/proofs | active | executable W7 helper `1697038a`; isolated W6 descriptor contract `ca25935e`; proof boundary `48e340b7`; tracked handoff `b605cd9d` | The stable descriptor is now promoted and proved through allocation, physical decoding, resolver, and compiler extraction: Float32 marker 6 has four payload bytes plus zero padding, Float marker 7 has eight payload bytes, and both are heap-only. The remaining work connects the actual installed production box/unbox bodies to those bit-exact contracts, including malformed-layout guards, exact trace/store, and caller-tail preservation. W7's helper names, bytes, allocator, ownership, ABI, and source behavior remain fixed. |
| `USIZE-BOXING-UPSTREAM-ALIGNMENT` | integration/W7 | semantic LCNF runtime, LCNF ElimDead proof, W6 concrete layout/proofs, W7 resident scalar boxing, validation fixtures | released | semantic contract `1db0b79d`; W7 helper `9c896a1d`; LCNF proof `d78d128c`; W6 functional head `0360367a`; exact clean checkpoint `de7c03ab`; bug card `FIR-BUG-impure-none-usize-box-tagged` fixed | Aligns FIR with upstream Lean's unconditional `lean_box_usize` allocation and constructor-only `lean_unbox_usize`. Every payload now uses the existing 40-byte owned scalar-box layout, while physical tagged words are rejected. Small and maximal source-generated boxes follow the same release trace. The semantic runtime, LCNF proof, W6 concrete refinement, zero-import W7 helper, full native/LCNF/V8 triangle, Talos cone, and deterministic artifact gate are linked and green. Other scalar policies, layout constants, semantic ABI signatures, allocator, ownership, and symbolic Wasm remain unchanged. |
...
| `W6-PRECISE-DIRECT-CALL-RESULT` | W6/integration | W6 compiler-correctness proof | released | functional `99a3c03e`; tracked handoff `ecbfed6b`; bug `FIR-BUG-wasm-none-direct-call-validation-result-kind-drift` fixed | Records the effective compiler-selected callee result kind in `DirectInternalCallSite` and retains it through the complete structured validation stack. The public source annotation is related separately, so refined object-family calls no longer require a false exact-kind equality. No production code, shared validator, runtime, ABI, layout, helper, symbolic Wasm, or ownership behavior changes. |
| `WASM-IMPURE-BINDER-HYGIENE-VISIBILITY` | integration/W7 | W6 compiler correctness, W7, validation | released | W7 contract `818a1be1`; W6 functional repair `1d44bfe5`; tracked handoff `e7554e0f`; operational threads `W6-W7-20260820-021` and `W7-W6-20260821-003` | Retains `Program.ImpureHygienic` as the sole admitted invariant while replacing its opaque partial binder traversal with a total, proof-visible shared traversal and exposing the existing invariant through `WasmSupported`. Declaration-wide pairwise freshness and transparent let/join/case equations are kernel-visible. W6 consumes the leading hygiene conjunct in supported-body and parameter-uniqueness proofs and updates the exact raw case simulation to globally fresh `r`/`u` alternative binders. Forced Talos recompilation exposed and repaired a third stale conjunction projection beyond the two reported consumers. No Wasm-only certificate, ABI, layout, lowering behavior for valid compiler LCNF, runtime, resident-helper, or artifact contract changes. W7 and validation rebase on the released stack before dependent work. |
...
| `ILLUMINATE-NAT-MOD-MACHINE` | integration | W7, W6 | released | symbolic unsigned i32 remainder; W7 consumer `8cb9cd82`; W6 proof functional `5653cd51`, tracked `7851e481` | Supplies unsigned i32 remainder to the generic two-immediate `Nat.mod` branch while retaining the arbitrary-precision checked fallback. The installed immediate path is now contract-proved for both zero and nonzero divisors, with exact store and caller-tail preservation. |
