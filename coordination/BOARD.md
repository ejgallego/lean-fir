...
| `UINT64-BOXING-UPSTREAM-ALIGNMENT` | integration/W7 | semantic LCNF runtime, W6 concrete layout/proofs, W7 resident scalar boxing, validation fixtures | active | pending; bug card `FIR-BUG-impure-none-uint64-box-tagged` | Align FIR with upstream Lean's target representation: `UInt64` is always an ordinary heap box, while FIR currently admits a tagged-small-value branch through the generic boxing predicate and resident helper. Final LCNF may consequently issue an unchecked reference decrement that traps on FIR's tagged value. Isolate the shared representation correction before dependent runtime, proof, or fixture work; audit `UInt32` target dependence and `USize` separately rather than conflating their policies with UInt64. No workaround is accepted. |
