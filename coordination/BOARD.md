...
|---|---|---|---|---|---|
| `QUIET-COMMAND-FAILURE-STATUS` | tooling/root | all quiet command callers and CI | active | request `ROOT-TOOLING-20260916-001`; no fix checkpoint | Root/W7 reproduce a false-success exit: quiet-run prints FAIL but captures status after the if statement and returns 0. Tooling owns a minimal main-based fix with direct negative regression, retaining concise output/full failure logs/FIR_VERBOSE and original status. Narrow lease: `scripts/quiet-run.sh`, focused tooling unit test and `tooling/Makefile` registration. No broad build/CI refactor or weakened 4.34 audit expectations; root integrates one clean green handoff. |
