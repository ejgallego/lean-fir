...
| `LCNF-PROCESS-CAPTURE-TRANSPORT` | W7/integration | VBP module-product closure | active | decision `ROOT-W7-20260915-026` | Tests one real conflicting owning module in an isolated compiler process and determines whether an exact, integrity-checked final-LCNF compiler product can be transported and rehydrated by a separate assembler process without source recompilation, plugin overlap, or generated-name seeding. Existing serialization/encoding surfaces are preferred; an unavailable safe representation is a reportable stop, not permission to invent a lossy format. The experiment does not resume recursive closure or lower/link/emit Wasm. |
