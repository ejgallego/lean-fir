...
| `LCNF-TRUSTED-LOCAL-CAPTURE-WORKER` | W7/integration | VBP module-product closure | active | decision `ROOT-W7-20260915-028` | Builds one bounded, trusted-local worker/assembler pair for the conflicting renderer and `VersoManual.Basic` owners. Each child receives exact source/setup/plugin/toolchain identity, produces or consumes only a validated CompactedRegion product, and keeps loaded regions alive through assembler use. It must demonstrate two-module product assembly without overlapping native images, then stop. No remote/untrusted import support, general durable format, cache reuse policy, recursive worklist continuation, lowering/linking/Wasm or runtime/ABI change is implied. |
