...
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | released | Rebased tracked checkpoint `7851e481`, functional head `5653cd51`, closes the production-installed immediate `Nat.mod` path with one case-independent theorem. Canonical immediate operands return the exact encoding of `left % right`, including Lean's `left % 0 = left`, while preserving the Wasm store and caller stack tail. | Additive W6 proof interface over the accepted helper. No source semantics, lowering, emitted body/signature, ABI, layout, ownership, arbitrary-precision fallback, symbolic Wasm, or artifact changed. The focused 3,123-job cone, root gate, and combined 3,174-job Talos cone pass. |
| W7 generation | generation owner | `wasm/generation` | released | Tracked checkpoint `640f30c1`, functional head `6725cf9b`, factors the upstream-shaped tagged-word decision fast path across `Nat.decEq`, `Nat.decLt`, and `Nat.decLe`, for primitive and post-resident targets. Mixed, promoted, and arbitrary-limb operands retain the checked resident fallback and UInt8 result lane. | Generation-only optimization over stable helpers. Four balanced campaigns improve by 3.4–4.9% (68/80 paired wins); the raw complete module grows by 18,303 bytes (+5.0%) because about 122 callers inline the branch. Full root, 3,174-job Talos, deterministic package, reclamation, concrete, and browser gates pass. No helper signature, semantic ABI, layout, ownership, import, export-name, or memory contract changed. |
...
| Heap-only resident `Float32`/`Float` box/unbox | W7 functional `1697038a`; tracked handoff `1d723b62` | accepted floating semantic/concrete stack through `8a8d1387` | generation-ready | W6 owner for descriptor promotion and production-helper refinement | resident Float Wasm `fa806f59e3c1c0e56d52a6fe35fb5a3a5496f7e09b7183084b2a13af697a6c58`; manifest `94926697892f084f9c2425130f239e8d1bb20ae1f102db676376e3ee7c5ba1ac` |
...
| `FLOAT32-RESIDENT-BOX-REFINEMENT` | W7 | W6 concrete runtime/proofs | open | executable W7 helper `1697038a`; accepted semantic/concrete base `8a8d1387` | Promote the stable executable descriptor (`Float32 = 6`, four payload bytes plus zero padding; `Float = 7`, eight payload bytes) into the W6 proved resident-box table and connect the installed production helpers to exact bit-level box/unbox refinement. W7's helper names, bytes, allocator, ownership, ABI, and source behavior remain fixed while the theorem catches up. |
