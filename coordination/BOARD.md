...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main contains structured symbolic Wasm loops, Talos lowering, stack-safe resident String walkers, and ledger-aware matching through failed-token reuse at `aae41229`. Validation head `6818e6dd` is based on that boundary and adds true Lean `IO` return/error comparison without requesting compiler work | Six isolated shared-contract domains remain queued: float representation, closure ownership, argument aliases, process termination, effectful native-oracle actions, and source-entry result boundaries. The last two are validation infrastructure; W7/V8 consumes them only when compiler-generated IO cases are ready. The umbrella check remains gated by the queued `takeClosureApplication` proof consumers; the retained V8 rebuild is independently gated by W7's exhaustive float manifest cases |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Ledger-aware failed-token reuse matching is integrated at semantic boundary `aae41229` after the literal, constructor, partial-application, and box checkpoints; the branch is one coordination-only main commit behind while a new owned continuation edits `ElimDeadExamples.lean` and `ElimDeadMachineRel.lean` | Continue the owned ledger-aware dispatcher and exact ownership invariant, adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172`, and prove the queued `takeClosureApplication` consumers |
