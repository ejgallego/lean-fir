...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main contains structured symbolic Wasm loops, Talos lowering, stack-safe resident String walkers, and ledger-aware matching through all internal invocation transitions, layout/tag writes, the full reference-count/delete and reset/reuse families, plus named/value request suspension and allocation-capable response resumption at `63750e62`; this board update records that proof boundary. Validation head `1762979a` remains based on the earlier semantic boundary and compares genuine native Lean stdout/stderr without requesting compiler work | Eight isolated shared-contract domains remain queued: float representation, closure ownership, argument aliases, process termination, effectful native-oracle actions, source-entry result boundaries, Lean IO-error normalization, and native source-stream capture. The last four are validation infrastructure; W7/V8 consumes them only when compiler-generated cases are ready. The umbrella check remains gated by queued proof-owned `takeClosureApplication` consumers; a fresh V8 rebuild is independently gated by W7's exhaustive float manifest cases |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | The ledger-aware foreign contract at `63750e62` requires response renaming extension and the exact resumed target owner table; named and closure external calls preserve the suspended ledger and return their larger allocation-capable response relation. All individual internal and external families needed by the ledger dispatcher are now available | Assemble the unified ledger-aware dispatcher, then connect it to the entry-indexed ownership invariant. The focused validation-stack consumers remain `SimpCaseRelation.lean:427` and `AlphaEqvCode.lean:2209,2358-2360` |
