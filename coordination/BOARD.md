...
| `VBP-FIR-RENDERER-VERSION-ALIGNMENT` | W7/integration | designated VIR widget-backend consumer owner | generation-ready | diagnostic `d504fb4a`; functional `65eeaa61`; immutable handoff `4776f86e` on official 4.34 child | Immutable VBP `92db6325` / VIR `36d26bc2` sources, manifest and five hashes verify; default JSON capture/resident linking succeeds (52 modules/1,985 bodies/18 native providers) with a 2,219,310-byte module, owned memory and exactly 13 typed VIR-only imports. The three-operation delta from the old adapter—`Js.Object.get`, `Js.UndefinedOr.isUndefined`, and `JsValue.toBool`—now executes against real compiled Wasm. `Object.get` preserves raw `undefined` and distinct `null`; the actual compiled `IO Bool` probe establishes Lean Boolean payloads `1`/`3`, rather than guessing a JS-resource layout. All 13 imports/signatures, valid/malformed JSON, property identity, provider-error identity, later render/callback survival, disposal, and structural poison controls pass. This is an official 4.34-child generation result, not a `main` migration or browser acceptance. Matched current-source SSR/Chromium and live-widget qualification remain consumer-owned. |
| `VBP-FIR-CURRENT-GENERIC-ADAPTER` | W7/integration | current renderer alignment | released | request `ROOT-W7-20260916-007`; functional `65eeaa61`; handoff `4776f86e`; consumer review `VBP-FIR-TYPED-ADAPTER-FOLLOWUP-20260916-001` | Adds only the three signature-checked current generic VIR bindings needed by the retarget. `Object.get` returns the exact raw JS property value—including a rooted `undefined` and distinct `null`—while retaining its borrowed parameter behavior; `UndefinedOr.isUndefined` and `JsValue.toBool` use their current strict provider behavior and construct the actual compiled Lean Boolean/result representation at the ABI edge. The compiled producer and independent consumer establish payloads `1`/`3`; swapped-value controls reject the wrong representation. Ordinary provider errors preserve identity and leave session/callbacks live, while malformed ABI values poison the session. Actual compiled-Wasm gates cover property/missing/undefined/null, both Booleans and non-Boolean rejection, valid/malformed JSON, callbacks/later renders after errors, provider identity, and disposal. It is not a generic dispatch framework, permanent host ABI/pin adoption, widget shim, consumer edit, package/main migration, or performance result. |
