...
| `VBP-RENDERER-DOCUMENT-JSON-ENTRY` | W7/integration | designated VIR widget-backend consumer owner | active | request `ROOT-W7-20260915-043`, diagnostic `60e087c9`, now resumed only through `PTR-ADDR-UNSAFE-RESIDENT` | Adds one renderer-only, synchronous `renderDocumentJson(string)` adapter entry: compiled Lean decodes JSON using the existing `VersoBlueprint.Experimental.VirPreview.Document.decode`, then calls actual `Renderer.render` with default options and returns the genuine React node. This is deliberately not a JS schema copy, general Document/Options transport, Preview/RPC wire-format migration, or options transport (attributes can contain JS values). Valid and malformed input must be covered; malformed JSON is visibly failing, never an empty successful render. W7 records the additional decoder closure/import frontier and composes the host-error alignment instead of duplicating it. The shared owner independently confirmed error-aligned `8ec770fd` in real React SSR and Chromium: 20 same-root updates, callback/unmount behavior, and ordinary provider-error identity/session/escaped-callback recovery all pass; an interactive title-only demo also passes. Full-FLT before/after `Document.encode` inputs are captured unchanged (6,598,303/6,598,333 bytes) with the frozen VIR control matching `c4430bfe`, but this is preparation—not FIR JSON acceptance, a full-document claim, timing evidence, or an optimization assignment. One designated VIR widget-backend owner owns both consumer integrations, comparable workload/acceptance boundaries and differential reports; the FIR preview fork owns later FIR optimization only, from that owner's measured bottleneck. Rich extension/details fixtures remain the shared consumer owner's follow-up. |
| `PTR-ADDR-UNSAFE-RESIDENT` | W7/integration | JSON renderer entry, later W6 refinement | active | diagnostic `60e087c9`; request `ROOT-W7-20260916-002` | The source-complete JSON renderer closure adds one unavoidable native boundary: borrowed `ptrAddrUnsafe : erased × tobject → usize`, used by Lean's private `Name.quickCmpImpl` before ordinary name comparison. Installed Lean 4.34 maps it to `lean_ptr_addr(a) = (size_t)a`; no exported Lean provider exists. W7 may add exactly the Wasm32 resident implementation: preserve borrowed ownership, use the existing object pointer word, zero-extend it into FIR's existing `USize` lane, and internalize the exact declaration with signature checks. No host import/fallback, allocation, retain/release, bespoke comparison algorithm, USize/layout/ABI/semantic change, VBP/VIR edit, or automatic proof claim. If the current lowering surface cannot express this without a shared-contract change, stop for root rather than duplicate a local model. The diagnostic's other 16 primitive boundaries already internalize; the twelve typed VIR imports remain unchanged. |
