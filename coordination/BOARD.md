...
| Compiler-native Wasm | integration owner | `wasm/lcnf-c` | parked | Landed checkpoint `2760e3e0` adds optimized final-LCNF-to-C-to-Wasm generation: a narrow freestanding profile, the primary full-runtime Emscripten profile, frozen WASI Preview 1 ABI 3 coverage, verified deployment manifests/loaders, and native-versus-Emscripten evidence collection. `docs/wasm-artifact-generation.md` distinguishes this compiler-native path from W7's FIR-native symbolic artifact package | No shared semantic contract changed. The lane consumes final impure LCNF and Lean's C ABI without changing the symbolic Wasm, W6 concrete-runtime, or W7 resident-runtime surfaces. Resume with controlled sampled profiling before accepting a generated-C or runtime optimization |
