...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Functional head `25cc1ceb`, recorded by coordination head `d6c4b701`, adds a direct source-owned exact contract for the checked concrete reset/reuse program. Reset readiness now consumes the maintained source carrier: reset preserves `HeapOwnershipBelowFrontier`, whose generic frontier theorem replaces a concrete post-reset heap enumeration. The endpoint reaches whole-program `LoweringCorrect` under the source foreign-ownership law. The worktree has active next-slice edits and is two commits behind functional `main`. | Generalize the ownership-consuming readiness interface from finite fixtures to static compiler typing/local-shape preservation, starting with a reusable reset/reuse invariant that does not enumerate the source execution graph. After lane 4 rebases its shared contracts, also re-probe the focused `SimpCaseRelation` and `AlphaEqvCode` consumers rather than assuming their historical locations remain exact. |
