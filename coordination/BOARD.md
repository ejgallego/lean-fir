...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through the source-only target-allocation-ledger checkpoint `c49fbad`. Validation head `4e716db` is rebased onto the preceding documentation checkpoint and advances the ready stack to 667 native/LCNF cases with evidence-derived semantic coverage floors | Three isolated shared contracts are ready but not yet eligible as one stack for `main`: the older float/closure consumers and the argument-alias consumer below must adapt first. Later validation-only coverage/effect/mutation commits do not add consumer work and may ride the same green integration stack |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Integrated head `c49fbad` adds an extensible target-allocation ledger, proves that its source-only criterion is exactly address-map absence, connects it to write/reuse/reset readiness, and exercises it with a retained allocation in a non-empty target | Thread the ledger through an entry-indexed exact compiler ownership invariant and derive edge-specific ledger facts from it. Afterwards adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172` without adding floating cases to `scalarFromType`; separately prove `takeClosureApplication` preservation/relatedness for `SimpCaseRelation.lean:427` and `AlphaEqvCode.lean:2209,2358-2360` |
