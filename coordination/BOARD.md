...
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | released | Ready mailbox and landed head `c82554fb`, functional head `f215c995`, on base `fb18c01f` derives exact generated named-call indices and makes the production call law consume the nested finite hereditary source theorem. | The proof-only export boundary retains existing phase name uniqueness; no semantic ABI/runtime contract or certificate premise is added. Full root and 3,125-job Talos gates pass; next is `DirectHereditaryGeneratedDeclarationInduction`. |
