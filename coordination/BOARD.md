...
| `VBP-FIR-RENDERER-VERSION-ALIGNMENT` | W7/integration | designated VIR widget-backend consumer owner | blocked | diagnostic functional `70e222a1`; checkpoint `d504fb4a`, base `a48bbb4a` | Immutable VBP `92db6325` / VIR `36d26bc2` sources, manifest and five hashes verify; default JSON capture/resident linking succeeds (52 modules/1,985 bodies/18 native providers) with a 2,219,310-byte module, owned memory and 13 VIR-only imports. The old fixed 12-import adapter rejects the exact new three-operation delta: `Js.Object.get`, `Js.UndefinedOr.isUndefined`, `JsValue.toBool`; text/tag calls disappear and ten shared signatures match. No execution, consumer, runtime or pin change is claimed. The live shell forwards server `Document.encode` directly, but feature parity and automated widget qualification remain consumer-owned. |
| `VBP-FIR-CURRENT-GENERIC-ADAPTER` | W7/integration | current renderer alignment | active | request `ROOT-W7-20260916-007`, after diagnostic `d504fb4a`; consumer review `VBP-FIR-TYPED-ADAPTER-FOLLOWUP-20260916-001` | Adds only the three signature-checked current generic VIR bindings needed by the retarget. `Object.get` returns the exact raw JS property value—including a rooted `undefined` and distinct `null`—while retaining its borrowed parameter behavior; `UndefinedOr.isUndefined` and `JsValue.toBool` use their current strict provider behavior and construct the actual compiled Lean Boolean/result representation at the ABI edge. W7 derives that representation with compiled probes rather than a guessed JS object layout, preserves ordinary-provider-error identity/session/callback behavior and fail-closed structural errors, and reports the resulting import inventory. Actual compiled-Wasm gates cover property/missing/undefined/null, both Booleans and non-Boolean rejection, valid/malformed JSON, callbacks/later renders after errors, provider identity, and disposal. It is not a generic dispatch framework, permanent host ABI/pin adoption, widget shim, consumer edit, package/main migration, or performance result. |
