...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through the elimDead pinned-pass conformance checkpoint `66ae13d`. Validation head `7c8c4b8` remains based on the earlier ownership checkpoint and advances the ready stack to 664 native/LCNF cases with evidence-derived semantic coverage floors | Two isolated shared contracts are ready but not yet eligible for `main`: the pass-proof, W6, and W7 consumers below must adapt first. The later validation-only coverage/effect/mutation commits do not add consumer work and may ride the same green integration stack |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Integrated head `66ae13d` adds an 11-row actual-pass/policy conformance matrix, kernel-checks representative accepted and rejected rows, and lifts the accepted allocation row into the strict compiler correctness package; no new discrepancy was found | Generalize closed fixtures into static-to-dynamic ownership laws for deleted writes, resets, and concrete reuse. Then adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172` without adding floating cases to `scalarFromType`; separately prove `takeClosureApplication` preservation/relatedness for `SimpCaseRelation.lean:427` and `AlphaEqvCode.lean:2209,2358-2360` |
