...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | active | failed setup `c8ba455a`; frozen source `2c1ef0f6`; setup authority `6bbd2b32`; request `ROOT-W7-20260916-015` | Assesses whether VBP's new once-per-runtime `createEncodedDocumentComponent (mathComponent? := none)` can be captured through the current typed FIR adapter, preserving native `Document.encode` String input and session-owned controls/identity/content. VBP's immutable source archive binds all 2,534 source file hashes/modes and captured VBP/Verso dirty changes plus resolved dependency sources; its setup authority binds the 256 source rows, 99 installed-module boundaries and 18,277 exact Lean 4.34-rc2 toolchain files. The original Git manifest is rejected as a capture setup because Lake automatically rematerializes package directories, replacing frozen dirty `Verso/Parser.lean`; its 254 artifacts/14 plugins/cross-workspace paths remain audit-only. W7 now derives and hashes a private path-only package mapping from the frozen dependency paths, with active fetch prevention, before regenerating setup. After that gate passes, it attempts only the `none` capture/lower/link inventory, recording every generated/consumed config, artifact and plugin identity. It stops at the first missing generic primitive, typed import, SDK input or reproducibility prerequisite, or reports the exact typed delta if feasible; optional native math follows only after this target. No renderer fork, adapter/ABI/options-schema/transport change, source/pin mutation, consumer acceptance, timing result, main migration, W6 work, or change to preserved `4776f86e`/`a48bbb4a` artifacts. |
