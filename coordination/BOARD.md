...
- Integration has split the required Lean float contract from the unrelated
  ...
  The pre-boundary candidate passed `make check` with 593 native/LCNF cases,
  all 581 V8 cases, and 1,764/1,764 comparisons; the rebased head passes
...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Functional proof head `3c071252` and documentation checkpoint `e6c60837` are on `main`; clean candidate `6ed38819` is one commit ahead and generalizes target-prefix owners. The combined source-owned/ledger simulation remains checked through `LoweringCorrect`. The target side of the retained-prefix contract uses a step-preserved three-phase allocation/control invariant rather than any finite target execution graph. Its post-allocation facts now project through the program-independent `TargetSingletonLiveReturnAt` interface; a generic ledger theorem excludes any distinct source owner from that target prefix for both reset and reuse. The integrated gate passes 622 unique cases, 1,784/1,784 comparisons, all coverage floors, 84 bug cards, and exactly one registered trusted axiom. | Publish the required handoff for `6ed38819`; then adapt `ElimDeadRuntimeRel.lean:8909-9218` on `integration/float-runtime-contract` to the shared `boxUsesTaggedRepresentation` predicate. Continue generalizing target-ledger exclusion and replace the remaining finite source reset/reuse classification with generic local operation-shape/ownership premises. |
