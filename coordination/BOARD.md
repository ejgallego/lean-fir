...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through closed reset/PAP/box proof checkpoint `6104100`; the isolated `FLOAT-SCALAR-RUNTIME` producer remains ready at `c901e0d`, and its validation stack now reaches the carded tagged-float repair and 655-case checkpoint `9be53cd` | The validation stack exposes one exact pass-proof adaptation plus the already-recorded W6/W7 exhaustive consumers; all must be green before the shared contract and dependent fixtures land on `main` |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Complete exact machine dispatch plus closed whole-program proofs for writes, failed/concrete/owned-child reset-reuse, PAP closure allocation, and heap-backed boxing are landed through `6104100`; a new proof slice is in progress | After rebasing onto validation contract commit `610dd0c`, extend `scalarFromType_ok_eq_immediate` in `Fir/LeanIR/Passes/ElimDeadRuntimeRel.lean` with `float32` and `float` scalar cases; its focused dependency cone reports this single unsolved goal at line 4120 |
...
| Validation | validation owner | `validation/float-corpus` | active | Clean head `9be53cd` records and fixes `FIR-BUG-impure-none-tagged-float-unbox`, adds tagged and heap boxing controls plus ten mixed float/object-layout projection and copy-on-write cases, and compares all 655 native and LCNF observations equal in evidence run `ee6a1eb3b06605e7b454c703895b6bd6b16a80665c60b51893faa4565c9bcfe0` | Runtime contract commit `610dd0c` adds tagged `float32`/`float` decoding. Root build, examples, 117 harness tests, 655-case validation, and nine direct-machine cases pass; the full gate stops only at the three queued W7 manifest matches, while the separate pass-proof cone has the one adaptation recorded above |
