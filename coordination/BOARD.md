...
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, FIR lower/link | active | provider linking `fd77cba8`, handoff `ecc2d328`; continuing decision `ROOT-W7-20260915-035` | One outcome-driven investigation establishes whether real VBP `Renderer.render` compiles through FIR using its faithful upstream Lean/Lake module contexts. It follows one deterministic owner worklist from the renderer through complete source closure, FIR lowering, and a checked artifact/import frontier; an external native/VIR requirement remains explicit rather than fabricated. Each owner retains exact source/setup/plugin/toolchain/body/interface identity and the accepted worker boundary prevents conflicting native images from coexisting. The generic, fail-closed `@[extern]` → exact upstream `@[export]` linkage is accepted: it verifies unique symbol metadata, final-LCNF interface and owning captured body, then emits ABI-checked forwarders; it closes 16 renderer edges without custom String helpers. W7 may continue existing-contract coverage for arbitrary-limb naturals and exact established aliases, but no support may change layout, helper signature, ownership/effect rule, host import, or consumer. It reports concise traversal progress and stops only for a genuine shared semantic/ABI contract, consumer-owned change, unrepresentable upstream boundary, or the final result. No reset, generated-name seeding, weakened identity check, plugin suppression, durable/untrusted transport, W6 work, or implicit runtime/consumer change is authorized. |
