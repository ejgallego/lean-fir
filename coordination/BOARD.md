...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main is green through accepted W7 float functional head `2b4d9d23` and coordination landing `3c437f2d`. The root gate covers 633 native/LCNF cases, 9 direct cases, and 601 native/LCNF/V8 cases: 642 unique cases and 1,844 equal comparisons. | Float representation, reinterpretation, manifest transport, and the zero-import package are released. Seven isolated shared-contract domains remain queued: closure ownership, argument aliases, process termination, effectful native-oracle actions, source-entry result boundaries, Lean IO-error normalization, and native source-stream capture. Rebased closure commits `d392e194`/`2ed6deb4` remain outside `main` until pass-proof and W6 adaptations are green. |
...
| W7 generation | generation owner | `wasm/generation` | released | Accepted functional head `2b4d9d23` contains the seven W7 float commits plus one split-boundary test correction. It executes exact Float32/Float64 arguments and results through the integer-lane facade and publishes the zero-import `prettyM` package. The original branch is rebased and clean at `2ed6deb4`, exactly two queued closure commits ahead of `main`. | The accepted slice passes `make check`, all 3,130 Talos jobs, and the full artifact/browser/native-oracle gate. Closure commits `d392e194`/`2ed6deb4` remain isolated; no further W7 float rebase is pending. |
...
| `CLOSURE-APPLICATION-OWNERSHIP` | integration/validation | pass proof, W6, W7, validation | active | rebased contract `d392e194`; W7 adapter transfer `2ed6deb4` | Matches Lean's `lean_apply_*` boundary: an exclusive closure transfers fixed arguments and is freed non-recursively; a shared closure drops one reference and retains each fixed heap argument. The clean two-commit W7 queue demonstrates the executable behavior but fails pass-proof `AlphaEqvCode` cases and W6/Talos `Correctness.Semantics` frame cases. Keep both commits out of `main` until the pass-proof and W6 owners provide separate green adaptations. |
