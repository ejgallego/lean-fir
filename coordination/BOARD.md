...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Integrated head `668ec6c` makes the conservative nullary policy fail-closed and executable, proves its exact graph/program certificates, and reaches the direct lowering endpoint from one checked equation plus the independent runtime certificate | Next prove the ownership contract-to-`ElimDeadRuntimeAdmissibility` bridge. Then adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172` without adding floating cases to `scalarFromType`; separately prove `takeClosureApplication` preservation/relatedness for `SimpCaseRelation.lean:427` and `AlphaEqvCode.lean:2209,2358-2360` |
