...
| `VBP-FIR-CURRENT-GENERIC-ADAPTER` | W7/integration | current renderer alignment | active | request `ROOT-W7-20260916-007`, after diagnostic `d504fb4a` | Adds only the three signature-checked current generic VIR bindings needed by the retarget. `Object.get` returns the exact raw JS property value—including a rooted `undefined`; `UndefinedOr.isUndefined` and `JsValue.toBool` use their current strict provider behavior and construct the actual compiled Lean Boolean/result representation at the ABI edge. W7 derives that representation with compiled probes rather than a guessed JS object layout, preserves ordinary-provider-error identity/session/callback behavior and fail-closed structural errors, and reports the resulting import inventory. It is not a generic dispatch framework, permanent host ABI/pin adoption, widget shim, consumer edit, package/main migration, or performance result. |
