...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | active | frozen source `2c1ef0f6`; setup authority `6bbd2b32`; request `ROOT-W7-20260916-013` | Assesses whether VBP's new once-per-runtime `createEncodedDocumentComponent (mathComponent? := none)` can be captured through the current typed FIR adapter, preserving native `Document.encode` String input and session-owned controls/identity/content. VBP's immutable source archive binds all 2,534 source file hashes/modes and captured VBP/Verso dirty changes plus resolved dependency sources; its setup authority binds the 256 source rows, 99 installed-module boundaries and 18,277 exact Lean 4.34-rc2 toolchain files. W7 must verify both manifests, privately extract the source, and regenerate—not reuse—the setup with only that source closure and exact external toolchain. Historical `e023c128` artifacts/plugins and cross-workspace paths are audit-only. After the private setup gate passes, W7 attempts only the `none` capture/lower/link inventory, recording every generated/consumed setup, artifact and plugin identity. It stops at the first missing generic primitive, typed import, SDK input or reproducibility prerequisite, or reports the exact typed delta if feasible; optional native math follows only after this target. No renderer fork, adapter/ABI/options-schema/transport change, source/pin mutation, consumer acceptance, timing result, main migration, W6 work, or change to preserved `4776f86e`/`a48bbb4a` artifacts. |
