...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main contains structured symbolic Wasm loops, Talos lowering, stack-safe resident String walkers, and the unified ledger-aware non-lockstep LCNF dispatcher at `9d290b7f`: every exact internal family, yielded restoration, named/value invocation, and allocation-capable external response returns exact target allocation history; this board update records that proof boundary. Validation head `1762979a` remains based on the earlier semantic boundary and compares genuine native Lean stdout/stderr without requesting compiler work | Eight isolated shared-contract domains remain queued: float representation, closure ownership, argument aliases, process termination, effectful native-oracle actions, source-entry result boundaries, Lean IO-error normalization, and native source-stream capture. The last four are validation infrastructure; W7/V8 consumes them only when compiler-generated cases are ready. The umbrella check remains gated by queued proof-owned `takeClosureApplication` consumers; a fresh V8 rebuild is independently gated by W7's exhaustive float manifest cases |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | `LedgerBinderReadyReachableMachineReadyAt` aligns exact compiler provenance, runtime roots, and owner ledger under one renaming; `match_codeStep_ledger`, yielded restoration, and `SomeLedgerBinderReadyReachableMachineRelated.matchStep_of_ready` exhaust internal and external source steps at `9d290b7f` | Define and discharge the entry-indexed exact ownership contract that supplies aligned ledger readiness for arbitrary compiler-selected edges. The focused validation-stack consumers remain `SimpCaseRelation.lean:427` and `AlphaEqvCode.lean:2209,2358-2360` |
