...
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Committed head `6862a6d2` is one commit behind functional `main`; the worktree has two uncommitted proof-owned edits in `ElimDeadExamples.lean` and `ElimDeadMachineRel.lean`. The landed source-owned/ledger simulation remains checked through `LoweringCorrect`, and the target-prefix interface exposes proof-visible source owners for arbitrary mapped prefixes. Main's post-rebase gate passes 642 unique cases, 1,844/1,844 comparisons, all coverage floors, 84 bug cards, and exactly one registered trusted axiom. | Finish and checkpoint the proof-owned edits, then rebase onto `c05e85d9` before handoff. Continue deriving `TargetMappedOwnerPrefix` from arbitrary exact checked compiler residuals and replacing the remaining finite source reset/reuse classification with generic operation-shape/ownership premises. When the queued float runtime contract lands, separately adapt the proof-owned `ElimDeadRuntimeRel.lean:8909-9218` consumer to `boxUsesTaggedRepresentation`. |
