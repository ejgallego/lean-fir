...
|---|---|---|---|---|---|
| Integration | integration owner | `integration/wasm-lanes` | active | Integrated W7's closed resident `prettyM` runtime through `071fcbc`, exact-provenance pass proofs through `a29d480`, and the complete exact signed validation family as `5305358`; subsequent lane releases remain independently queued | Stable W6/W7 contracts are unchanged; validation consumes the released semantic ABI without creating a new cross-lane contract |
...
| Validation | validation owner | `validation/interpreter-corpus` | active | The complete exact signed Int8/Int16/Int32/Int64/ISize family and all 20 directed conversions are landed through `5305358`: 581 source cases, 590 unique cases, 1,752 comparisons, and zero native/LCNF/V8 findings; the next corpus-gap audit is active | No shared-contract change; consumes the signed semantic Wasm ABI released at `504d1ad`, while compiler-generated Wasm adoption remains a downstream W7 user of the same fixtures |
