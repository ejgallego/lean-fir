...
- `USIZE-BOXING-UPSTREAM-ALIGNMENT` is an active isolated contract candidate
  ...
  already below this candidate; LCNF and W6 still adapt the exact tagged-unbox
  and heap-only policy proof sites before W7 adds and ratchets the executable
  helpers. Do not confuse the provider-backed V8 result with resident-linker
  readiness.
...
| `USIZE-BOXING-UPSTREAM-ALIGNMENT` | integration/W7 | semantic LCNF runtime, LCNF ElimDead proof, W6 concrete layout/proofs, W7 resident scalar boxing, validation fixtures | active | bug card `93ed2510`; semantic/fixture candidate `73eb55cc` on base `3b96b2b3`; operational threads `W7-LCNF-20260826-005` and `W7-W6-20260826-020` | Aligns FIR with upstream Lean's unconditional `lean_box_usize` allocation and constructor-only `lean_unbox_usize`. Small and maximal source-generated boxes follow the same owned heap-release trace. The semantic runtime and provider-backed native/LCNF/V8 triangle are green. LCNF and W6 adapt the exact stale policy sites before W7 adds zero-import `.box/.unbox .usize` helpers. Other scalar policies, layout constants, semantic ABI signatures, and symbolic Wasm remain unchanged. |
