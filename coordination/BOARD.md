...
| `VBP-FIR-LIVE-WIDGET-SHELL` | designated VIR widget-backend consumer owner | VBP VS Code preview | active | consumer-only live observation, 2026-09-16 | The newer VBP RPC/shell can display the frozen FIR renderer live, but currently decodes Preview then re-encodes Document before FIR decoding. It uses default renderer options and is intentionally outside the matched renderer control: no timing, highlighting, KaTeX, live-edit qualification, producer change, pin change, or optimization conclusion is claimed. Automated live-widget edit qualification is a separate consumer task; FIR has no active producer lease here. |
