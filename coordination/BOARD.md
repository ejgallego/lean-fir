...
|---|---|---|---|---|---|
| Integration | integration owner | `integration/wasm-lanes` | active | Integrated W7's closed resident `prettyM` runtime through `071fcbc`, exact-provenance pass proofs through `a29d480`, and the complete exact signed validation family as `5305358`; the isolated `FLOAT-SCALAR-RUNTIME` producer is ready at `c901e0d` | The root Lean and pass-proof cones are green on the producer; W6 and W7 must adapt the exact exhaustive consumers recorded below before the contract stack can land on green `main` |
...
| `FLOAT-SCALAR-RUNTIME` | integration/validation | pass proof, W6, W7, validation | active | `c901e0d` | Adds bit-exact `float32Bits`/`float64Bits`, typed validation schemas, and LCNF codecs. Root Lean, source examples, pass proofs, 581 native/LCNF cases, and nine direct-machine cases pass. W7 must extend exhaustive matches in `Fir/Wasm/Emit/Manifest.lean` at lines 149, 255, and 280; W6 must extend `Fir/Wasm/Concrete/HeapRefinement.lean` at lines 35, 116, and 136. `make talos-check` fails only at those two owned modules. After their adaptations rebase onto the contract stack, validation adds native-oracle identity, conversion, arithmetic, comparison, exceptional-bit-pattern, boxing, and field-mutation cases, with V8 transporting raw bits rather than JSON numbers. |
