...
| `VBP-RENDERER-DOCUMENT-JSON-ENTRY` | W7/integration | designated VIR widget-backend consumer owner | active | request `ROOT-W7-20260915-043`, after `ROOT-W7-20260915-042` | Adds one renderer-only, synchronous `renderDocumentJson(string)` adapter entry: compiled Lean decodes JSON using the existing `VersoBlueprint.Experimental.VirPreview.Document.decode`, then calls actual `Renderer.render` with default options and returns the genuine React node. This is deliberately not a JS schema copy, general Document/Options transport, Preview/RPC wire-format migration, or options transport (attributes can contain JS values). Valid and malformed input must be covered; malformed JSON is visibly failing, never an empty successful render. W7 records the additional decoder closure/import frontier and composes the host-error alignment instead of duplicating it. VBP independently established real React SSR and Chromium success for the frozen title fixture `0c2eaa62` (including 20 DOM-identity updates, callback retention and unmount-before-dispose). One designated VIR widget-backend owner now owns both consumer integrations, comparable workload/acceptance boundaries and differential reports; the FIR preview fork owns later FIR optimization only, from that owner's measured bottleneck. Rich extension/details fixtures remain the shared consumer owner's follow-up. |
