...
|---|---|---|---|---|---|
| Integration | integration owner | `main` | active | Main contains structured symbolic Wasm loops, Talos lowering, stack-safe resident String walkers, ledger-aware matching through failed-token reuse at `d41f6a54`, and the effectful-native-oracle coordination checkpoint at `3ffaae85`. Validation head `fa8fdb96` is based on `cfb1daa8` and preserves all 673 native/LCNF cases while replacing the pure native-oracle callback with a delayed `IO` action | Five isolated shared-contract domains remain queued: float representation, closure ownership, argument aliases, process termination, and effectful native-oracle actions. The latter is validation infrastructure only and does not request compiler work. The umbrella check remains gated by the queued `takeClosureApplication` proof consumers; the retained V8 rebuild is independently gated by W7's exhaustive float manifest cases |
| Lean pass proof | pass-proof owner | `proof/simpcase` | active | Ledger-aware failed-token reuse matching is integrated at `d41f6a54` after the literal, constructor, partial-application, and box checkpoints; the suite has 590 unique cases, 1752/1752 equal comparisons, 4790 interpreter steps, zero findings, and exactly one registered trusted axiom | Continue with existing-address and external-response branches, then assemble the unified ledger-aware dispatcher. Afterwards define the entry-indexed exact ownership invariant, adapt `box` proofs to `boxUsesTaggedRepresentation` in `ElimDeadRuntimeRel.lean:8863-9172`, and prove the queued `takeClosureApplication` consumers |
| W6 runtime proof | W6 owner | `wasm/talos-runtime` | active | Committed head `a68c79fa` composes constructor-tag, object-field mutation, and erased-object-field correctness; the worktree has a W6-owned concrete-runtime and documentation continuation and is one main commit behind | Checkpoint the owned edit before rebasing. Extend the already-float-aware concrete relation with the missing exhaustive `HeapRefinement.lean` cases at 35, 116, and 136, then audit closure application against `takeClosureApplication`; W6 owns the concrete refinement, not the validation implementation |
| W7 generation | generation owner | `wasm/generation` | active | The stack-safe resident `prettyM` String walker and stress fixture are integrated on `main` at `7d9c1ad9` and `0e1a564a`; the clean generation branch is one coordination commit behind | Add the exhaustive `float32Bits`/`float64Bits` manifest consumers at `Manifest.lean:149,255,280` in the next generation slice. The current retained V8 rebuild stops exactly at those three missing cases. Consume `processTermination` only when admitting a compiler-generated real-engine exit case; do not implement lane-4 source-exit projection in W7 |
| Validation | validation owner | `validation/float-corpus` | ready | Clean head `fa8fdb96`, based on `cfb1daa8`, retains the earlier validation stack and isolates the effectful native-oracle action contract. Run `dc515e7ee7ed4047a0c1907ca600b2561eb5b155435953a36b37e582e2070222` preserves 673/673 observations equal; evidence `2a79303fed8b926b48982a76a7d979a6a9a5e2cef140c503f7dfa56efa8f8b77` verifies offline. The direct tier also preserves 9/9 at run `b45e7560863ab491b1f995f1feb62c82de28df32e4121a3d7b7c306f77de3f1c` | Shared contracts remain isolated at `4d1a830d`/`c6e8796a`, `8dff773f`, `4c1f1efc`, `13568541`, `cada180b`, and `fa8fdb96`. The new action shape lets native execution throw while post-run effect/stderr drains still execute, but no exception fixture depends on it yet. Harness tests are 126/126. `make check` still stops only at the queued `takeClosureApplication` proof consumer in `AlphaEqvCode.lean:2209,2358-2360` |
