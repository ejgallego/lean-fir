...
| `TOOLCHAIN-LEAN-4.34-COMPATIBILITY` | tooling/root | W7 and other user-project compiler consumers | active | official lane smoke `05a2e64e` on `tooling/lean-4.34` | Establishes the official FIR 4.34 compatibility branch and `.worktrees/tooling-lean-4.34`, pinned to `leanprover/lean4:v4.34.0-rc2` / `6a10ac8c`. Workers whose user-project Lake setup requires 4.34 use this lane or a track-owned child rebased on it, preserving the project’s actual Lake module setup. The truthful 4.34 CI smoke covers the pin, isolated cache, FIR library/examples and 163-function scalar Wasm execution; it is not the full gate. `lake build`, examples and scalar execution pass. Full `make check` remains red only because the version-indexed `generic-uint8-array-get` trace expects 4.33's `instInhabitedUInt8` while 4.34 uses `Array.get!Internal`; the 4.33 trust/Talos pins also remain outside the 4.34 policy. Tooling owns its pin, Lake/CI compatibility and migration evidence; `main` remains 4.33 until root deliberately accepts a migration. No expectation is weakened and no ABI/runtime/proof/consumer change is included. |
