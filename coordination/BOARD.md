...
| `VBP-FIR-ENCODED-DOCUMENT-OPTIONS` | W7/integration | designated VIR widget-backend consumer owner | active | prior inventory `c2945976`; snapshot identity `2c1ef0f6`; request `ROOT-W7-20260916-011` | Assesses whether VBP's new once-per-runtime `createEncodedDocumentComponent (mathComponent? := none)` can be captured through the current typed FIR adapter, preserving native `Document.encode` String input and session-owned controls/identity/content. VBP resolved the former provenance blocker with immutable archive `2c1ef0f6c7d78636af5f50384aafe6407282d1c4b06bece068d7ec49b1e4475b.tar.gz` (archive SHA-256 `41b88ae7…`), whose `identity.json` binds all 2,534 source file hashes/modes and captured VBP/Verso dirty changes plus resolved dependency sources. W7 verifies and privately extracts it under its own `.deps`, never consulting the mutable original tree, then attempts only the `none` capture/lower/link inventory. It stops at the first missing generic primitive or typed import, or reports the exact typed delta if feasible; optional native math follows only after this target. No renderer fork, adapter/ABI/options-schema/transport change, source/pin mutation, consumer acceptance, timing result, main migration, W6 work, or change to preserved `4776f86e`/`a48bbb4a` artifacts. |
