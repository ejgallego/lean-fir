...
| `VBP-FIR-RENDERER-VERSION-ALIGNMENT` | W7/integration | designated VIR widget-backend consumer owner | active | request `ROOT-W7-20260916-004`, base `a48bbb4a` | Retargets the existing synchronous compiled `Document.decode → Renderer.render` JSON bridge, for one diagnostic run only, from accepted VBP `c4430bfe` / VIR `9fafe9cf` to immutable VBP `92db6325` / VIR `36d26bc2` under the same Lean 4.34 rc2 and Verso `52c8c955`. W7 must archive/verify the named commit, manifest, and source hashes rather than consume the dirty consumer worktree; use the current generic VIR hosts where signatures fit and report the exact capture/import frontier. Preserve `a48bbb4a` unchanged, retained callbacks/error identity/unmount behavior, default options, and the existing physical adapter. Stop at the first concrete unsupported primitive or host-signature mismatch. No consumer/source/pin/protocol write, generic transport/widget shim, host ABI adoption, package publication, browser campaign, optimization lane, or performance/differential conclusion is included. The live shell now forwards server `Document.encode` directly (no decode/re-encode detour), but feature parity and automated widget qualification remain consumer-owned. |
