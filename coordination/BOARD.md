...
| `LCNF-MODULE-PRODUCT-CLOSURE` | W7/integration | VBP preview renderer capture, FIR lower/link | active | continuing decision `ROOT-W7-20260915-034`; capture `ca0b0a40`; worker `fb08ada2` | One outcome-driven investigation establishes whether real VBP `Renderer.render` compiles through FIR using its faithful upstream Lean/Lake module contexts. It follows one deterministic owner worklist from the renderer through complete source closure, FIR lowering, and a checked artifact/import frontier; an external native/VIR requirement remains explicit rather than fabricated. Each owner retains exact source/setup/plugin/toolchain/body/interface identity and the accepted worker boundary prevents conflicting native images from coexisting. W7 may repair demonstrated architectural deviations in its owned capture/emitter path with regressions, without a fresh approval for each diagnostic. At a demonstrated runtime frontier it may implement W7-owned coverage by reusing the exact existing semantic ABI and W6 concrete layouts—currently arbitrary-limb natural allocation and exact established aliases. The observed String/Substring frontier is instead followed generically from `@[extern]` to an exact upstream `@[export]` Lean provider only after export metadata and signature/implementation checks; it is not a license to recreate custom native helpers. No support may change layout, helper signature, ownership/effect rule, host import, or consumer. It reports concise traversal progress and stops only for a genuine shared semantic/ABI contract, consumer-owned change, unrepresentable upstream boundary, or the final result. No reset, generated-name seeding, weakened identity check, plugin suppression, durable/untrusted transport, W6 work, or implicit runtime/consumer change is authorized. |
