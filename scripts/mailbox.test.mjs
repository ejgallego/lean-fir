...
test("validates and summarizes a complete cross-project lifecycle", async () => {
  await withMailbox(async (mailbox) => {
    ...
    await put(mailbox, "FIR-ROOT-20260813-001", message({
    ...
    await put(mailbox, "ROOT-FIR-20260813-002", message({
      id: "ROOT-FIR-20260813-002",
      thread,
      reply: "FIR-ROOT-20260813-002",
...
test("rejects forks and replies after a terminal state", () => {
  ...
  }), "ROOT-FIR-20260813-002.md");
  const afterCancel = parseMessage(message({
    id: "FIR-ROOT-20260813-002",
    thread,
    reply: "ROOT-FIR-20260813-002",
    from: "fir/wasm-gen",
    to: "lean-zip/root",
    kind: "update",
    state: "in-progress",
...
test("rejects malformed operational metadata", () => {
  ...
  const result = validateMessages([parseMessage(source, "ROOT-FIR-20260813-001.md")]);
...
test("rejects lifecycle metadata on kinds that cannot own it", () => {
  const source = message({
    ...
    fields: {
      ...
      "parent-thread": "ROOT-FIR-20260812-001",
      "depends-on": "ROOT-FIR-20260812-002",
    },
  });
  const result = validateMessages([parseMessage(source, "ROOT-FIR-20260813-001.md")]);
...
test("rejects empty optional fields and cyclic or duplicate thread links", () => {
  ...
  const source = message({
    ...
    fields: {
      ...
      "depends-on": `${thread}, ROOT-FIR-20260812-001, ROOT-FIR-20260812-001`,
    },
  });
...
test("rejects normalized but impossible timestamp dates", () => {
  const source = message({
    id: "ROOT-FIR-20260230-001",
    time: "2026-02-30T14:30:00+02:00",
  });
  const result = validateMessages([parseMessage(source, "ROOT-FIR-20260230-001.md")]);
...
test("requires concrete sender and owner addresses", () => {
  ...
  const acknowledgement = parseMessage(message({
    id: "FIR-ROOT-20260813-001",
    thread: "ROOT-FIR-20260813-001",
    reply: "ROOT-FIR-20260813-001",
    from: "fir/wasm-gen",
    to: "lean-zip/root",
    kind: "acknowledgement",
    state: "claimed",
...
test("handoff transfers completion ownership while requester updates remain allowed", () => {
  ...
  assert.deepEqual(result.errors, []);
  assert.equal(result.threads[0].owner, "fir/wasm-proof");
});
...
test("same-project replies remain addressed within the shared project", () => {
  const thread = "FIR-FIR-20260813-001";
  const request = parseMessage(message({
    id: thread,
    from: "fir/root",
    to: "fir/*",
  ...
  const wrongDestination = parseMessage(message({
    ...
    state: "claimed",
    fields: { owner: "fir/wasm-gen" },
  }), "FIR-FIR-20260813-002.md");
  const result = validateMessages([request, wrongDestination]);
  assert(result.errors.some((error) => error.includes("must address project `fir`")));
});
...
test("rejects state regressions and terminal messages without disposition", () => {
  ...
  const completion = parseMessage(message({
    ...
    thread,
    reply: "FIR-ROOT-20260813-003",
    from: "fir/wasm-gen",
    to: "lean-zip/root",
    kind: "completion",
    state: "completed",
  }), "FIR-ROOT-20260813-004.md");
...
test("rejects obsolete directional ledgers as malformed messages", async () => {
  await withMailbox(async (mailbox) => {
    await writeFile(join(mailbox, "root-to-fir.md"), "# legacy\n");
...
test("reports malformed messages and filename mismatches", async () => {
  await withMailbox(async (mailbox) => {
    ...
    await writeFile(join(mailbox, "WRONG-FIR-20260813-001.md"), message({ id }));
    await writeFile(join(mailbox, "BROKEN-FIR-20260813-001.md"), "not front matter\n");
    const result = inspectMailbox(mailbox);
...
test("CLI lists active threads and hides terminal threads by default", async () => {
  await withMailbox(async (mailbox) => {
    ...
    await put(mailbox, open, message({
      ...
      fields: {
        "parent-thread": "ROOT-FIR-20260812-001",
        "depends-on": "ROOT-FIR-20260812-002, ROOT-FIR-20260812-003",
      },
    }));
    await put(mailbox, "FIR-ROOT-20260813-001", message({
      id: "FIR-ROOT-20260813-001",
    ...
    await put(mailbox, "ROOT-FIR-20260813-003", message({
      id: "ROOT-FIR-20260813-003",
      thread: closed,
      reply: closed,
